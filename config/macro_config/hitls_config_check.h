...
#error "[HiTLS] To use bn, the number of system bits must be specified first."
    ...
    !defined(HITLS_CRYPTO_SM2_CRYPT) && !defined(HITLS_CRYPTO_ED25519) && !defined(HITLS_CRYPTO_RSA))
    #error "[HiTLS] The encode must work with ecdsa or sm2_sign or sm2_crypt or ed25519 or rsa."
#endif
...
#if defined(HITLS_PKI_INFO) && !defined(HITLS_PKI_X509)
#error "[HiTLS] The info must work with x509."
#endif
