...
static int32_t SetExtCtrl(HITLS_X509_Ext *ext, int32_t cmd, void *val, int32_t valLen)
{
    ...
    switch (cmd) {
        ...
        case HITLS_X509_EXT_SET_AKI:
            return SetExt(ext, BSL_CID_CE_AUTHORITYKEYIDENTIFIER, &buff, sizeof(HITLS_X509_ExtAki), (EncodeExtCb)SetExtAki);
        case HITLS_X509_EXT_SET_SKI:
            return SetExt(ext, BSL_CID_CE_SUBJECTKEYIDENTIFIER, &buff, sizeof(HITLS_X509_ExtSki), (EncodeExtCb)SetExtSki);
