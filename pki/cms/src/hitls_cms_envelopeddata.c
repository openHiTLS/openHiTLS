...
void CMS_EnvelopedDataFree(CMS_EnvelopedData *envData)
{
    ...
    BSL_LIST_FREE(envData->recipientInfos, (BSL_LIST_PFUNC_FREE)RecipientInfoFree);
...
static bool MatchRecipientId(CMS_KeyTransRecipientInfo *ktri, HITLS_X509_Cert *cert)
{
...
static int32_t DecryptCekForRecipient(CMS_EnvelopedData *envData, const BSL_Param *param,
    uint8_t **cek, uint32_t *cekLen)
{
    bool found = false;
    CMS_RecipientInfo *recipInfo = BSL_LIST_GET_FIRST(envData->recipientInfos);
    ...
    while (recipInfo != NULL) {
        ...
        if (handler == NULL || handler->decrypt == NULL) {
            recipInfo = BSL_LIST_GET_NEXT(envData->recipientInfos);
            continue;
        }
        ...
        BSL_ERR_PUSH_ERROR(HITLS_CMS_ERR_ENVELOPEDDATA_DECRYPT_FAIL);
        return HITLS_CMS_ERR_ENVELOPEDDATA_DECRYPT_FAIL;
    }
    ...
    const BSL_Param *param, BSL_Buffer *plaintext)
{
    ...
    int32_t ret = DecryptCekForRecipient(envData, param, &envData->key.data, &envData->key.dataLen);
    if (ret != HITLS_PKI_SUCCESS) {
        return ret;
    }
...
static int32_t ParseOriginatorInfo(BSL_ASN1_Buffer *asn, CMS_OriginatorInfo *orig)
{
    ...
    *keyEncryAlg = cid;
    switch (cid) {
        case BSL_CID_RSA:
            ...
            keyEncryAlgParams->data = NULL;
            keyEncryAlgParams->dataLen = 0;
            break;
        case BSL_CID_RSAES_OAEP:
            /*
...
static int32_t ParseRecipientListItem(uint8_t tag, uint8_t *encode, uint32_t valueLen, CMS_RecipientInfos *list)
{
    ...
    return ret;
ERR:
    RecipientInfoFree(recip);
    return ret;
}

static int32_t ParseRecipientList(BSL_ASN1_Buffer *recipSet, CMS_RecipientInfos *list)
{
...
static int32_t ParseEnvData(BSL_ASN1_Buffer *asnArr, CMS_EnvelopedData *envData)
{
    ...
    if (asnArr[HITLS_CMS_ENVDATA_ORIGINATORINFO_IDX].tag != BSL_ASN1_TAG_EMPTY) {
        ...
        ret = ParseOriginatorInfo(&asnArr[HITLS_CMS_ENVDATA_ORIGINATORINFO_IDX], envData->originatorInfo);
        if (ret != HITLS_PKI_SUCCESS) {
            return ret;
        }
    }
    // parse recipientInfos
    ret = ParseRecipientList(&asnArr[HITLS_CMS_ENVDATA_RECIPIENTINFOS_IDX], envData->recipientInfos);
    if (ret != HITLS_PKI_SUCCESS) {
        return ret;
    }
...
static int32_t EncodeOriginatorInfo(CMS_OriginatorInfo *originator, BSL_ASN1_Buffer *encode)
{
    ...
    uint8_t **cek, uint32_t *cekLen)
{
    ...
    ret = GetCtxParam(param, HITLS_CMS_PARAM_PRIVATE_KEY,
        sizeof(CRYPT_EAL_PkeyCtx *), (void **)(&privateKey));
    if (ret != HITLS_PKI_SUCCESS) {
        return ret;
    }
    if (cert == NULL || !MatchRecipientId(recip->d.ktri, cert)) {
        return HITLS_CMS_ERR_RECIPIENT_MISMATCH;
    }
    if (privateKey == NULL) {
        BSL_ERR_PUSH_ERROR(HITLS_CMS_ERR_KTRI_PRIKEY_REQUIRED);
        return HITLS_CMS_ERR_KTRI_PRIKEY_REQUIRED;
    }
...
static int32_t EncodeRecipientList(CMS_RecipientInfos *list, BSL_ASN1_Buffer *encode)
{
...
static int32_t PrepareOriginatorInfo(CMS_EnvelopedData *envData, BSL_ASN1_Buffer *originatorInfo)
{
    if (envData->originatorInfo != NULL) {
        if ((BSL_LIST_COUNT(envData->originatorInfo->certs) > 0) ||
            ...
            return EncodeOriginatorInfo(envData->originatorInfo, originatorInfo);
        }
    }
    return HITLS_PKI_SUCCESS;
}
...
static int32_t CMS_GenEnvDataBuffAsn1(CMS_EnvelopedData *envData, BSL_Buffer *envBuff)
{
    ...
    // recipient infos
    ret = EncodeRecipientList(envData->recipientInfos, &recipientInfos);
    if (ret != HITLS_PKI_SUCCESS) {
        goto EXIT;
    }
...
static int32_t CreateKtriAndWrapCek(CMS_RecipientInfo **recipient, const BSL_Param *param, BSL_Buffer *cek)
{
    ...
    *recipient = CMS_RecipientInfoNew(CMS_RECIPIENT_TYPE_KTRI, HITLS_CMS_FLAG_GEN);
    ...
    if (cert == NULL) {
        ret = HITLS_CMS_ERR_KTRI_CERT_REQUIRED;
        BSL_ERR_PUSH_ERROR(ret);
        goto ERR;
    }
...
static int32_t AddRecipientAndWrapCek(CMS_EnvelopedData *envData, const BSL_Param *param)
{
    if (envData == NULL || envData->recipientInfos == NULL || envData->key.data == NULL || param == NULL) {
        BSL_ERR_PUSH_ERROR(HITLS_CMS_ERR_NULL_POINTER);
        return HITLS_CMS_ERR_NULL_POINTER;
    }
    CMS_RecipientInfo *recipient = NULL;
    int32_t ret = CreateRecipientAndWrapCek(&envData->key, param, &recipient);
    if (ret != HITLS_PKI_SUCCESS) {
        return ret;
    }
    ret = BSL_LIST_AddElement(envData->recipientInfos, recipient, BSL_LIST_POS_END);
    if (ret != BSL_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        RecipientInfoFree(recipient);
        return ret;
...
static int32_t CheckRecipientsNotEmpty(const CMS_EnvelopedData *envData)
{
    if (envData == NULL || envData->recipientInfos == NULL) {
        BSL_ERR_PUSH_ERROR(HITLS_CMS_ERR_NULL_POINTER);
        return HITLS_CMS_ERR_NULL_POINTER;
    }
    if (BSL_LIST_GET_FIRST(envData->recipientInfos) == NULL) {
        BSL_ERR_PUSH_ERROR(HITLS_CMS_ERR_NO_RECIPIENT);
        return HITLS_CMS_ERR_NO_RECIPIENT;
    }
    return HITLS_PKI_SUCCESS;
}
...
int32_t HITLS_CMS_DataEncrypt(HITLS_CMS *cms, const BSL_Buffer *plaintext, const BSL_Param *optionalParam)
{
    ...
    if (envData->state == HITLS_CMS_ENCRYPT_INIT) {
        return AddRecipientAndWrapCek(envData, optionalParam);
    }
    ...
    ret = AddRecipientAndWrapCek(envData, optionalParam);
    if (ret != HITLS_PKI_SUCCESS) {
        goto ERR;
    }
...
static int32_t EnvelopedData_EncryptFinal(HITLS_CMS *cms, const BSL_Param *param, BSL_Buffer *ciphertext)
{
    ...
    int32_t ret = CheckRecipientsNotEmpty(envData);
    if (ret != HITLS_PKI_SUCCESS) {
        goto CLEANUP;
    }
    // Finalize encryption
...
static int32_t EnvelopedData_DecryptInit(HITLS_CMS *cms, const BSL_Param *param)
{
    ...
    uint8_t *cek = NULL;
    uint32_t cekLen = 0;
    int32_t ret = DecryptCekForRecipient(envData, param, &cek, &cekLen);
    if (ret != HITLS_PKI_SUCCESS) {
        return ret;
    }
    ClearEnvDataKey(envData);
