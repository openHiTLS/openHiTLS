...
static CRYPT_DECODER_Node *CreateDecoderNode(const char *format, const char *type, const char *targetFormat,
    const char *targetType, BSL_Param *input)
{
...
static int32_t SetSourceBufferData(CRYPT_DECODER_PoolCtx *poolCtx, void *val, size_t valLen)
{
    if (valLen != sizeof(BSL_Param *)) {
        BSL_ERR_PUSH_ERROR(CRYPT_INVALID_ARG);
        return CRYPT_INVALID_ARG;
    }
    poolCtx->input = val;
...
static int32_t SetFlagFreeOutData(CRYPT_DECODER_PoolCtx *poolCtx, void *val, int32_t valLen)
{
    ...
    CRYPT_DECODER_Node *currNode = BSL_LIST_GET_PREV(poolCtx->decoderPath);
    if (currNode == NULL) {
        return CRYPT_SUCCESS;
    }
    if (currNode->decoderCtx == NULL) {
        BSL_ERR_PUSH_ERROR(CRYPT_INVALID_ARG);
        return CRYPT_INVALID_ARG;
    }
    ...
    BSL_Param param[2] = {
        {CRYPT_PARAM_DECODE_FLAG_FREE_OUTDATA, BSL_PARAM_TYPE_BOOL, val, 0, 0},
        BSL_PARAM_END
    };
    int32_t ret = CRYPT_DECODE_SetParam(currNode->decoderCtx, param);
...
int32_t CRYPT_DECODE_PoolCtrl(CRYPT_DECODER_PoolCtx *poolCtx, int32_t cmd, void *val, int32_t valLen)
{
    ...
    switch (cmd) {
        case CRYPT_DECODE_POOL_CMD_SET_SOURCE_DATA:
            return SetSourceBufferData(poolCtx, val, valLen);
...
{
    int32_t ret;
    currNode->decoderCtx->decoderState = CRYPT_DECODER_STATE_TRING;
    /* Convert password buffer to parameter if provided */
    BSL_Param *decoderParam = NULL;
    ret = CRYPT_DECODE_Decode(currNode->decoderCtx, currNode->inData.data, &decoderParam);
    if (ret == CRYPT_SUCCESS) {
...
static int32_t ProcessEachProviderDecoder(CRYPT_EAL_ProvMgrCtx *ctx, void *args)
{
    ...
    for (int32_t i = 0; algInfos != NULL && algInfos[i].algId != 0; i++) {
        ...
        ret = CRYPT_DECODE_ParseDecoderAttr(algInfos[i].attr, &attrInfo);
        if (ret != CRYPT_SUCCESS) {
            BSL_ERR_PUSH_ERROR(ret);
            return ret;
        }
        decoderCtx = CRYPT_DECODE_NewDecoderCtxByMethod(algInfos[i].implFunc, ctx, attrInfo.inFormat,
            attrInfo.inType, attrInfo.outFormat, attrInfo.outType);
        BSL_SAL_Free(attrInfo.attrName);
        if (decoderCtx == NULL) {
            BSL_ERR_PUSH_ERROR(CRYPT_MEM_ALLOC_FAIL);
            return CRYPT_MEM_ALLOC_FAIL;
        }
...
int32_t CRYPT_DECODE_PoolDecode(CRYPT_DECODER_PoolCtx *poolCtx, BSL_Param **outParam)
{
    if (poolCtx == NULL || outParam == NULL) {
        BSL_ERR_PUSH_ERROR(CRYPT_NULL_INPUT);
        return CRYPT_NULL_INPUT;
    }
    ...
    CRYPT_DECODER_Node *initialNode = CreateDecoderNode(poolCtx->inputFormat, poolCtx->inputType,
        poolCtx->targetFormat, poolCtx->targetType, poolCtx->input);
    if (initialNode == NULL) {
        BSL_ERR_PUSH_ERROR(CRYPT_MEM_ALLOC_FAIL);
        return CRYPT_MEM_ALLOC_FAIL;
    }
