    ...
    const char *inFormat, const char *inType, const char *outFormat, const char *outType)
{
    ...
    if (inFormat != NULL) {
        ctx->inFormat = BSL_SAL_Dump(inFormat, strlen(inFormat) + 1);
        if (ctx->inFormat == NULL) {
            BSL_ERR_PUSH_ERROR(CRYPT_MEM_ALLOC_FAIL);
            goto ERR;
        }
    }
    if (inType != NULL) {
        ctx->inType = BSL_SAL_Dump(inType, strlen(inType) + 1);
        if (ctx->inType == NULL) {
            BSL_ERR_PUSH_ERROR(CRYPT_MEM_ALLOC_FAIL);
            goto ERR;
        }
    ...
    if (outFormat != NULL) {
        ctx->outFormat = BSL_SAL_Dump(outFormat, strlen(outFormat) + 1);
        if (ctx->outFormat == NULL) {
            BSL_ERR_PUSH_ERROR(CRYPT_MEM_ALLOC_FAIL);
            goto ERR;
        }
    }
    if (outType != NULL) {
        ctx->outType = BSL_SAL_Dump(outType, strlen(outType) + 1);
        if (ctx->outType == NULL) {
            BSL_ERR_PUSH_ERROR(CRYPT_MEM_ALLOC_FAIL);
            goto ERR;
        }
...
CRYPT_DECODER_Ctx *CRYPT_DECODE_ProviderNewCtx(CRYPT_EAL_LibCtx *libCtx, int32_t keyType, const char *inFormat,
    const char *inType, const char *outFormat, const char *outType, const char *attrName)
{
    ...
    CRYPT_DECODE_ParseDecoderAttr(attrName, &attrInfo);
    const char *inFormat1 = inFormat == NULL ? attrInfo.inFormat : inFormat;
    const char *inType1 = inType == NULL ? attrInfo.inType : inType;
    const char *outFormat1 = outFormat == NULL ? attrInfo.outFormat : outFormat;
    const char *outType1 = outType == NULL ? attrInfo.outType : outType;
    CRYPT_DECODER_Ctx *ctx = CRYPT_DECODE_NewDecoderCtxByMethod(funcsDecoder, mgrCtx, inFormat1, inType1,
        outFormat1, outType1);
...
void CRYPT_DECODE_Free(CRYPT_DECODER_Ctx *ctx)
{
    ...
    BSL_SAL_Free(ctx->inFormat);
    BSL_SAL_Free(ctx->inType);
    BSL_SAL_Free(ctx->outFormat);
    BSL_SAL_Free(ctx->outType);
    BSL_SAL_Free(ctx);
}
