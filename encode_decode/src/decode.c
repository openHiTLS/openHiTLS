...
CRYPT_DECODER_Ctx *CRYPT_DECODE_ProviderNewCtx(CRYPT_EAL_LibCtx *libCtx, int32_t keyType, const char *attrName)
{
    ...
    const char *inFormat = attrInfo.inFormat;
    const char *inType = attrInfo.inType;
    const char *outFormat = attrInfo.outFormat;
    const char *outType = attrInfo.outType;
    CRYPT_DECODER_Ctx *ctx = CRYPT_DECODE_NewDecoderCtxByMethod(funcsDecoder, mgrCtx, inFormat, inType,
        outFormat, outType);
