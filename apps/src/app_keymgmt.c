...
#define APP_KEYMGMT_UUID_STR_SIZE (2 * (HITLS_APP_UUID_LEN) + 1)
...
static char *GetKeyFullPath(const char *workPath, const char *uuid, const char *suffix)
{
    ...
    int ret = snprintf(path, APP_MAX_PATH_LEN, "%s/%s%s", workPath, uuid, suffix);
    if (ret < 0 || ret >= APP_MAX_PATH_LEN) {
        BSL_SAL_Free(path);
    ...
    char **uuid)
{
    ...
    uuidStr = (char *)BSL_SAL_Malloc(APP_KEYMGMT_UUID_STR_SIZE);
    if (uuidStr == NULL) {
        AppPrintError("keymgmt: Failed to allocate memory.\n");
...
static int32_t SplitUuidString(const char *uuid, BslList **uuidList)
{
    ...
    char sep[] = ",";
    char *nextTmp = NULL;
    char *tmp = strtok_r(src, sep, &nextTmp);
    while (tmp != NULL) {
...
static int32_t GetAllKeyUuids(const char *workPath, BslList **fileList)
{
    ...
    for (dp = readdir(dir); dp != NULL; dp = readdir(dir)) {
        if (dp->d_type == DT_REG && IsP12File(dp->d_name)) {
            uuid = BSL_SAL_Dump(dp->d_name, APP_KEYMGMT_UUID_STR_SIZE);
            if (uuid == NULL) {
                closedir(dir);
                BSL_LIST_FREE(list, BSL_SAL_Free);
                AppPrintError("keymgmt: Failed to dump single uuid string.\n");
            ...
            uuid[APP_KEYMGMT_UUID_STR_SIZE - 1] = '\0';
