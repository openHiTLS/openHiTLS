测试工程依赖编译源码，参考[《构建及安装指导》](./1_构建及安装指导.md)准备构建源码编译依赖的环境等，确保源码能够得到正确编译。
...
测试结果中 `skipped` 表示该用例依赖的特性宏在当前构建配置中未启用（见 `cmake/hitls_options.cmake`），属正常跳过，不代表失败；如需执行，开启对应特性宏后重新构建。如果发现社区仓库问题，可先在仓库issue查看是否有问题单，如果不存在问题单则提单跟踪。
