...
**功能**：建立 TLS、TLCP 或 DTLCP 客户端连接，支持指定密码套件、信任链和客户端证书。

**用法**：

```bash
hitls s_client -host <主机> [-port <端口>] [-tls|-tlcp|-dtlcp]
...
**功能**：启动 TLS、TLCP 或 DTLCP 服务端，支持指定密码套件、服务端证书以及客户端证书验证策略。

**用法**：

```bash
hitls s_server [-accept <主机:端口>] [-port <端口>] [-tls|-tlcp|-dtlcp]
    [-cipher <密码套件>] [-CAfile <文件>] [-chainCAfile <文件>]
    [-cert <文件> -key <文件>] [-noverify] [-accept_once] [-state]
```

`-cert/-key` 配置 TLS 服务端证书和私钥。服务端默认验证客户端证书；使用 `-CAfile/-chainCAfile` 指定信任链，或使用 `-noverify` 关闭客户端证书验证。密码套件和 TLCP 证书选项的规则与 `s_client` 相同。
