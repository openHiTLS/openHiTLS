...
```
hitls s_client -host <主机名|IP> [-port <端口>] [-tls | -tlcp | -dtlcp] [-cipher <密码套件列表>] [-CAfile <CA证书文件>] [-chainCAfile <CA链文件>] [-noverify] [-tlcp_enc_cert <文件> -tlcp_enc_key <文件>] [-tlcp_sign_cert <文件> -tlcp_sign_key <文件>] [-certform PEM|DER] [-keyform PEM|DER] [-quiet] [-state] [-prexit] [-provider <名称>] [-provider-path <路径>] [-provider-attr <属性>]
```

**支持的选项**：

- `-help`：显示帮助信息
- `-host <主机名｜IP>`：必需。目标服务器的主机名或IP地址
- `-port <端口>`：目标服务器端口号，默认4433
- `-tls`：使用TLS协议，默认为此值
- `-tlcp`：使用TLCP协议，默认为TLS协议
- `-dtlcp`：使用DTLCP协议，默认为TLS协议
- `-cipher <套件列表>`：指定密码套件，多个套件用`:`分隔，如果不指定此选项将使用协议默认套件
- `-CAfile <文件>`：单个CA证书文件，用于验证服务器证书，如果不指定此选项将使用系统默认CA
