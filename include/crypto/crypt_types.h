...
typedef enum {
    ...
    CRYPT_CTRL_GET_RSA_SALTLEN,         /**< Obtain the real salt len in pss mode. The salt len can be set to -1, -2,
                                             and -3, these values require additional conversion during encoding or other
                                             operations.if salt len = -3, will return the max salt len. */
