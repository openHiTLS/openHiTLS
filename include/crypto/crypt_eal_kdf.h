 ...
 *         Fails: NULL.
 */
CRYPT_EAL_KdfCtx *CRYPT_EAL_KdfNewCtxWithLib(CRYPT_EAL_LibCtx *libCtx, const char *attrName, int32_t algId);
 ...
 *          For other error codes, see crypt_errno.h.
 */
int32_t CRYPT_EAL_KdfDreive(CRYPT_EAL_KdfCtx *ctx, uint8_t *key, uint32_t keyLen);
 ...
 *          For other error codes, see crypt_errno.h.
 */
int32_t CRYPT_EAL_KdfKdfCtrl(CRYPT_EAL_KdfCtx *ctx, int32_t cmd, void *val, uint32_t valLen);
