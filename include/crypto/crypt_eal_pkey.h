 ...
 * @param userData [IN] Reserved user data, currently NULL.
 ...
 */
typedef int32_t (*CRYPT_EAL_PKEY_CB)(void *userData, BSL_Param *param);
 ...
 * @attention Stateful hash-based signature algorithms such as XMSS, XMSSMT,
 * LMS, and HSS must not copy private signing state. Their algorithm-level copy
 * creates a public-key-only context that can verify signatures but cannot sign.
 *
 ...
 * @attention
 * Stateful hash-based signature algorithms such as XMSS, XMSSMT, LMS, and HSS
 * must not duplicate private signing state. Their algorithm-level duplication
 * creates a public-key-only context that can verify signatures but cannot sign.
 *
...
/**
 * @ingroup crypt_eal_pkey
 * @brief   Sign hash data(RSA9796-2 Signature is not supported)
 *
 * @param   pkey      [IN] Key session
...
/**
 * @ingroup crypt_eal_pkey
 * @brief   Verify the signature of the hash data(RSA9796-2 Signature is not supported)
 *
 * @param   pkey     [IN] Key session
 * @param   hash     [IN] Hash data
 * @param   hashLen  [IN] Hash length.
