...
typedef enum {
    ...
    HITLS_AUTH_PRIVPASS_CRYPTO_RSA = 0x1001,
    HITLS_AUTH_PRIVPASS_CRYPTO_SHA256 = 0x2001,
    HITLS_AUTH_PRIVPASS_CRYPTO_SHA384 = 0x2002,
 ...
 */
typedef void *(*HITLS_Auth_PrivPassNewPkeyCtx)(void *libCtx, int32_t algId);
 ...
 */
typedef int32_t (*HITLS_Auth_PrivPassDigest)(void *libCtx, int32_t algId, const uint8_t *input, uint32_t inputLen,
 ...
 *          other error codes, failed.
 */
typedef int32_t (*HITLS_Auth_PrivPassDecodePubKey)(void *libCtx, uint8_t *pubKey, uint32_t pubKeyLen, void **pkeyCtx);

/**
 * @ingroup auth_privpass
 * @brief Decodes a private key and gen a key ctx. The default algorithm callback implementation is supported only from
 *        PEM-encoded PKCS #8 RSA Issuer Private Key.
 *
 * @param   libCtx [IN] Library context
 ...
 *          other error codes, failed.
 */
typedef int32_t (*HITLS_Auth_PrivPassDecodePrvKey)(void *libCtx, void *param, uint8_t *prvKey, uint32_t prvKeyLen,
    void **pkeyCtx);
