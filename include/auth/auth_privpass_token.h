...
/* Token types for different stages of the Private Pass protocol */
typedef enum {
    HITLS_AUTH_PRIV_PASS_PRV_VERIFY_TOKENS = 0, // Private key verification tokens
    HITLS_AUTH_PRIV_PASS_PUB_VERIFY_TOKENS = 1, // Public key verification tokens
...
typedef enum {
    HITLS_AUTH_PRIVPASS_INVALID_CRYPTO_ALGID = 0,
    HITLS_AUTH_PRIVPASS_CRYPTO_ALGID_SHA256 = 1,
    HITLS_AUTH_PRIVPASS_CRYPTO_ALGID_SHA384 = 2,
...
/**
 * @ingroup auth_privpass
 * @brief Decodes a public key from a DER-encoded SubjectPublicKeyInfo (SPKI) object using the RSASSA-PSS OID,
 *        and creates a key context.
 *
 ...
 *          other error codes, failed.
 */
typedef int32_t (*HITLS_Auth_PrivPassDecodePubKey)(uint8_t *pubKey, uint32_t pubKeyLen, void **pkeyCtx);

/**
 * @ingroup auth_privpass
 * @brief Decodes a private key from PEM-encoded PKCS #8 RSA Issuer Private Key and creates a key context.
 *
 * @param   prvKey [IN] A PEM-encoded PKCS #8 RSA Issuer Private Key
 ...
 *          other error codes, failed.
 */
typedef int32_t (*HITLS_Auth_PrivPassDecodePrvKey)(uint8_t *prvKey, uint32_t prvKeyLen, void **pkeyCtx);
 ...
 *
 * @param   ctx [IN] PrivPass context
 * @param   ski [IN] A PEM-encoded PKCS #8 RSA Issuer Private Key
