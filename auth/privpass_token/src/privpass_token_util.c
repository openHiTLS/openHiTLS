...
#define MAX_ISSUER_NAME_LEN 65535
#define MAX_REDEMPTION_LEN 32
#define MAX_ORIGIN_INFO_LEN 65535
    ...
    const uint8_t *buffer, uint32_t buffLen)
{
    ...
    // tokenType (2 bytes) + issuerNameLen(2 bytes) + redemptionLen(1 byte)
    if (remainLen < 2 + 2 + 1) {
    ...
    curr += 1;
    remainLen -= 1;
    if (remainLen < redemptionLen || redemptionLen > MAX_REDEMPTION_LEN) {
        ret = HITLS_AUTH_INVALID_TOKEN_CHALLENGE;
        goto ERR;
    }
...
static int32_t CheckTokenChallengeParam(HITLS_Auth_PrivPassCtx *ctx, const PrivPass_TokenChallenge *challenge)
{
    ...
    if (challenge->issuerName.dataLen == MAX_ISSUER_NAME_LEN || challenge->issuerName.dataLen > MAX_ISSUER_NAME_LEN ||
        challenge->originInfo.dataLen > MAX_ORIGIN_INFO_LEN || challenge->redemption.dataLen > MAX_REDEMPTION_LEN) {
        BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_TOKEN_CHALLENGE);
        return HITLS_AUTH_INVALID_TOKEN_CHALLENGE;
    }
    return HITLS_AUTH_SUCCESS;
    ...
    uint8_t *buffer, uint32_t *outBuffLen)
{
    ...
    BSL_Uint16ToByte(challenge->tokenType, curr);
    curr += 2;

    // Write IssuerName length (2 bytes) and data
    BSL_Uint16ToByte(challenge->issuerName.dataLen, curr);
    curr += 2;
    ...
    *curr = challenge->redemption.dataLen;
    ...
    uint32_t digestLen = PRIVPASS_TOKEN_SHA256_SIZE;
    ret = ctx->method.digest(CRYPT_MD_SHA256, tmp, totalLen, ctx->challengeDigest, &digestLen);
    if (ret != HITLS_AUTH_SUCCESS) {
        BSL_SAL_FREE(tmp);
        BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_CRYPTO_METHOD);
        return HITLS_AUTH_INVALID_CRYPTO_METHOD;
