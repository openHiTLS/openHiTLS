...
static int32_t DecodeIssuerName(uint8_t **issueName, uint32_t *issuerNameLen, const uint8_t **curr,
    uint32_t *remainLen)
{
    ...
    uint8_t *buffer, uint32_t *outBuffLen)
{
    ...
    *curr++ = (uint16_t)challenge->redemption.dataLen;
...
static int32_t GetTokenChallengeContent(PrivPass_TokenChallenge *challenge, BSL_Param *param, uint32_t target,
...
static int32_t CopyTokenContent(PrivPass_TokenInstance *token, BSL_Param *param, uint32_t target, uint8_t *targetBuff,
    uint32_t targetLen)
{
    BSL_Param *output = BSL_PARAM_FindParam(param, target);
