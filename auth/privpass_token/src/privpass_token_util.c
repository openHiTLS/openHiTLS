    ...
    uint32_t *buffLen)
{
    ...
    if (tokenChallengeReq->challengeReqLen == 0) {
        BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_TOKEN_CHALLENGE_REQ);
        return HITLS_AUTH_INVALID_TOKEN_CHALLENGE_REQ;
    }
...
static int32_t ValidateInitialParams(HITLS_Auth_PrivPassCtx *ctx, int32_t remainLen) 
{
    if (ctx == NULL) {
...
static int32_t DecodeTokenTypeAndValidate(const uint8_t **curr, int32_t *remainLen) 
{
    uint16_t tokenType = BSL_ByteToUint16(*curr);
...
static int32_t DecodeIssuerName(uint8_t **issueName, uint16_t *issuerNameLen, 
    const uint8_t **curr, int32_t *remainLen) 
{
...
static int32_t DecodeRedemption(uint8_t **redemption, uint8_t *redemptionLen,
    const uint8_t **curr, int32_t *remainLen) 
{
    if (*remainLen < 1) {
        BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_TOKEN_CHALLENGE);
    ...
    *remainLen -= 1;
    ...
    if (*remainLen < *redemptionLen || 
        (*redemptionLen != PRIVPASS_REDEMPTION_LEN && *redemptionLen != 0)) {
        BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_TOKEN_CHALLENGE);
        return HITLS_AUTH_INVALID_TOKEN_CHALLENGE;
    }
...
static int32_t DecodeOriginInfo(uint8_t **originInfo, uint16_t *originInfoLen,
    const uint8_t **curr, int32_t *remainLen) 
{
    if (*remainLen == 0) {
        return HITLS_AUTH_SUCCESS;
...
static int32_t DecodeTokenChallenge(HITLS_Auth_PrivPassCtx *ctx, 
    PrivPass_TokenChallenge *challenge, const uint8_t *buffer, uint32_t buffLen)
{
    ...
    if ((ret = DecodeTokenTypeAndValidate(&curr, &remainLen)) != HITLS_AUTH_SUCCESS ||
        (ret = DecodeIssuerName(&issueName, &issuerNameLen, &curr, &remainLen)) != HITLS_AUTH_SUCCESS ||
        (ret = DecodeRedemption(&redemption, &redemptionLen, &curr, &remainLen)) != HITLS_AUTH_SUCCESS ||
        (ret = DecodeOriginInfo(&originInfo, &originInfoLen, &curr, &remainLen)) != HITLS_AUTH_SUCCESS) {
        goto ERR;
    }

    ret = ctx->method.digest(NULL, HITLS_AUTH_PRIVPASS_CRYPTO_SHA256, buffer,
        buffLen, ctx->challengeDigest, &digestLen);
    if (ret != HITLS_AUTH_SUCCESS) {
    ...
    challenge->tokenType = BSL_ByteToUint16(buffer);
    ...
    uint8_t *buffer, uint32_t *outBuffLen)
{
    ...
    *curr++ = challenge->redemption.dataLen;
    ...
    uint32_t digestLen = PRIVPASS_TOKEN_SHA256_SIZE;
    ret = ctx->method.digest(NULL, HITLS_AUTH_PRIVPASS_CRYPTO_SHA256, tmp, totalLen, ctx->challengeDigest, &digestLen);
    if (ret != HITLS_AUTH_SUCCESS) {
        BSL_SAL_Free(tmp);
        BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_CRYPTO_METHOD);
        return HITLS_AUTH_INVALID_CRYPTO_METHOD;
...
HITLS_Auth_PrivPassCtx *HITLS_Auth_PrivPassCtxNew(int32_t protocolType)
{
    if (protocolType != HITLS_AUTH_PRIV_PASS_PUB_VERIFY_TOKENS) {
        BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_TOEKN_PROTOCOL_TYPE);
        return NULL;
    }
