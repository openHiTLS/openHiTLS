    ...
    const uint8_t *buffer, uint32_t buffLen)
{
    ...
    // MinLength: tokenType (2 bytes) + issuerNameLen(2 bytes) + redemptionLen(1 byte) + originInfoLen(2 bytes)
    ...
    if (remainLen < 2) { // remainLen >= originInfoLen (2 bytes)
        ret = HITLS_AUTH_INVALID_TOKEN_CHALLENGE;
        goto ERR;
    }
    ...
    if (remainLen - originInfoLen != 0) {
        ret = HITLS_AUTH_INVALID_TOKEN_CHALLENGE;
        goto ERR;
    }
EXIT:
    ret = ctx->method.digest(HITLS_AUTH_PRIVPASS_CRYPTO_ALGID_SHA256, buffer, buffLen, ctx->challengeDigest, &digestLen);
    if (ret != HITLS_AUTH_SUCCESS) {
