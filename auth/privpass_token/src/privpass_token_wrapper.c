...
int32_t PrivPassUnblind(void *pkeyCtx, const uint8_t *blindedData, uint32_t blindedDataLen, uint8_t *data,
    uint32_t *dataLen)
{
...
int32_t PrivPassDecodePubKey(uint8_t *pubKey, uint32_t pubKeyLen, void **pkeyCtx)
{
...
int32_t PrivPassDecodePrvKey(uint8_t *prvKey, uint32_t prvKeyLen, void **pkeyCtx)
{
    ...
    return CRYPT_EAL_DecodeBuffKey(BSL_FORMAT_PEM, CRYPT_PRIKEY_PKCS8_UNENCRYPT, &encode, NULL, 0, ctx);
}
