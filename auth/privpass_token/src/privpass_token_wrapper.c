...
{
    switch (algId) {
        case HITLS_AUTH_PRIVPASS_CRYPTO_ALGID_SHA256:
            return CRYPT_MD_SHA256;
        case HITLS_AUTH_PRIVPASS_CRYPTO_ALGID_SHA384:
...
int32_t PrivPassPubDigest(int32_t algId, const uint8_t *input, uint32_t inputLen, uint8_t *digest, uint32_t *digestLen)
{
    ...
    if (cryptAlgId == HITLS_AUTH_INVALID_ALG) {
        BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_ALG);
        return HITLS_AUTH_INVALID_ALG;
    }
...
int32_t PrivPassPubDecodePubKey(uint8_t *pubKey, uint32_t pubKeyLen, void **pkeyCtx)
{
...
int32_t PrivPassPubDecodePrvKey(uint8_t *prvKey, uint32_t prvKeyLen, void **pkeyCtx)
{
