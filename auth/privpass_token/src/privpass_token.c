    ...
    HITLS_Auth_PrivPassToken **challenge)
{
    ...
    if (challengeLen > UINT32_MAX) {
        HITLS_Auth_PrivPassTokenFree(output);
        BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_TOKEN_CHALLENGE_PARAM);
        return HITLS_AUTH_INVALID_TOKEN_CHALLENGE_PARAM;
    }
    *challenge = output;
    return HITLS_AUTH_SUCCESS;
}
    ...
    HITLS_Auth_PrivPassToken **tokenRequest)
{
    if (ctx == NULL || tokenChallenge == NULL || tokenChallenge->type != HITLS_AUTH_PRIVPASS_TOKEN_CHALLENGE
        || tokenRequest == NULL || *tokenRequest != NULL || ctx->method.blind == NULL  || ctx->method.digest == NULL ||
        ctx->method.random == NULL) {
        BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_INPUT);
        return HITLS_AUTH_INVALID_INPUT;
    }
    ...
    uint8_t *challengeDigest)
{
    ...
    ret = ctx->method.digest(NULL, HITLS_AUTH_PRIVPASS_CRYPTO_SHA256, challenge, challengeLen, challengeDigest,
        &challengeDigestLen);
    BSL_SAL_Free(challenge);
    if (ret != HITLS_AUTH_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
    ...
    HITLS_Auth_PrivPassToken **tokenRequest)
{
    ...
    ret = ctx->method.blind(ctx->pubKeyCtx, HITLS_AUTH_PRIVPASS_CRYPTO_SHA384, tokenInput, HITLS_AUTH_PRIVPASS_TOKEN_INPUT_LEN,
...
int32_t HITLS_Auth_PrivPassSetPubkey(HITLS_Auth_PrivPassCtx *ctx, uint8_t *pki, uint32_t pkiLen)
{
    ...
    void *pubKeyCtx = NULL;
    int32_t ret = ctx->method.decodePubKey(NULL, pki, pkiLen, &pubKeyCtx);
    if (ret != HITLS_AUTH_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ...
    ret = ctx->method.digest(NULL, HITLS_AUTH_PRIVPASS_CRYPTO_SHA256, pki, pkiLen, ctx->tokenKeyId,
        &tokenKeyIdLen);
    if (ret != HITLS_AUTH_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        goto ERR;
...
int32_t HITLS_Auth_PrivPassSetPrvkey(HITLS_Auth_PrivPassCtx *ctx, void *param, uint8_t *ski, uint32_t skiLen)
{
    ...
    void *prvKeyCtx = NULL;
    int32_t ret = ctx->method.decodePrvKey(NULL, param, ski, skiLen, &prvKeyCtx);
    if (ret != HITLS_AUTH_SUCCESS) {
        return ret;
    }
    if (ctx->pubKeyCtx != NULL) {
