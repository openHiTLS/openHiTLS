    ...
    HITLS_Auth_PrivPassToken **challenge)
{
    ...
    if ((temp = BSL_PARAM_FindConstParam(param, AUTH_PARAM_PRIV_PASS_TOKENTYPE)) != NULL) {
        uint32_t tokenTypeLen = (uint32_t)sizeof(tokenChallenge->tokenType);
        uint16_t tokenType = 0;
        ret = BSL_PARAM_GetValue(temp, AUTH_PARAM_PRIV_PASS_TOKENTYPE, BSL_PARAM_TYPE_UINT16,
    ...
    if ((temp = BSL_PARAM_FindConstParam(param, AUTH_PARAM_PRIV_PASS_ISSUERNAME)) != NULL) {
        if (temp->useLen == 0 || temp->useLen > PRIVPASS_MAX_ISSUER_NAME_LEN) {
            ret = HITLS_AUTH_INVALID_ISSUER_NAME;
            BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_ISSUER_NAME);
            goto ERR;
        }
        ...
        ret = HITLS_AUTH_NO_TOKEN_CHALLENGE_ISSUERNAME;
        BSL_ERR_PUSH_ERROR(HITLS_AUTH_NO_TOKEN_CHALLENGE_ISSUERNAME);
        goto ERR;
    }
    // redemption can be NULL.
    if ((temp = BSL_PARAM_FindConstParam(param, AUTH_PARAM_PRIV_PASS_REDEMPTION)) != NULL) {
        if (temp->useLen != 0 && temp->useLen != PRIVPASS_REDEMPTION_LEN) {
            ret = HITLS_AUTH_INVALID_REDEMPTION;
            BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_REDEMPTION);
            goto ERR;
        }
    ...
    if ((temp = BSL_PARAM_FindConstParam(param, AUTH_PARAM_PRIV_PASS_ORIGININFO)) != NULL && temp->useLen > 0) {
        if (temp->useLen > PRIVPASS_MAX_ORIGIN_INFO_LEN) {
            ret = HITLS_AUTH_INVALID_ORIGIN_INFO;
            BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_ORIGIN_INFO);
            goto ERR;
        }
    ...
    HITLS_Auth_PrivPassToken **tokenRequest)
{
    if (ctx == NULL || tokenChallenge == NULL || tokenChallenge->type != HITLS_AUTH_PRIVPASS_TOKEN_CHALLENGE
        || tokenRequest == NULL || *tokenRequest != NULL || ctx->method.blind == NULL || ctx->method.random == NULL) {
        BSL_ERR_PUSH_ERROR(HITLS_AUTH_INVALID_INPUT);
        return HITLS_AUTH_INVALID_INPUT;
    }
    ...
    HITLS_Auth_PrivPassToken **tokenRequest)
{
    ...
    ret = ctx->method.blind(ctx->pubKeyCtx, HITLS_AUTH_PRIVPASS_CRYPTO_ALGID_SHA384, tokenInput, HITLS_AUTH_PRIVPASS_TOKEN_INPUT_LEN,
    ...
    HITLS_Auth_PrivPassToken *token)
{
    ...
    ret = ctx->method.verify(ctx->pubKeyCtx, HITLS_AUTH_PRIVPASS_CRYPTO_ALGID_SHA384, tokenInput, HITLS_AUTH_PRIVPASS_TOKEN_INPUT_LEN,
...
int32_t HITLS_Auth_PrivPassSetPubkey(HITLS_Auth_PrivPassCtx *ctx, uint8_t *pki, uint32_t pkiLen)
{
    ...
    void *pubKeyCtx = NULL;
    int32_t ret = ctx->method.decodePubKey(pki, pkiLen, &pubKeyCtx);
    if (ret != HITLS_AUTH_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ...
    ret = ctx->method.digest(HITLS_AUTH_PRIVPASS_CRYPTO_ALGID_SHA256, pki, pkiLen, ctx->tokenKeyId, &tokenKeyIdLen);
    if (ret != HITLS_AUTH_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        goto ERR;
    }
...
int32_t HITLS_Auth_PrivPassSetPrvkey(HITLS_Auth_PrivPassCtx *ctx, uint8_t *ski, uint32_t skiLen)
{
    ...
    void *prvKeyCtx = NULL;
    int32_t ret = ctx->method.decodePrvKey(ski, skiLen, &prvKeyCtx);
    if (ret != HITLS_AUTH_SUCCESS) {
        return ret;
    }
    if (ctx->pubKeyCtx != NULL) {
