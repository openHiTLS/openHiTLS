...
/**
 * @brief Write an unsigned integer value to a buffer in big-endian format
 * @param buffer The destination buffer to write to
 * @param value  The unsigned integer value to be written (up to 32 bits)
 * @param bytes  Number of bytes to write (1-4 bytes)
 */
void PrivPassWriteUint(uint8_t *buffer, uint32_t value, uint8_t bytes);

/**
 * @brief Read an unsigned integer value from a buffer in big-endian format
 * @param buffer The source buffer to read from
 * @param bytes  Number of bytes to read (1-4 bytes)
