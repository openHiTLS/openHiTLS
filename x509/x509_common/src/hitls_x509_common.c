...
static int32_t X509_SetRsaPssParam(CRYPT_EAL_PkeyCtx *privKey, int32_t mdId, const HITLS_X509_SignAlgParam *algParam,
    bool checkKeyParam, HITLS_X509_Asn1AlgId *signAlgId)
{
    ...
    if (checkKeyParam) {
        int32_t ret = X509_CheckPssParam(privKey, algParam->algId, &algParam->rsaPss);
        if (ret != HITLS_X509_SUCCESS) {
            return ret;
        }
    }
...
static int32_t X509_SetRsaPkcsParam(CRYPT_EAL_PkeyCtx *privKey, int32_t mdId, bool setPadding)
{
    if (setPadding) {
...
int32_t X509_SetRsaSignParam(CRYPT_EAL_PkeyCtx *privKey, int32_t mdId, const HITLS_X509_SignAlgParam *algParam,
    HITLS_X509_Asn1AlgId *signAlgId)
{
    ...
    switch (pad) {
        case CRYPT_PKEY_EMSA_PSS:
            if (algParam != NULL) {
                return X509_SetRsaPssParam(privKey, mdId, algParam, true, signAlgId);
            } else {
                return X509_SetRsaPssDefaultParam(privKey, mdId, signAlgId);
            }
        case CRYPT_PKEY_EMSA_PKCSV15:
            ret = X509_SetRsaPkcsParam(privKey, mdId, false);
            break;
        default:
            if (algParam != NULL) {
                return X509_SetRsaPssParam(privKey, mdId, algParam, false, signAlgId);
            } else {
                ret = X509_SetRsaPkcsParam(privKey, mdId, true);
            }
            break;
    }
    if (ret != HITLS_X509_SUCCESS) {
