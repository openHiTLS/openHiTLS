...
static int32_t X509_CopyAndParseAsn1Crl(bool isCopy, bool isMange, BSL_Buffer *encode, HITLS_X509_Crl *crl)
{
    ...
    int32_t ret = HITLS_X509_ParseAsn1Crl(isMange, &data, &dataLen, crl);
...
int32_t HITLS_X509_ParseBuffCrl(bool isCopy, int32_t format, BSL_Buffer *encode, HITLS_X509_Crl *crl)
{
    ...
    switch (format) {
        case BSL_PARSE_FORMAT_ASN1:
            ret = X509_CopyAndParseAsn1Crl(isCopy, isCopy, encode, crl);
            break;
        case BSL_PARSE_FORMAT_PEM:
            ret = HITLS_X509_ParsePem(encode, false, (HITLS_X509_Asn1Parse)X509_CopyAndParseAsn1Crl, crl);
            break;
        case BSL_PARSE_FORMAT_UNKNOWN:
            ret = HITLS_X509_ParseUnkonw(encode, isCopy, false, (HITLS_X509_Asn1Parse)X509_CopyAndParseAsn1Crl,
                crl);
            break;
        default:
