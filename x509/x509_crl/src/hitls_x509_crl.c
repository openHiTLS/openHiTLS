...
HITLS_X509_Crl *HITLS_X509_CrlNew()
{
    ...
    ext = HITLS_X509_ExtNew(&crl->tbs.crlExt, HITLS_X509_EXT_TYPE_CRL);
    if (ext == NULL) {
        goto ERR;
    }
...
static int32_t X509_EncodeCrlEntry(HITLS_X509_CrlEntry *crlEntry, BSL_ASN1_Buffer *asnBuf)
{
    ...
    if (crlEntry->extList != NULL) {
        return HITLS_X509_EncodeExt(BSL_ASN1_TAG_CONSTRUCTED | HITLS_CRL_CTX_SPECIFIC_TAG_EXTENSION,
            crlEntry->extList, &asnBuf[2]);
    } else {
        asnBuf[2].tag = 0;
        asnBuf[2].buff = NULL;
        asnBuf[2].len = 0;
...
static int32_t X509_CrlGetThisUpdate(HITLS_X509_Crl *crl, int32_t *val, int32_t valLen)
{
    ...
    *(BSL_TIME *)val = crl->tbs.validTime.start;
    return HITLS_X509_SUCCESS;
}

static int32_t X509_CrlGetNextUpdate(HITLS_X509_Crl *crl, int32_t *val, int32_t valLen)
{
    if (val == NULL || valLen != sizeof(BSL_TIME)) {
        BSL_ERR_PUSH_ERROR(HITLS_X509_ERR_INVALID_PARAM);
        return HITLS_X509_ERR_INVALID_PARAM;
...
static HITLS_X509_CrlEntry *X509_CrlEntryDup(const HITLS_X509_CrlEntry *src)
{
    ...
    (void)memcpy_s(dest->serialNumber.buff, src->serialNumber.len, 
                   src->serialNumber.buff, src->serialNumber.len);
...
{
    if (crl == NULL || val == NULL || valLen != sizeof(HITLS_X509_CrlEntry)) {
        BSL_ERR_PUSH_ERROR(HITLS_X509_ERR_INVALID_PARAM);
        return HITLS_X509_ERR_INVALID_PARAM;
    }
