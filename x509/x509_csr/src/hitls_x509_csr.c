...
static int32_t ParseCertRequestInfo(BSL_ASN1_Buffer *asnArr, HITLS_X509_Csr *csr)
{
    ...
    /* attributes */
    ret = HITLS_X509_ParseAttrList(&asnArr[HITLS_X509_CSR_REQINFO_ATTRS_IDX], csr->reqInfo.attributes, NULL);
    if (ret != BSL_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        goto ERR;
    }
...
static int32_t CheckCsrValid(HITLS_X509_Csr *csr)
{
    ...
    if (csr->reqInfo.attributes == NULL || csr->reqInfo.attributes->list == NULL) {
