...
static int32_t X509EncodeAsn1CsrCore(HITLS_X509_Csr *csr)
{
    ...
    BSL_ASN1_Buffer reqInfoAsn1 = {BSL_ASN1_TAG_CONSTRUCTED | BSL_ASN1_TAG_SEQUENCE,
        csr->reqInfo.reqInfoRawDataLen, csr->reqInfo.reqInfoRawData};
    BSL_ASN1_Buffer signAlgAsn1 = {BSL_ASN1_TAG_CONSTRUCTED | BSL_ASN1_TAG_SEQUENCE, 0, NULL};
    uint32_t valLen = 0;
    int32_t ret = BSL_ASN1_DecodeTagLen(reqInfoAsn1.tag, &reqInfoAsn1.buff, &reqInfoAsn1.len, &valLen);
    if (ret != HITLS_X509_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }

    ret = HITLS_X509_EncodeSignAlgInfo(&csr->signAlgId, &signAlgAsn1);
    if (ret != HITLS_X509_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ...
    ret = BSL_ASN1_EncodeTemplate(&csrTempl, csrAsn1Buff, HITLS_X509_CSR_BRIEF_SIZE, &csr->rawData, &csr->rawDataLen);
        ...
        if (csr->state == HITLS_X509_CSR_STATE_SIGN) {
            ...
            if (ret != HITLS_X509_SUCCESS) {
                BSL_LOG_BINLOG_FIXLEN(BINLOG_ID05065, BSL_LOG_LEVEL_ERR, BSL_LOG_BINLOG_TYPE_RUN,
                    "csr: failed to encode the csr.", 0, 0, 0, 0);
                BSL_ERR_PUSH_ERROR(ret);
                return ret;
            }
...
int32_t CsrSignCb(uint32_t mdId, CRYPT_EAL_PkeyCtx *pivKey, HITLS_X509_Asn1AlgId *signAlgId,
    HITLS_X509_Csr *csr)
{
    ...
    int32_t ret = CRYPT_EAL_PkeyPairCheck((CRYPT_EAL_PkeyCtx *)csr->reqInfo.ealPubKey, pivKey);
    if (ret != CRYPT_SUCCESS) {
    ...
    if (ret != HITLS_X509_SUCCESS) {
        BSL_LOG_BINLOG_FIXLEN(BINLOG_ID05068, BSL_LOG_LEVEL_ERR, BSL_LOG_BINLOG_TYPE_RUN,
            "csr: failed to encode csr req info.", 0, 0, 0, 0);
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ret = HITLS_X509_SignAsn1Data(pivKey, mdId, &reqInfoAsn1, &signBuff, &csr->signature);
    ...
    HITLS_X509_Csr *csr)
{
    ...
    if (ret != HITLS_X509_SUCCESS) {
        BSL_LOG_BINLOG_FIXLEN(BINLOG_ID05066, BSL_LOG_LEVEL_ERR, BSL_LOG_BINLOG_TYPE_RUN,
            "csr: encode csr failed due to invalid reqinfo.", 0, 0, 0, 0);
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ...
    BSL_SAL_FREE(csr->rawData);
    csr->rawDataLen = 0;
    return HITLS_X509_Sign(mdId, pivKey, algParam, csr, (HITLS_X509_SignCb)CsrSignCb);
}
