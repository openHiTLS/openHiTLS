...
{
    int32_t ret = HITLS_X509_ParseAttrList(attrBuff, attributes, X509_ParseP12AttrItem);
    if (ret != BSL_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
    }
    return ret;
...
static HITLS_PKCS12_Bag *BagDump(HITLS_PKCS12_Bag *input)
{
    ...
    if (BSL_LIST_COUNT(input->attributes->list) > 0) {
        BSL_ASN1_List *dumpList = BSL_LIST_Copy(input->attributes->list, AttrCopy, HITLS_PKCS12_AttributesFree);
        if (dumpList == NULL) {
            HITLS_PKCS12_BagFree(target);
            BSL_ERR_PUSH_ERROR(BSL_MALLOC_FAIL);
            return NULL;
        }
        target->attributes->list = dumpList;
    }
    return target;
}
