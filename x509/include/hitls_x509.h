...
typedef enum {
    HITLS_X509_ATTR_SET_REQUESTED_EXTENSIONS = 0x1,
...
/**
 * While the standard imposes no constraints on password length, the combined length of password (pwdLen) and
 * salt (saltLen) should be kept below 2^31 to avoid integer overflow in internal calculations.
*/
typedef struct {
    uint32_t saltLen;
    uint32_t itCnt;
...
typedef enum{
    HITLS_PKCS12_GEN_LOCALKEYID = 0x1,          /** Gen and set localKeyId of entity-key and entity-cert in p12-ctx. */
