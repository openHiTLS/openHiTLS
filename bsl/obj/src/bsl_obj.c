...
{
    if (inputCid >= BSL_CID_MAX || inputCid == BSL_CID_UNKNOWN) {
...
{
    if (oid == NULL || oidName == NULL || cid >= BSL_CID_MAX || cid == BSL_CID_UNKNOWN) {
        BSL_ERR_PUSH_ERROR(BSL_INVALID_ARG);
    ...
    if (g_oidHashTable == NULL) {
        g_oidHashTable = BSL_HASH_Create(BSL_OBJ_HASH_BKT_SIZE, NULL, NULL, NULL, NULL);
        if (g_oidHashTable == NULL) {
            BSL_ERR_PUSH_ERROR(BSL_MALLOC_FAIL);
            return BSL_MALLOC_FAIL;
        }
