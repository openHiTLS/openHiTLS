...
int32_t BSL_SAL_GetSockName(int32_t sockId, BSL_SAL_SockAddr addr, size_t *len)
{
    if (addr == NULL || len == NULL) {
        return BSL_NULL_INPUT;
    }
    if (SAL_GetNetCallBack().pfGetSocketName != NULL && SAL_GetNetCallBack().pfGetSocketName != BSL_SAL_GetSockName) {
        return SAL_GetNetCallBack().pfGetSocketName(sockId, addr, len);
    }

#ifdef HITLS_BSL_SAL_LINUX
    return SAL_NET_GetSockName(sockId, addr, len);
#else
    BSL_ERR_PUSH_ERROR(BSL_SAL_NET_NO_REG_FUNC);
    return BSL_SAL_NET_NO_REG_FUNC;
#endif
...
int32_t BSL_SAL_SockAccept(int32_t sockId, BSL_SAL_SockAddr addr, size_t *len)
{
    if (addr == NULL || len == 0) {
        return BSL_NULL_INPUT;
    }
    if (SAL_GetNetCallBack().pfSockAccept != NULL && SAL_GetNetCallBack().pfSockAccept != BSL_SAL_SockAccept) {
        return SAL_GetNetCallBack().pfSockAccept(sockId, addr, len);
    }

#ifdef HITLS_BSL_SAL_LINUX
    return SAL_NET_SockAccept(sockId, addr, len);
#else
    BSL_ERR_PUSH_ERROR(BSL_SAL_NET_NO_REG_FUNC);
    return BSL_SAL_NET_NO_REG_FUNC;
#endif
