    ...
    HITLS_ParseType type, HITLS_ParseFormat format)
{
    ...
    switch (type) {
        case TLS_PARSE_TYPE_FILE:
            ret = CRYPT_EAL_DecodeFileKey_Ex(libCtx, attrName, format, CRYPT_ENCODE_UNKNOW,
                (const char *)buf, &pwdBuff, (CRYPT_EAL_PkeyCtx **)&ealPriKey);
            break;
        case TLS_PARSE_TYPE_BUFF:
            encode.data = (uint8_t *)(uintptr_t)buf;
            encode.dataLen = len;
            ret = CRYPT_EAL_DecodeBuffKey_Ex(libCtx, attrName, format, CRYPT_ENCODE_UNKNOW, &encode, &pwdBuff,
