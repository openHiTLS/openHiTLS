...
CERT_MgrCtx *SAL_CERT_MgrCtxNew(void)
{
    ...
    newCtx->certHash = BSL_HASH_Create(CERT_DEFAULT_HASH_BKT_SZIE, NULL, NULL, NULL, NULL);
    if (newCtx->certHash == NULL) {
        BSL_SAL_FREE(newCtx);
        BSL_LOG_BINLOG_FIXLEN(BINLOG_ID17338, BSL_LOG_LEVEL_ERR, BSL_LOG_BINLOG_TYPE_RUN,
            "new cert manager context error: new certHash failed.", 0, 0, 0, 0);
        return NULL;
    }
    ...
    if (newCtx->certStore == NULL) {
        BSL_HASH_Destory(newCtx->certHash);
        BSL_SAL_FREE(newCtx);
...
void SAL_CERT_MgrCtxFree(CERT_MgrCtx *mgrCtx)
{
    ...
    BSL_HASH_Destory(mgrCtx->certHash);
    mgrCtx->certHash = NULL;
    BSL_SAL_FREE(mgrCtx);
    return;
}