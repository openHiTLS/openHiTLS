...
static int32_t GetCertPair(CERT_MgrCtx *mgrCtx, HITLS_CERT_KeyType keyType, CERT_Pair **certPair)
{
    CERT_Pair *newCertPair = SAL_CERT_HashFind(mgrCtx, keyType);
    if (newCertPair == NULL) {
        ...
        int32_t ret = SAL_CERT_HashInsert(mgrCtx, keyType, newCertPair);
...
int32_t SAL_CERT_SetCurrentCert(HITLS_Config *config, HITLS_CERT_X509 *cert, bool isTlcpEncCert)
{
    ...
    ret = GetCertPair(mgrCtx, keyType, &certPair);
...
HITLS_CERT_Chain *SAL_CERT_GetCurrentChainCerts(CERT_MgrCtx *mgrCtx)
{
    ...
    CERT_Pair *certPair = SAL_CERT_HashFind(mgrCtx, mgrCtx->currentCertKeyType);
    if (certPair == NULL) {
