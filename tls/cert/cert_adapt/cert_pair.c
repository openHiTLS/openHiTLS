...
{
    if (mgrCtx == NULL) {
        BSL_LOG_BINLOG_FIXLEN(BINLOG_ID17343, BSL_LOG_LEVEL_ERR, BSL_LOG_BINLOG_TYPE_RUN, "mgrCtx null", 0, 0, 0, 0);
        return NULL;
    }
    CERT_Pair *certPair = NULL;
    if (BSL_HASH_At(mgrCtx->certPairs, (uintptr_t)keyType, (uintptr_t *)&certPair) != BSL_SUCCESS) {
        BSL_LOG_BINLOG_FIXLEN(
            BINLOG_ID17344, BSL_LOG_LEVEL_DEBUG, BSL_LOG_BINLOG_TYPE_RUN, "not find certPair", 0, 0, 0, 0);
        return NULL;
    }
    return certPair;
}
...
{
    if (mgrCtx == NULL || certPair == NULL) {
        BSL_LOG_BINLOG_FIXLEN(BINLOG_ID17345, BSL_LOG_LEVEL_ERR, BSL_LOG_BINLOG_TYPE_RUN,
            "mgrCtx or certPair is null", 0, 0, 0, 0);
        return HITLS_NULL_INPUT;
    }
    int32_t ret = BSL_HASH_Insert(mgrCtx->certPairs, keyType, 0, (uintptr_t)certPair, sizeof(CERT_Pair));
    if (ret != HITLS_SUCCESS) {
        BSL_LOG_BINLOG_FIXLEN(BINLOG_ID17346, BSL_LOG_LEVEL_ERR, BSL_LOG_BINLOG_TYPE_RUN, "insert failed", 0, 0, 0, 0);
        return HITLS_CERT_ERR_INSERT_CERTPAIR;
    }
    return HITLS_SUCCESS;
}
...
int32_t SAL_CERT_HashDup(CERT_MgrCtx *destMgrCtx, CERT_MgrCtx *srcMgrCtx)
{
    ...
    while (iter != BSL_HASH_IterEnd(certPairs)) {
        ...
        if (certPair != NULL) {
            ...
            int32_t ret = SAL_CERT_HashInsert(destMgrCtx, keyType, newCertPair);
            if (ret != HITLS_SUCCESS) {
