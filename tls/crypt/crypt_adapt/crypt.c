...
#ifdef HITLS_TLS_FEATURE_PROVIDER
    return HITLS_CRYPT_GenerateDhKeyBySecbits(libCtx, attrName, groupInfo->paraId);
#else
    return g_cryptDhMethod.generateDhKeyBySecbits(groupInfo->secBits);
#endif
...
{
#ifdef HITLS_TLS_FEATURE_PROVIDER
    int32_t ret = HITLS_CRYPT_HkdfExpand(libCtx, attrName, input, prk, prkLen);
#else
