...
#ifdef HITLS_TLS_FEATURE_PROVIDER
    *ctx = CRYPT_EAL_ProviderCipherNewCtx(libCtx, cipher->algo, attrName);
#else
    (void)libCtx;
    (void)attrName;
    *ctx = CRYPT_EAL_CipherNewCtx((CRYPT_CIPHER_AlgId)cipher->algo);
#endif
...
HITLS_CRYPT_Key *CRYPT_DEFAULT_GenerateEcdhKey(const HITLS_ECParameters *curveParams)
{
    switch (curveParams->type) {
        case HITLS_EC_CURVE_TYPE_NAMED_CURVE:
            return GenerateKeyByNamedGroup(NULL, NULL, NULL, curveParams->param.namedcurve);
        default:
            break;
    }
    return NULL;
