...
static int32_t ServerCheckEncryptThenMac(TLS_Ctx *ctx, const ClientHelloMsg *clientHello)
{
    ...
    bool newEncryptThenMac = ctx->config.tlsConfig.isEncryptThenMac &&
        !IS_TLS13_FAMILY_CTX(ctx) && haveEncryptThenMac;
#ifdef HITLS_TLS_FEATURE_RENEGOTIATION
...
static int32_t ProcessClientHelloExt(TLS_Ctx *ctx, const ClientHelloMsg *clientHello, bool isNeedSendHrr)
{
    ...
    ret = DTLS_CID_ProcessClientHello(ctx, clientHello->extension.flag.haveConnectionId,
        clientHello->extension.content.connectionId, clientHello->extension.content.connectionIdLen, isNeedSendHrr);
    if (ret != HITLS_SUCCESS) {
        return ret;
    }
...
static int32_t ServerCheckKeyShare(TLS_Ctx *ctx, const ClientHelloMsg *clientHello)
{
    ...
    if (ctx->hsCtx->haveHrr && ctx->hsCtx->isHrrKeyShare) {
...
static bool IsPSKValid(TLS_Ctx *ctx, HITLS_Session *pskSession)
{
    ...
    uint16_t expectedVersion = ctx->negotiatedInfo.version == HITLS_VERSION_DTLS13 ?
        HITLS_VERSION_DTLS13 : HITLS_VERSION_TLS13;
    ...
    uint32_t truncateHelloLen)
{
    ...
    /* 1. Select the external or resumption binder label from the PSK lookup result. */
    bool isExternalPsk = !(ctx->negotiatedInfo.isResume);
    uint8_t computedBinder[HS_MAX_BINDER_SIZE] = {0};

    /* 2. Require the received binder length to match the negotiated cipher hash. */
    ...
    /* 3. Calculate the binder over the truncated ClientHello; the verifier includes the HRR transcript. */
...
static int32_t ServerSelectPskAndCheckBinder(TLS_Ctx *ctx, const ClientHelloMsg *clientHello, bool certWithExternalPsk)
{
    ...
    LIST_FOR_EACH_ITEM_SAFE(node, tmpNode, &(offeredPsks->pskNode))
    {
        ...
        /* 2. Use one lookup path and the already-selected cipher hash for both PSK types. */
        ret = ServerFindPsk(ctx, cur, psk, &pskLen, clientHello->extension.flag.haveCertWithExternalPsk);
        if (ret != HITLS_SUCCESS) {
        ...
        /* 3. Store the selected PSK and verify its binder through the shared path. */
        ...
        if (ret != HITLS_SUCCESS) {
            ...
            /* 4. RFC 9973 uses illegal_parameter; other PSK handshakes use decrypt_error. */
            ctx->method.sendAlert(ctx, ALERT_LEVEL_FATAL,
                                  certWithExternalPsk ? ALERT_ILLEGAL_PARAMETER : ALERT_DECRYPT_ERROR);
            return ret;
        }
        cur->isValid = true;
        break;
...
#ifdef HITLS_TLS_FEATURE_CERT_WITH_EXTERNAL_PSK
/* Validate RFC 9973 requirements after the shared extension dependency checks. */
static int32_t Tls13ServerCheckCertWithExternalPsk(TLS_Ctx *ctx, const ClientHelloMsg *clientHello)
{
    /* 1. Apply these checks only when the client sends extension 33. */
    if (!clientHello->extension.flag.haveCertWithExternalPsk) {
        return HITLS_SUCCESS;
    }

    /* 2. Require key_share and pre_shared_key (RFC 9973 Section 4).
...
#ifdef HITLS_TLS_FEATURE_CERT_WITH_EXTERNAL_PSK
        /* 1. Reuse the shared dependency checks above, then apply the extension 33 requirements.
         * 2. The helper sends the fatal alert itself; return its error without sending another alert.
         */
        int32_t ret = Tls13ServerCheckCertWithExternalPsk(ctx, clientHello);
        if (ret != HITLS_SUCCESS) {
            return ret;
        }
#endif
...
#ifdef HITLS_TLS_FEATURE_CERT_WITH_EXTERNAL_PSK
        /* 1. Compare extension 33 presence in both ClientHellos.
...
#endif
#ifdef HITLS_TLS_FEATURE_DTLS_CID
        const ClientHelloMsg *firstClientHello = ctx->hsCtx->firstClientHello;
        if (firstClientHello->extension.flag.haveConnectionId != clientHello->extension.flag.haveConnectionId ||
            !DTLS_CID_IsEq(firstClientHello->extension.content.connectionId,
                           firstClientHello->extension.content.connectionIdLen,
                           clientHello->extension.content.connectionId,
                           clientHello->extension.content.connectionIdLen)) {
            BSL_LOG_BINLOG_FIXLEN(BINLOG_ID17062, BSL_LOG_LEVEL_ERR, BSL_LOG_BINLOG_TYPE_RUN,
                "ClientHello CID changed after HRR.", 0, 0, 0, 0);
            ctx->method.sendAlert(ctx, ALERT_LEVEL_FATAL, ALERT_ILLEGAL_PARAMETER);
            return HITLS_MSG_HANDLE_HANDSHAKE_FAILURE;
        }
#endif /* HITLS_TLS_FEATURE_DTLS_CID */
...
static int32_t Dtls13ServerCheckLegacyCookie(TLS_Ctx *ctx, const ClientHelloMsg *clientHello)
{
    if (clientHello->version != HITLS_VERSION_DTLS13 || clientHello->cookieLen == 0u) {
        return HITLS_SUCCESS;
    }

    BSL_ERR_PUSH_ERROR(HITLS_MSG_HANDLE_COOKIE_ERR);
    BSL_LOG_BINLOG_FIXLEN(BINLOG_ID17384, BSL_LOG_LEVEL_ERR, BSL_LOG_BINLOG_TYPE_RUN,
        "(d)tls1.3 client hello legacy cookie is not empty.", 0, 0, 0, 0);
    ctx->method.sendAlert(ctx, ALERT_LEVEL_FATAL, ALERT_ILLEGAL_PARAMETER);
    return HITLS_MSG_HANDLE_COOKIE_ERR;
}

#if defined(HITLS_TLS_PROTO_DTLS13) && defined(HITLS_BSL_UIO_UDP)
static bool Dtls13ServerNeedCookieExchange(const TLS_Ctx *ctx)
{
...
static int32_t Tls13ServerBasicCheckClientHello(TLS_Ctx *ctx, ClientHelloMsg *clientHello)
{
    ...
    ctx->negotiatedInfo.version = clientHello->version;

    ret = Dtls13ServerCheckLegacyCookie(ctx, clientHello);
    if (ret != HITLS_SUCCESS) {
        return ret;
    }
...
static int32_t Tls13ServerCheckClientHello(TLS_Ctx *ctx, ClientHelloMsg *clientHello, bool *isNeedSendHrr)
{
    ...
    if (Dtls13ServerNeedCookieExchange(ctx) && !clientHello->extension.flag.haveCookie) {
        ...
        if (ret != HITLS_SUCCESS) {
            BSL_LOG_BINLOG_FIXLEN(BINLOG_ID15241, BSL_LOG_LEVEL_ERR, BSL_LOG_BINLOG_TYPE_RUN,
                "calc dtls13 cookie fail when process client hello.", 0, 0, 0, 0);
            ctx->method.sendAlert(ctx, ALERT_LEVEL_FATAL, ALERT_INTERNAL_ERROR);
            return ret;
        }
        *isNeedSendHrr = true;
...
static int32_t CheckSupportVersion(TLS_Ctx *ctx, uint16_t version, uint16_t *selectVersion)
{
    ...
    if (version <= HITLS_VERSION_DTLS13 && version != HITLS_VERSION_TLCP_DTLCP11 && IS_SUPPORT_DATAGRAM(ctx->config.tlsConfig.originVersionMask)) {
        ...
        for (int i = 0; i < clientHello->extension.content.supportedVersionsCount; i++) {
            ...
            if (IS_TLS13_FAMILY_VERSION(version) && !IsTls13KeyExchAvailable(ctx)) {
                ...
                BSL_LOG_BINLOG_FIXLEN(BINLOG_ID17380, BSL_LOG_LEVEL_ERR, BSL_LOG_BINLOG_TYPE_RUN,
                    "(d)tls1.3 handshake state error: must have an available PSK or certificate", 0, 0, 0, 0);
                ctx->method.sendAlert(ctx, ALERT_LEVEL_FATAL, ALERT_HANDSHAKE_FAILURE);
                return HITLS_MSG_HANDLE_UNSUPPORT_VERSION;
            }
...
int32_t Tls13ServerRecvClientHelloProcess(TLS_Ctx *ctx, HS_Msg *msg)
{
    ...
    /* Perform the ClientHello callback. The pause handshake status is not considered */
        ...
        if (ctx->hsCtx->haveHrr) {
            if (selectedVersion != ctx->negotiatedInfo.version) {
                BSL_ERR_PUSH_ERROR(HITLS_MSG_HANDLE_UNSUPPORT_VERSION);
                ctx->method.sendAlert(ctx, ALERT_LEVEL_FATAL, ALERT_ILLEGAL_PARAMETER);
                return HITLS_MSG_HANDLE_UNSUPPORT_VERSION;
            }
    ...
    switch (clientHello->version) {
        ...
        case HITLS_VERSION_DTLS12:
            BSL_LOG_BINLOG_FIXLEN(BINLOG_ID15251, BSL_LOG_LEVEL_INFO, BSL_LOG_BINLOG_TYPE_RUN,
                "(d)tls1.3 server receive a 0x%x clientHello.", clientHello->version, 0, 0, 0);
            return ServerRecvClientHelloProcess(ctx, msg, false);
#endif /* HITLS_TLS_PROTO_TLS_BASIC */
        case HITLS_VERSION_TLS13:
        case HITLS_VERSION_DTLS13:
            ret = Tls13ServerProcessClientHello(ctx, msg);
            return ret;
        default:
            break;
    }
