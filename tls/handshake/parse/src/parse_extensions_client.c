...
static int32_t ParseServerCertWithExternalPsk(ParsePacket *pkt, ServerHelloMsg *msg)
{
    /* 1. Reject a second copy of extension 33. */
    if (msg->haveCertWithExternalPsk) {
        return ParseDupExtProcess(pkt->ctx, BINLOG_ID15156, BINGLOG_STR("tls_cert_with_extern_psk"));
    }
    /* 2. Require an empty body; extension 33 carries no payload. */
    if (pkt->bufLen != 0) {
        return ParseErrorProcess(pkt->ctx, HITLS_PARSE_INVALID_MSG_LEN, BINLOG_ID15157,
    ...
    msg->haveCertWithExternalPsk = true;
    return HITLS_SUCCESS;
}
#endif
#ifdef HITLS_TLS_FEATURE_DTLS_CID
static int32_t ParseServerConnectionId(ParsePacket *pkt, ServerHelloMsg *msg)
{
    uint8_t cidLen = 0;
    int32_t ret = ParseOneByteLengthField(pkt, &cidLen, &msg->connectionId);
    if (ret == HITLS_PARSE_INVALID_MSG_LEN) {
        return ParseErrorExtLengthProcess(pkt->ctx, BINLOG_ID15157, BINGLOG_STR("connection_id"));
    } else if (ret == HITLS_MEMALLOC_FAIL) {
    ...
    msg->connectionIdLen = cidLen;
    msg->haveConnectionId = true;
    return HITLS_SUCCESS;
}
...
int32_t ParseServerCookie(ParsePacket *pkt, ServerHelloMsg *msg)
{
    ...
    if (ret != HITLS_SUCCESS) {
        if (ret == HITLS_PARSE_INVALID_MSG_LEN) {
            return ParseErrorExtLengthProcess(pkt->ctx, BINLOG_ID15163, BINGLOG_STR("cookie"));
        }
        return ret;
    }
    msg->haveCookie = true;
    ...
    ServerHelloMsg *msg)
{
    ...
    switch (extMsgType) {
...
#ifdef HITLS_TLS_FEATURE_CERT_WITH_EXTERNAL_PSK
        /* 1. Route the extension body to the duplicate and length checks. */
        case HS_EX_TYPE_CERT_WITH_EXTERNAL_PSK:
            return ParseServerCertWithExternalPsk(&pkt, msg);
#endif
        ...
        case HS_EX_TYPE_CONNECTION_ID:
            return ParseServerConnectionId(&pkt, msg);
#endif /* HITLS_TLS_FEATURE_DTLS_CID */
#endif /* HITLS_TLS_PROTO_TLS13_FAMILY */
