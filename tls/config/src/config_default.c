...
int32_t DefaultConfig(CRYPT_EAL_LibCtx *libCtx, const char *attribute, uint16_t version, HITLS_Config *config)
{
    // Static settings
    ...
    if (SAL_CERT_MgrIsEnable()) {
        config->certMgrCtx = SAL_CERT_MgrCtxNewWithProvider(libCtx, attribute);
