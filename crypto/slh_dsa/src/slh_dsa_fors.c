...
int32_t ForsPkFromSig(const uint8_t *sig, uint32_t sigLen, const uint8_t *md, uint32_t mdLen, SlhDsaAdrs *adrs,
                      const SlhDsaCtx *ctx, uint8_t *pk)
{
    int32_t ret;
    ...
    for (uint32_t i = 0; i < k; i++) {
        ...
        // It's a "F" function
        ret = ctx->pthf(ctx, false, ctx->prvkey.pub.seed, adrs, sig + (a + 1) * n * i, n, node0);
        if (ret != 0) {
            goto ERR;
        }
        ...
        for (uint32_t j = 0; j < a; j++) {
            ...
            // It's a "H" function
            ret = ctx->pthf(ctx, true, ctx->prvkey.pub.seed, adrs, tmp, 2 * n, node1);
            if (ret != 0) {
                goto ERR;
            }
    ...
    // It's a "Tl" function
    ret = ctx->pthf(ctx, true, ctx->prvkey.pub.seed, &forspkAdrs, root, n * k, pk);
    if (ret != 0) {
        goto ERR;
    }
...
int32_t ForsGenPrvKey(const SlhDsaAdrs *adrs, uint32_t idx, const SlhDsaCtx *ctx, uint8_t *sk)
{
    ...
    // It's a "PRF" function
    return ctx->pthf(ctx, false, ctx->prvkey.pub.seed, &skadrs, ctx->prvkey.seed, n, sk);
}
...
int32_t ForsNode(uint32_t idx, uint32_t height, SlhDsaAdrs *adrs, const SlhDsaCtx *ctx, uint8_t *node)
{
    ...
    if (height == 0) {
        ...
        // It's a "F" function
        return ctx->pthf(ctx, false, ctx->prvkey.pub.seed, adrs, sk, n, node);
    }
    ...
    ctx->adrsOps.setTreeIndex(adrs, idx);
    // It's a "H" function
    return ctx->pthf(ctx, true, ctx->prvkey.pub.seed, adrs, dnode, 2 * n, node);
}
