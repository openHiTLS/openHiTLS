...
typedef int32_t (*SlhDsaTl)(const CryptSlhDsaCtx *ctx, const SlhDsaAdrs *adrs,
                        const uint8_t *msg, uint32_t msgLen, uint8_t *out);

typedef int32_t (*SlhDsaH)(const CryptSlhDsaCtx *ctx, const SlhDsaAdrs *adrs,
                        const uint8_t *msg, uint32_t msgLen, uint8_t *out);


typedef int32_t (*SlhDsaF)(const CryptSlhDsaCtx *ctx, const SlhDsaAdrs *adrs,
                        const uint8_t *msg, uint32_t msgLen, uint8_t *out);

// The length of "prf", "rand" and "out" is n, the max length is SLH_DSA_MAX_N
typedef int32_t (*SlhDsaPrfMsg)(const CryptSlhDsaCtx *ctx, const uint8_t *rand, const uint8_t *msg,
...
typedef int32_t (*SlhDsaHmsg)(const CryptSlhDsaCtx *ctx, const uint8_t *r,
                        const uint8_t *msg, uint32_t msgLen, uint8_t *out);
struct HashFuncs{
    SlhDsaPrf prf;
    SlhDsaTl tl;
