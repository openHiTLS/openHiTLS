...
int32_t XmssNode(uint8_t *node, uint32_t idx, uint32_t height, SlhDsaAdrs *adrs, const SlhDsaCtx *ctx)
{
    int32_t ret;
    ...
    // It's a "H" function
    return ctx->pthf(ctx, true, ctx->prvkey.pub.seed, adrs, tmp, 2 * n, node);
}
...
int32_t XmssPkFromSig(uint32_t idx, const uint8_t *sig, uint32_t sigLen, const uint8_t *msg, uint32_t msgLen,
                      SlhDsaAdrs *adrs, const SlhDsaCtx *ctx, uint8_t *pk)
{
    int32_t ret;
