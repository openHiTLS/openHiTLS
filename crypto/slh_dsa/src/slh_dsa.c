...
int32_t CRYPT_SLH_DSA_Gen(CryptSlhDsaCtx *ctx)
{
    ...
    (void)memcpy(ctx->prvkey.pub.root, node, n);
    return CRYPT_SUCCESS;
}

static int32_t GetAddRand(SlhDsaCtx *ctx)
{
    if (ctx->addrand != NULL) {
...
static void GetTreeAndLeafIdx(const uint8_t *digest, const SlhDsaCtx *ctx, uint64_t *treeIdx, uint32_t *leafIdx)
{
    ...
    uint32_t mdIdx = SPLIT_CEIL(k * a, 8);
    uint32_t treeIdxLen = SPLIT_CEIL(h - h / d, 8);
    uint32_t leafIdxLen = SPLIT_CEIL(h / d, 8);
                                          ...
                                          uint32_t *sigLen)
{
    ...
    ret = ctx->prfmsg(ctx, ctx->prvkey.prf, ctx->addrand, msg, msgLen, sig);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ...
    ret = ctx->hmsg(ctx, sig, ctx->prvkey.pub.seed, ctx->prvkey.pub.root, msg, msgLen, digest);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
...
static int32_t Prfmsg(const SlhDsaCtx *ctx, const uint8_t *prf, const uint8_t *rand, const uint8_t *msg,
                      uint32_t msgLen, uint8_t *out, CRYPT_MAC_AlgId macId)
{
    int32_t ret;
    uint32_t n = ctx->para.n;
    ...
    GOTO_ERR_IF_EX(CRYPT_EAL_MacInit(mdCtx, prf, n), ret);
...
static int32_t PrfmsgShake256(const SlhDsaCtx *ctx, const uint8_t *prf, const uint8_t *rand, const uint8_t *msg,
                              uint32_t msgLen, uint8_t *out)
{
    ...
    GOTO_ERR_IF_EX(CRYPT_EAL_MdUpdate(mdCtx, prf, n), ret);
    GOTO_ERR_IF_EX(CRYPT_EAL_MdUpdate(mdCtx, rand, n), ret);
...
static int32_t PrfmsgSha256(const SlhDsaCtx *ctx, const uint8_t *prf, const uint8_t *rand, const uint8_t *msg,
                            uint32_t msgLen, uint8_t *out)
{
    return Prfmsg(ctx, prf, rand, msg, msgLen, out, CRYPT_MAC_HMAC_SHA256);
}

static int32_t PrfmsgSha512(const SlhDsaCtx *ctx, const uint8_t *prf, const uint8_t *rand, const uint8_t *msg,
                            uint32_t msgLen, uint8_t *out)
{
    return Prfmsg(ctx, prf, rand, msg, msgLen, out, CRYPT_MAC_HMAC_SHA512);
}

static int32_t HmsgShake256(const SlhDsaCtx *ctx, const uint8_t *r, const uint8_t *seed, const uint8_t *root,
                            const uint8_t *msg, uint32_t msgLen, uint8_t *out)
{
    ...
    GOTO_ERR_IF_EX(CRYPT_EAL_MdInit(mdCtx), ret);
    GOTO_ERR_IF_EX(CRYPT_EAL_MdUpdate(mdCtx, r, n), ret);
    GOTO_ERR_IF_EX(CRYPT_EAL_MdUpdate(mdCtx, seed, n), ret);
    GOTO_ERR_IF_EX(CRYPT_EAL_MdUpdate(mdCtx, root, n), ret);
    GOTO_ERR_IF_EX(CRYPT_EAL_MdUpdate(mdCtx, msg, msgLen), ret);
    GOTO_ERR_IF_EX(CRYPT_EAL_MdFinal(mdCtx, tmp, &tmpLen), ret);
    (void)memcpy_s(out, m, tmp, m);
ERR:
    CRYPT_EAL_MdFreeCtx(mdCtx);
    return ret;
}

static int32_t PthfShake256(const SlhDsaCtx *ctx, bool isHT, const uint8_t *seed, const SlhDsaAdrs *adrs,
                            const uint8_t *msg, uint32_t msgLen, uint8_t *out)
{
    (void)isHT;
    int32_t ret;
...
static int32_t HmsgSha(const SlhDsaCtx *ctx, const uint8_t *r, const uint8_t *seed, const uint8_t *root,
...
static int32_t HmsgSha256(const SlhDsaCtx *ctx, const uint8_t *r, const uint8_t *seed, const uint8_t *root,
                          const uint8_t *msg, uint32_t msgLen, uint8_t *out)
{
    return HmsgSha(ctx, r, seed, root, msg, msgLen, out, CRYPT_MD_SHA256);
}

static int32_t HmsgSha512(const SlhDsaCtx *ctx, const uint8_t *r, const uint8_t *seed, const uint8_t *root,
                          const uint8_t *msg, uint32_t msgLen, uint8_t *out)
{
    return HmsgSha(ctx, r, seed, root, msg, msgLen, out, CRYPT_MD_SHA512);
}

static int32_t PthfSha2(const SlhDsaCtx *ctx, bool isHT, const uint8_t *seed, const SlhDsaAdrs *adrs,
                        const uint8_t *msg, uint32_t msgLen, uint8_t *out)
{
    int32_t ret;
    uint32_t n = ctx->para.n;
