...
int32_t WotsChain(const uint8_t *x, uint32_t xLen, uint32_t start, uint32_t end, const uint8_t *seed, SlhDsaAdrs *adrs,
                  const SlhDsaCtx *ctx, uint8_t *output)
{
    int32_t ret;
    ...
    for (uint32_t i = start; i < start + end; i++) {
        ...
        // It's a "F" function
        ret = ctx->pthf(ctx, false, seed, adrs, tmp, tmpLen, tmp);
        if (ret != 0) {
            return ret;
        }
    }
...
int WotsGeneratePublicKey(uint8_t *pub, SlhDsaAdrs *adrs, const SlhDsaCtx *ctx)
{
    int32_t ret;
    ...
    for (uint32_t i = 0; i < len; i++) {
        ...
        // It's a "PRF" function
        ret = ctx->pthf(ctx, false, ctx->prvkey.pub.seed, &skAdrs, ctx->prvkey.seed, n, sk);
        if (ret != 0) {
            goto ERR;
        }
    ...
    ret = ctx->pthf(ctx, true, ctx->prvkey.pub.seed, &wotspk, tmp, len * n, pub);
...
int WotsPubKeyFromSig(const uint8_t *msg, uint32_t msgLen, const uint8_t *sig, uint32_t sigLen, SlhDsaAdrs *adrs,
                      const SlhDsaCtx *ctx, uint8_t *pub)
{
