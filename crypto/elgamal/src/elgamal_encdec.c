                             ...
                             uint32_t *out1Len, uint8_t *out2, uint32_t *out2Len)
{
    ...
    bool createFailed = (m == NULL || r == NULL || yr == NULL || c1 == NULL || c2 == NULL || gcd_result == NULL || top == NULL);
    if (createFailed) {
        BSL_ERR_PUSH_ERROR(CRYPT_MEM_ALLOC_FAIL);
    ...
    ret = BN_ModExp(c1, pubKey->g, r, pubKey->p, optimizer);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        goto OUT;
    }
    ret = BN_ModExp(yr, pubKey->y, r, pubKey->p, optimizer);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        goto OUT;
    }
...
int32_t CRYPT_ELGAMAL_Encrypt(CRYPT_ELGAMAL_Ctx *ctx, const uint8_t *data, uint32_t dataLen,
    uint8_t *out,uint32_t *outLen)
{
    ...
    (void)memcpy_s(out, *outLen, out1, out1Len); // c1
    (void)memcpy_s(out+out1Len, *outLen-out1Len, out2, out2Len); // c2
ERR:
    BSL_SAL_FREE(out1);
    BSL_SAL_FREE(out2);
    return ret;
...
static int32_t DecryptInputCheck(const CRYPT_ELGAMAL_Ctx *ctx, const uint8_t *data, uint32_t dataLen,const uint8_t *out, const uint32_t *outLen)
{
...
static int32_t CRYPT_ELGAMAL_CheckCiphertext(const BN_BigNum *c1, const BN_BigNum *c2,
...
int32_t CRYPT_ELGAMAL_Decrypt(CRYPT_ELGAMAL_Ctx *ctx, const uint8_t *data, uint32_t dataLen,uint8_t *out, uint32_t *outLen)
{
    ...
    ret = CRYPT_ELGAMAL_CheckCiphertext(c1, c2, ctx->prvKey);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        goto OUT;
    }
...
static int32_t CRYPT_ELGAMAL_GetLen(const CRYPT_ELGAMAL_Ctx *ctx, GetLenFunc func, void *val, uint32_t len)
{
