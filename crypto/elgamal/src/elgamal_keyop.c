...
typedef struct {
    ...
    BSL_Param *q; /**< ElGamal private key parameter marked as x */
...
static int32_t SetPrvPara(const CRYPT_ELGAMAL_PrvKey *prvKey, const CRYPT_ElGamalPrvParam *prv)
{
    ...
    ret = BN_Bin2Bn(prvKey->q, prv->q->value, prv->q->valueLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
...
static int32_t SetPrvBasicCheck(const CRYPT_ELGAMAL_Ctx *ctx, BSL_Param *para, CRYPT_ElGamalPrvParam *prv)
{
    ...
    prv->q = BSL_PARAM_FindParam(para, CRYPT_PARAM_ELGAMAL_Q);
    if (PARAMISNULL(prv->p) || PARAMISNULL(prv->g) || PARAMISNULL(prv->x) || PARAMISNULL(prv->q) ||
        prv->p->valueLen == 0 || prv->g->valueLen == 0 || prv->x->valueLen == 0 || prv->q->valueLen == 0) {
