...
static CRYPT_ELGAMAL_PrvKey *ElGamalPrvKeyDupCtx(CRYPT_ELGAMAL_PrvKey *prvKey)
{
    ...
    GOTO_ERR_IF_SRC_NOT_NULL(newPrvKey->q, prvKey->q, BN_Dup(prvKey->q), CRYPT_MEM_ALLOC_FAIL);
...
CRYPT_ELGAMAL_Ctx *CRYPT_ELGAMAL_DupCtx(CRYPT_ELGAMAL_Ctx *keyCtx)
{
    ...
    CRYPT_ELGAMAL_Ctx *newKeyCtx = NULL;
    newKeyCtx = BSL_SAL_Malloc(sizeof(CRYPT_ELGAMAL_Ctx));
    if (newKeyCtx == NULL) {
        BSL_ERR_PUSH_ERROR(CRYPT_MEM_ALLOC_FAIL);
        return NULL;
    }
...
CRYPT_ELGAMAL_PrvKey *ElGamal_NewPrvKey(uint32_t bits)
{
    ...
    prvKey->q = BN_Create(bits);

    if (prvKey->p == NULL || prvKey->g == NULL || prvKey->x == NULL|| prvKey->q == NULL) {
        BSL_ERR_PUSH_ERROR(CRYPT_MEM_ALLOC_FAIL);
...
static int32_t ElGamal_CalcPrvKey(CRYPT_ELGAMAL_PrvKey *prvKey, CRYPT_ELGAMAL_Para *para, BN_Optimizer *optimizer)
{
    ...
    ret = BN_OriginalRoot(prvKey->g, prvKey->p, para->q, optimizer);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        goto OUT;
    }
