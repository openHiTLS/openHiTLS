...
static int32_t SetRsaesPkcsV15(CRYPT_RSA_Ctx *ctx, const void *val, uint32_t len)
{
    ...
    (void)memset_s(&(ctx->pad), sizeof(RSAPad), 0, sizeof(RSAPad));
    ctx->pad.para.pkcsv15.mdId = *(int32_t * )val;
...
static int32_t EalSetOaep(CRYPT_RSA_Ctx *ctx, BSL_Param *param)
{
    ...
    if ((temp = BSL_PARAM_FindParam(param, CRYPT_PARAM_RSA_MD_ID)) != NULL) {
    ...
    if ((temp = BSL_PARAM_FindParam(param, CRYPT_PARAM_RSA_MGF1_ID)) != NULL) {
...
static int32_t EalSetPss(CRYPT_RSA_Ctx *ctx, BSL_Param *param)
{
    ...
    if ((temp = BSL_PARAM_FindParam(param, CRYPT_PARAM_RSA_SALTLEN)) != NULL) {
