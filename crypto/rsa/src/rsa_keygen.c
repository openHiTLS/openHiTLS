...
static void InitRsaPubKeyParams(BSL_Param *params, uint32_t *index, uint8_t *buffer)
{
    (void)BSL_PARAM_InitValue(&params[(*index)++], CRYPT_PARAM_RSA_E, 
        BSL_PARAM_TYPE_OCTETS, buffer, RSA_MAX_MODULUS_LEN);
    (void)BSL_PARAM_InitValue(&params[(*index)++], CRYPT_PARAM_RSA_N, 
        BSL_PARAM_TYPE_OCTETS, buffer + RSA_MAX_MODULUS_LEN, RSA_MAX_MODULUS_LEN);
}
...
static void InitRsaPrvKeyParams(BSL_Param *params, uint32_t *index, uint8_t *buffer)
{
    (void)BSL_PARAM_InitValue(&params[(*index)++], CRYPT_PARAM_RSA_D, 
        BSL_PARAM_TYPE_OCTETS, buffer + RSA_MAX_MODULUS_LEN * 2, RSA_MAX_MODULUS_LEN);
    (void)BSL_PARAM_InitValue(&params[(*index)++], CRYPT_PARAM_RSA_P, 
        BSL_PARAM_TYPE_OCTETS, buffer + RSA_MAX_MODULUS_LEN * 3, RSA_MAX_MODULUS_LEN);
    (void)BSL_PARAM_InitValue(&params[(*index)++], CRYPT_PARAM_RSA_Q, 
        BSL_PARAM_TYPE_OCTETS, buffer + RSA_MAX_MODULUS_LEN * 4, RSA_MAX_MODULUS_LEN);
    (void)BSL_PARAM_InitValue(&params[(*index)++], CRYPT_PARAM_RSA_DP, 
        BSL_PARAM_TYPE_OCTETS, buffer + RSA_MAX_MODULUS_LEN * 5, RSA_MAX_MODULUS_LEN);
    (void)BSL_PARAM_InitValue(&params[(*index)++], CRYPT_PARAM_RSA_DQ, 
        BSL_PARAM_TYPE_OCTETS, buffer + RSA_MAX_MODULUS_LEN * 6, RSA_MAX_MODULUS_LEN);
    (void)BSL_PARAM_InitValue(&params[(*index)++], CRYPT_PARAM_RSA_QINV, 
        BSL_PARAM_TYPE_OCTETS, buffer + RSA_MAX_MODULUS_LEN * 7, RSA_MAX_MODULUS_LEN);
}

static void InitRsaPssParams(CRYPT_RSA_Ctx *ctx, BSL_Param *params, uint32_t *index)
{
...
static void InitRsaPkcsParams(CRYPT_RSA_Ctx *ctx, BSL_Param *params, uint32_t *index)
{
    (void)BSL_PARAM_InitValue(&params[(*index)++], CRYPT_PARAM_RSA_MD_ID, 
...
static BSL_Param *CallocRsaParams(CRYPT_RSA_Ctx *ctx, int32_t type)
{
    ...
    (void)BSL_PARAM_InitValue(&params[index++], CRYPT_PARAM_RSA_BITS, 
        BSL_PARAM_TYPE_UINT32, &bits, sizeof(bits));
    ...
    if (type == CRYPT_KEYMGMT_SELECT_UNKNOWN) {
        ...
        if (ctx->pad.type == EMSA_PSS) {
            InitRsaPssParams(ctx, params, &index);
        } else if (ctx->pad.type == EMSA_PKCSV15) {
            InitRsaPkcsParams(ctx, params, &index);
        }
        return params;
    }
    ...
    if ((type & CRYPT_KEYMGMT_SELECT_PARAMETER) != 0) {
        if (ctx->pad.type == EMSA_PSS) {
            InitRsaPssParams(ctx, params, &index);
        } else if (ctx->pad.type == EMSA_PKCSV15) {
            InitRsaPkcsParams(ctx, params, &index);
...
int32_t CRYPT_RSA_Export(CRYPT_RSA_Ctx *ctx, int32_t flag, int32_t type, CRYPT_EAL_ProcessFuncCb cb, void *args)
{
    ...
    int32_t ret = CRYPT_RSA_GetPubKey(ctx, params);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        goto EXIT;
    }
