...
{
    uint32_t len = GetEncapsulatedKeyLen(&cipher);
    if (len == 0) {
        return CRYPT_INVALID_ARG;
    }
    *encLen = len;
...
static int32_t HkdfExtract(CRYPT_MAC_AlgId macId, uint8_t *key, uint32_t keyLen, 
    ...
    uint8_t *suiteid, uint32_t suiteidLen, uint8_t *out, uint32_t outLen)
{
    ...
    int32_t ret = HkdfExtract(macId, labeledIkm, labeledIkmLen, salt, saltLen, out, outLen);
    ...
    uint8_t *info, uint32_t infoLen, uint8_t *suiteid, uint32_t suiteidLen, uint8_t *out, uint32_t outLen)
{
    ...
    int32_t ret = HkdfExpand(macId, prk, prkLen, labeledInfo, labeledInfoLen, out, outLen);
    ...
    uint32_t plainTextLen, uint8_t *cipherText, uint32_t *cipherTextLen)
{
    ...
    ctx->seq++;
    return HpkeAeadEncrypt(ctx, nonce, HPKE_AEAD_NONCE_LEN, aad, aadLen, plainText, plainTextLen, cipherText,
        cipherTextLen);
}
    ...
    uint8_t *buff, uint32_t buffLen)
{
    ...
    if (buffLen != GetKemSharedKeyLen(&ctx->cipher)) {
        return CRYPT_INVALID_ARG;
    }

    int32_t ret = KeySchedule(ctx, buff, buffLen, info, infoLen);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
