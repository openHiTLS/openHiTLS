...
static uint8_t HpkeGetKemIndex(CRYPT_HPKE_CipherSuite *cipher)
{
    uint8_t i;
    for (i = 0; i < sizeof(g_hpkeKemAlgInfo) / sizeof(HPKE_KemAlgInfo); i++) {
        if (cipher->kemId == g_hpkeKemAlgInfo[i].hpkeKemId) {
            return i;
        }
    }
    return HPKE_INVALID_ALG_INDEX;
}

static uint8_t HpkeGetKdfIndex(CRYPT_HPKE_CipherSuite *cipher)
{
    uint8_t i;
    for (i = 0; i < sizeof(g_hpkeKdfAlgInfo) / sizeof(HPKE_KdfAlgInfo); i++) {
        if (cipher->kdfId == g_hpkeKdfAlgInfo[i].hpkeKdfId) {
            return i;
        }
    }
    return HPKE_INVALID_ALG_INDEX;
...
static uint8_t HpkeGetAeadIndex(CRYPT_HPKE_CipherSuite *cipher)
{
    uint8_t i;
    for (i = 0; i < sizeof(g_hpkeAeadAlgInfo) / sizeof(HPKE_AeadAlgInfo); i++) {
        if (cipher->aeadId == g_hpkeAeadAlgInfo[i].hpkeAeadId) {
            return i;
        }
    }
    return HPKE_INVALID_ALG_INDEX;
...
static int32_t HpkeInitCipherSuite(CRYPT_EAL_HpkeCtx *ctx, CRYPT_HPKE_CipherSuite *cipher, CRYPT_EAL_LibCtx *libCtx,
    const char *attrName)
{
    uint8_t kemIndex = HpkeGetKemIndex(cipher);
    uint8_t kdfIndex = HpkeGetKdfIndex(cipher);
    uint8_t aeadIndex = HpkeGetAeadIndex(cipher);
    if (kemIndex == HPKE_INVALID_ALG_INDEX || kdfIndex == HPKE_INVALID_ALG_INDEX
        || aeadIndex == HPKE_INVALID_ALG_INDEX) {
    ...
    CRYPT_EAL_LibCtx *libCtx, const char *attrName)
{
    ...
    int32_t ret = HpkeInitCipherSuite(ctx, &cipher, libCtx, attrName);
    ...
    if (attrName != NULL && strlen(attrName) > 0) {
        ctx->attrName = BSL_SAL_Malloc(strlen(attrName) + 1);
        if (ctx->attrName == NULL) {
            CRYPT_EAL_HpkeFreeCtx(ctx);
            return NULL;
        }
        (void)memcpy_s(ctx->attrName, strlen(attrName) + 1, attrName, strlen(attrName) + 1);
    }
    ctx->mode = mode;
    ctx->role = role;
    ctx->libCtx = libCtx;
...
{
    uint8_t kemIndex = HpkeGetKemIndex(&cipher);
    if (kemIndex == HPKE_INVALID_ALG_INDEX) {
        return CRYPT_INVALID_ARG;
    }
    *encLen = g_hpkeKemAlgInfo[kemIndex].encapsulatedKeyLen;
...
static int32_t HpkeCreatePubKey(uint8_t kemIdex, const uint8_t *pubKey, uint32_t pubKeyLen, CRYPT_EAL_PkeyCtx **pkey,
    CRYPT_EAL_LibCtx *libCtx, const char *attrName)
{
...
static void HpkeGenerateHpkeSuiteId(uint8_t kemIndex, uint8_t kdfIndex, uint8_t aeadIndex, uint8_t *suiteId,
    uint32_t suiteIdLen)
{
    (void)memcpy_s(suiteId, suiteIdLen, "HPKE", strlen("HPKE"));
    uint32_t offset = strlen("HPKE");

    uint16_t id = HpkeI2OSPUint16(g_hpkeKemAlgInfo[kemIndex].hpkeKemId);
    (void)memcpy_s(suiteId + offset, suiteIdLen - offset, &id, sizeof(uint16_t));
    offset += sizeof(uint16_t);

    id = HpkeI2OSPUint16(g_hpkeKdfAlgInfo[kdfIndex].hpkeKdfId);
    (void)memcpy_s(suiteId + offset, suiteIdLen - offset, &id, sizeof(uint16_t));
    offset += sizeof(uint16_t);

    id = HpkeI2OSPUint16(g_hpkeAeadAlgInfo[aeadIndex].hpkeAeadId);
    (void)memcpy_s(suiteId + offset, suiteIdLen - offset, &id, sizeof(uint16_t));
}

static void HpkeGenerateKemSuiteId(uint8_t kemIdex, uint8_t *suiteId, uint32_t suiteIdLen)
{
    ...
    uint32_t outLen)
{
    ...
    ret = BSL_PARAM_InitValue(&params[2], CRYPT_PARAM_KDF_KEY, BSL_PARAM_TYPE_OCTETS, (void *)extractParams->key,
        extractParams->keyLen);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }

    ret = BSL_PARAM_InitValue(&params[3], CRYPT_PARAM_KDF_SALT, BSL_PARAM_TYPE_OCTETS, (void *)extractParams->salt,
        extractParams->saltLen);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
    ...
    uint32_t outLen)
{
    ...
    ret = BSL_PARAM_InitValue(&params[2], CRYPT_PARAM_KDF_PRK, BSL_PARAM_TYPE_OCTETS, (void *)expandParams->prk,
        expandParams->prkLen);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }

    ret = BSL_PARAM_InitValue(&params[3], CRYPT_PARAM_KDF_INFO, BSL_PARAM_TYPE_OCTETS, (void *)expandParams->info,
        expandParams->infoLen);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
    ...
    uint32_t kemContextLen, uint8_t *suiteId, uint32_t suiteIdLen, uint8_t *sharedSecret, uint32_t sharedSecretLen)
{
    ...
    HPKE_LabeledExtractParams extractParams = {macId, NULL, 0, (uint8_t *)"eae_prk", strlen("eae_prk"), dh, dhLen,
        suiteId, suiteIdLen};
    int32_t ret = HpkeLabeledExtract(ctx->kdfCtx, &extractParams, eaePrk, eaePrkLen);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }

    HPKE_LabeledExpandParams expandParams = {macId, eaePrk, eaePrkLen, (uint8_t *)"shared_secret",
        strlen("shared_secret"), kemContext, kemContextLen, suiteId, suiteIdLen};
    ...
    uint8_t *encapsulatedKey, uint32_t *encapsulatedKeyLen, uint8_t *sharedSecret, uint32_t sharedSecretLen)
{
    ...
    if (pkeyS == NULL) {
        CRYPT_HPKE_CipherSuite cipher = {g_hpkeKemAlgInfo[ctx->kemIndex].hpkeKemId,
            g_hpkeKdfAlgInfo[ctx->kdfIndex].hpkeKdfId, g_hpkeAeadAlgInfo[ctx->aeadIndex].hpkeAeadId};
        ret = CRYPT_EAL_HpkeGenerateKeyPair(cipher, NULL, 0, &pkeyS, ctx->libCtx, ctx->attrName);
        if (ret != CRYPT_SUCCESS) {
            return ret;
        }
    }
    ...
    uint32_t infoLen)
{
    ...
    HPKE_LabeledExtractParams params = {macId, sharedSecret, sharedSecretLen, (uint8_t*)"secret", strlen("secret"),
        NULL, 0, suiteId, suiteIdLen};
    ret = HpkeLabeledExtract(ctx->kdfCtx, &params, secret, secretLen);
    if (ret != CRYPT_SUCCESS) {
        goto end;
    }
...
static int32_t HpkeDecap(CRYPT_EAL_HpkeCtx *ctx, CRYPT_EAL_PkeyCtx *pkey, const uint8_t *encKey, uint32_t encKeyLen,
    uint8_t *sharedSecret, uint32_t sharedSecretLen)
{
    ...
    // kemContext = enc || pkRm
    uint32_t kemContextLen = encKeyLen + pubKeyDataLen;
    ...
    HPKE_LabeledExpandParams *params, uint8_t *sk, uint32_t skLen)
{
    ...
    uint8_t counter = 0;
    params->infoLen = 1;
    uint8_t bitmask = (g_hpkeKemAlgInfo[kemIndex].hpkeKemId == CRYPT_KEM_DHKEM_P521_HKDF_SHA512) ? 0x01 : 0xFF;
    do {
    ...
    CRYPT_EAL_PkeyCtx **pctx, CRYPT_EAL_LibCtx *libCtx, const char *attrName)
{
    ...
    if (g_hpkeKemAlgInfo[kemIndex].hpkeKemId == CRYPT_KEM_DHKEM_X25519_HKDF_SHA256) {
        expandParams.label = (uint8_t *)"sk";
        expandParams.labelLen = strlen("sk");
    ...
    CRYPT_EAL_PkeyCtx **pctx, CRYPT_EAL_LibCtx *libCtx, const char *attrName)
{
    ...
    int32_t ret = HpkeInitCipherSuite(NULL, &cipher, libCtx, attrName);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
    ...
    uint32_t keyLen)
{
    ...
    if (keyLen > 255 * g_hpkeKdfAlgInfo[ctx->kdfIndex].hkdfExtractKeyLen) {
        return CRYPT_INVALID_ARG;
    }
