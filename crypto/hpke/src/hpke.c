...
{
    int32_t ret = HpkeCheckSenderParams(ctx, pkR, pkRLen, encapsulatedKey, encapsulatedKeyLen);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
    ...
    ret = HpkeEncap(ctx, pkey, pkR, pkRLen, encapsulatedKey, encapsulatedKeyLen, sharedSecret, sharedSecretLen);
    if (ret != CRYPT_SUCCESS) {
...
{
    int32_t ret = HpkeCheckRecipientParams(ctx, pkey, encapsulatedKey, encapsulatedKeyLen);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
    ...
    ret = HpkeDecap(ctx, pkey, encapsulatedKey, encapsulatedKeyLen, sharedSecret, sharedSecretLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_SAL_ClearFree(sharedSecret, sharedSecretLen);
        return ret;
    }
