    ...
    uint32_t outLen)
{
    ...
    ret = BSL_PARAM_InitValue(&params[2], CRYPT_PARAM_KDF_KEY, BSL_PARAM_TYPE_OCTETS,
        (void *)extractParams->key, extractParams->keyLen); // param index 2
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }

    ret = BSL_PARAM_InitValue(&params[3], CRYPT_PARAM_KDF_SALT, BSL_PARAM_TYPE_OCTETS,
        (void *)extractParams->salt, extractParams->saltLen); // param index 3
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }

    ret = BSL_PARAM_InitValue(&params[4], CRYPT_PARAM_KDF_EXLEN, BSL_PARAM_TYPE_UINT32_PTR,
        (void *)&outLen, sizeof(outLen)); // param index 4
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
    ...
    uint32_t outLen)
{
    ...
    ret = BSL_PARAM_InitValue(&params[3], CRYPT_PARAM_KDF_INFO, BSL_PARAM_TYPE_OCTETS,
        (void *)expandParams->info, expandParams->infoLen); // param index 3
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
