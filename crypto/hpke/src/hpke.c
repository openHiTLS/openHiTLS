...
static int32_t GenerateKeyScheduleContext(CRYPT_EAL_HpkeCtx *ctx, const uint8_t *info, uint32_t infoLen,
    const uint8_t *pskId, uint32_t pskIdLen, const uint8_t *suiteid, uint32_t suiteidLen,
    uint8_t **keyScheduleContext, uint32_t *keyScheduleContextLen)
{
    ...
    int32_t ret = LabeledExtract(macId, NULL, 0, (const uint8_t*)"psk_id_hash", strlen("psk_id_hash"), pskId, pskIdLen,
    ...
    uint32_t contextLen = sizeof(uint8_t) + pskIdHashLen + infoHashLen;
    ...
    (void)memcpy_s(context + offset, contextLen - offset, pskIdHash, pskIdHashLen);
    offset += pskIdHashLen;
    (void)memcpy_s(context + offset, contextLen - offset, infoHash, infoHashLen);
...
int32_t CRYPT_EAL_HpkeGenerateKeyPair(CRYPT_HPKE_CipherSuite cipher, const uint8_t *ikm, uint32_t ikmLen,
    CRYPT_EAL_PkeyCtx **pctx)
{
    if (pctx == NULL || (ikm == NULL && ikmLen != 0) || (ikm != NULL && ikmLen == 0)) {
        return CRYPT_INVALID_ARG;
    }
