...
static int32_t CreatePriKey(CRYPT_HPKE_CipherSuite *cipher, const uint8_t *priKey, uint32_t priKeyLen,
    CRYPT_EAL_PkeyCtx **pkey)
{
    CRYPT_EAL_PkeyCtx *tmpPkey = NULL;
    int32_t ret = CreatePkeyCtx(cipher, &tmpPkey); 
...
static void GenerateHpkeSuiteid(CRYPT_HPKE_CipherSuite *cipher, uint8_t *suiteid)
{
...
static int32_t LabeledExtract(CRYPT_MAC_AlgId macId, const uint8_t *salt, uint32_t saltLen,
    const uint8_t *label, uint32_t labelLen, const uint8_t *ikm, uint32_t ikmLen,
    const uint8_t *suiteid, uint32_t suiteidLen, uint8_t *out, uint32_t *outLen)
{
...
static int32_t LabeledExpand(CRYPT_MAC_AlgId macId, const uint8_t *prk, uint32_t prkLen,
    const uint8_t *label, uint32_t labelLen, const uint8_t *info, uint32_t infoLen,
    const uint8_t *suiteid, uint32_t suiteidLen, uint8_t *out, uint32_t outLen)
{
    ...
    uint8_t *sharedSecret, uint32_t *sharedSecretLen)
{
    ...
    uint32_t eaePrkLen = HPKE_HKDF_MAX_EXTRACT_KEY_LEN;
    int32_t ret = LabeledExtract(macId, NULL, 0, (const uint8_t *)"eae_prk", strlen("eae_prk"), dh, dhLen,
        suiteid, suiteidLen, eaePrk, &eaePrkLen);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }

    ret = LabeledExpand(macId, eaePrk, eaePrkLen, (const uint8_t *)"shared_secret", strlen("shared_secret"),
    ...
    uint8_t *enc, uint32_t *encLen, uint8_t *sharedSecret, uint32_t *sharedSecretLen)
{
    ...
    memcpy(kemContext, tmpEnc, tmpEncLen);
    memcpy(kemContext + tmpEncLen, pkR, pkRLen);

    uint8_t suiteid[HPKE_KEM_SUITEID_LEN];
    GenerateKemSuiteid(ctx->cipher.kemId, suiteid);
    ret = ExtractAndExpand(ctx, dh, dhLen, kemContext, kemContextLen, suiteid, HPKE_KEM_SUITEID_LEN,
        sharedSecret, sharedSecretLen);
    BSL_SAL_Free(kemContext);

    memcpy(enc, tmpEnc, tmpEncLen);
    *encLen = tmpEncLen;
end:
    if (pkey == NULL) {
...
{
    // psk_id_hash
    uint8_t pskIdHash[HPKE_HKDF_MAX_EXTRACT_KEY_LEN];
    uint32_t pskIdHashLen = HPKE_HKDF_MAX_EXTRACT_KEY_LEN;
    CRYPT_MAC_AlgId macId = GetKdfMacAlgId(&ctx->cipher);
    int32_t ret = LabeledExtract(macId, NULL, 0, (const uint8_t*)"psk_id_hash", strlen("psk_id_hash"),
        (const uint8_t *)"", 0, suiteid, suiteidLen, pskIdHash, &pskIdHashLen);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }

    // info_hash
    uint8_t infoHash[HPKE_HKDF_MAX_EXTRACT_KEY_LEN];
    uint32_t infoHashLen = HPKE_HKDF_MAX_EXTRACT_KEY_LEN;
    ...
    const uint8_t *info, uint32_t infoLen)
{
    ...
    uint32_t secretLen = HPKE_KEM_MAX_SHARED_KEY_LEN;
    ret = LabeledExtract(macId, sharedSecret, sharedSecretLen, (uint8_t*)"secret", strlen("secret"), NULL, 0,
    ...
    ret = LabeledExpand(macId, secret, secretLen, (uint8_t*)"key", strlen("key"), context, contextLen,
        suiteid, suiteidLen, ctx->symKey, ctx->symKeyLen);
    if (ret != CRYPT_SUCCESS) {
        goto end;
    }

    ret = LabeledExpand(macId, secret, secretLen, (uint8_t*)"base_nonce", strlen("base_nonce"), context, contextLen,
        suiteid, suiteidLen, ctx->baseNonce,ctx->baseNonceLen);
    if (ret != CRYPT_SUCCESS) {
        goto end;
    }

    ret = LabeledExpand(macId, secret, secretLen, (uint8_t*)"exp", strlen("exp"), context, contextLen,
...
{
    if (ctx == NULL || seq == 0xFFFFFFFFFFFFFFFF) {
        return CRYPT_INVALID_ARG;
    }
    ...
    const uint8_t *plain, uint32_t plainLen, uint8_t *cipher, uint32_t *cipherLen)
{
    ...
    if (cipher == NULL) {
        *cipherLen = plainLen + HPKE_AEAD_TAG_LEN; // TAG len
        return CRYPT_SUCCESS;
    }
    ...
    uint32_t encapsulatedKeyLen, uint8_t *sharedSecret, uint32_t *sharedSecretLen)
{
    ...
    memcpy(kemContext + encapsulatedKeyLen, keyData, keyDataLen);
    ...
    const uint8_t *cipher, uint32_t cipherLen, uint8_t *plain, uint32_t *plainLen)
{
    ...
    if (cipherLen <= HPKE_AEAD_TAG_LEN) {
        return CRYPT_INVALID_ARG;
    }
    ...
    if (cipher == NULL) {
        *plainLen = cipherLen - HPKE_AEAD_TAG_LEN;
        return CRYPT_SUCCESS;
    }
    ...
    return HpkeAeadDecrypt(ctx, nonce, HPKE_AEAD_NONCE_LEN, aad, aadLen, cipher, cipherLen, plain, plainLen);
}
...
static int32_t ExpandEccPriKey(CRYPT_HPKE_CipherSuite *cipher, const uint8_t *dkpPrk, uint32_t dkpPrkLen,
    const uint8_t *suiteid, uint32_t suiteidLen, uint8_t *sk, uint32_t skLen)
{
    ...
    do {
        ret = LabeledExpand(macId, dkpPrk, dkpPrkLen, (const uint8_t *)"candidate", strlen("candidate"),
            (const uint8_t *)&counter, 1, suiteid, suiteidLen, sk, skLen);
        if (ret != CRYPT_SUCCESS) {
            break;
        }
...
static int32_t DeriveKeyPair(CRYPT_HPKE_CipherSuite *cipher, const uint8_t *ikm, uint32_t ikmLen,
    CRYPT_EAL_PkeyCtx **pctx)
{
    ...
    uint32_t dkpPrkLen = HPKE_HKDF_MAX_EXTRACT_KEY_LEN;
    CRYPT_MAC_AlgId macId = GetKemMacAlgId(cipher);
    int32_t ret =  LabeledExtract(macId, (const uint8_t *)"", 0, (const uint8_t *)"dkp_prk", strlen("dkp_prk"),
    ...
    if (cipher->kemId == CRYPT_KEM_DHKEM_X25519_HKDF_SHA256 || cipher->kemId == CRYPT_KEM_DHKEM_X448_HKDF_SHA512) {
        ret = LabeledExpand(macId, dkpPrk, dkpPrkLen, (const uint8_t *)"sk", strlen("sk"), (const uint8_t *)"", 0,
    ...
    uint32_t infoLen, uint8_t *key, uint32_t keyLen)
{
    ...
    CRYPT_MAC_AlgId macId = GetKdfMacAlgId(&ctx->cipher);
    return LabeledExpand(macId, ctx->exportSecret, ctx->exportSecretLen, (const uint8_t *)"sec", strlen("sec"),
        info, infoLen, suiteid, HPKE_HPKE_SUITEID_LEN, key, keyLen);
}

int32_t CRYPT_EAL_HpkeGetParam(CRYPT_EAL_HpkeCtx *ctx, CRYPT_HPKE_PARAM_TYPE type, unsigned char *buff,
    uint32_t *buffLen)
{
...
static int32_t HpkeSetSymKey(CRYPT_EAL_HpkeCtx *ctx, const unsigned char *buff, uint32_t buffLen)
{
    if (ctx->symKey != NULL) {
        return CRYPT_HPKE_ERR_CALL;
    }
    ...
    memcpy(ctx->symKey, buff, buffLen);
    ctx->symKeyLen = buffLen;
    return CRYPT_SUCCESS;
}

static int32_t HpkeSetBaseNonce(CRYPT_EAL_HpkeCtx *ctx, const unsigned char *buff, uint32_t buffLen)
{
    ...
    memcpy(ctx->baseNonce, buff, buffLen);
    ctx->baseNonceLen = buffLen;
    return CRYPT_SUCCESS;
}

static int32_t HpkeSetExporterSecret(CRYPT_EAL_HpkeCtx *ctx, const unsigned char *buff, uint32_t buffLen)
{
    if (ctx->exportSecret != NULL) {
        return CRYPT_HPKE_ERR_CALL;
    }
    ...
    memcpy(ctx->exportSecret, buff, buffLen);
    ctx->exportSecretLen = buffLen;
    return CRYPT_SUCCESS;
}

int32_t CRYPT_EAL_HpkeSetParam(CRYPT_EAL_HpkeCtx *ctx, CRYPT_HPKE_PARAM_TYPE type, const unsigned char *buff,
    uint32_t buffLen)
{
