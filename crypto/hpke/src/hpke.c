...
struct CRYPT_EAL_HpkeCtx {
    ...
    uint8_t *sharedSecret;            // Shared secret
    uint32_t sharedSecretLen;         // Length of the shared secret
};

static uint16_t HpkeI2OSPUint16(uint16_t value)
{
    uint16_t a = 0x1213;
    uint8_t *p = (uint8_t *)&a;
...
static CRYPT_MAC_AlgId HpkeGetKdfMacAlgId(CRYPT_HPKE_CipherSuite *cipher)
{
    switch (cipher->kdfId) {
...
static uint32_t HpkeGetKdfExtractKeyLen(CRYPT_HPKE_CipherSuite *cipher)
{
    switch (cipher->kdfId) {
        case CRYPT_KDF_HKDF_SHA256:
            return 32;
...
static CRYPT_CIPHER_AlgId HpkeGetAeadCipherAlgId(CRYPT_HPKE_CipherSuite *cipher)
{
...
static uint32_t HpkeGetAeadKeyLen(CRYPT_HPKE_CipherSuite *cipher)
{
    switch (cipher->aeadId) {
        case CRYPT_AEAD_AES_128_GCM:
            return 16;
...
static CRYPT_PKEY_AlgId HpkeGetKemPkeyAlgId(CRYPT_HPKE_CipherSuite *cipher)
{
    switch (cipher->kemId) {
...
static CRYPT_PKEY_ParaId HpkeGetKemPkeyCurveId(CRYPT_HPKE_CipherSuite *cipher)
{
    switch (cipher->kemId) {
        case CRYPT_KEM_DHKEM_P256_HKDF_SHA256:
...
static CRYPT_MAC_AlgId HpkeGetKemMacAlgId(CRYPT_HPKE_CipherSuite *cipher)
{
    switch (cipher->kemId) {
        case CRYPT_KEM_DHKEM_P256_HKDF_SHA256:
            return CRYPT_MAC_HMAC_SHA256;
...
static uint32_t HpkeGetKemPrivateKeyLen(CRYPT_HPKE_CipherSuite *cipher)
{
    switch (cipher->kemId) {
        case CRYPT_KEM_DHKEM_P256_HKDF_SHA256:
            return 32;
...
static uint32_t HpkeGetKemSharedKeyLen(CRYPT_HPKE_CipherSuite *cipher)
{
    switch (cipher->kemId) {
        case CRYPT_KEM_DHKEM_P256_HKDF_SHA256:
            return 32;
...
static uint32_t HpkeGetEncapsulatedKeyLen(CRYPT_HPKE_CipherSuite *cipher)
{
    switch (cipher->kemId) {
        case CRYPT_KEM_DHKEM_P256_HKDF_SHA256:
            return 65;
...
static uint32_t HpkeGetKemExtractKeyLen(CRYPT_HPKE_CipherSuite *cipher)
{
    switch (cipher->kemId) {
        case CRYPT_KEM_DHKEM_P256_HKDF_SHA256:
            return 32;
...
{
    if (HpkeGetKdfMacAlgId(cipher) == CRYPT_MAC_MAX) {
        return CRYPT_INVALID_ARG;
    }

    if (HpkeGetAeadCipherAlgId(cipher) == CRYPT_CIPHER_MAX) {
        return CRYPT_INVALID_ARG;
    }

    if (HpkeGetKemPkeyAlgId(cipher) == CRYPT_PKEY_MAX) {
        return CRYPT_INVALID_ARG;
    }
...
int32_t CRYPT_EAL_HpkeGetEncapsulatedKeyLen(CRYPT_HPKE_CipherSuite cipher, uint32_t *encLen)
{
    uint32_t len = HpkeGetEncapsulatedKeyLen(&cipher);
    if (len == 0) {
        return CRYPT_INVALID_ARG;
    }
    *encLen = len;
...
static int32_t HpkeCreatePkeyCtx(CRYPT_HPKE_CipherSuite *cipher, CRYPT_EAL_PkeyCtx **pkeyCtx)
{
    CRYPT_PKEY_AlgId algId = HpkeGetKemPkeyAlgId(cipher);
    ...
    if (algId == CRYPT_PKEY_ECDH) {
        CRYPT_PKEY_ParaId curveId = HpkeGetKemPkeyCurveId(cipher);
...
static int32_t HpkeCreatePubKey(CRYPT_HPKE_CipherSuite *cipher, const uint8_t *pubKey, uint32_t pubKeyLen,
    CRYPT_EAL_PkeyCtx **pkey)
{
    CRYPT_EAL_PkeyCtx *tmpPkey = NULL;
    int32_t ret = HpkeCreatePkeyCtx(cipher, &tmpPkey);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
...
static int32_t HpkeCreatePriKey(CRYPT_HPKE_CipherSuite *cipher, uint8_t *priKey, uint32_t priKeyLen,
    CRYPT_EAL_PkeyCtx **pkey)
{
    CRYPT_EAL_PkeyCtx *tmpPkey = NULL;
    int32_t ret = HpkeCreatePkeyCtx(cipher, &tmpPkey); 
...
static void HpkeGenerateHpkeSuiteId(CRYPT_HPKE_CipherSuite *cipher, uint8_t *suiteId, uint32_t suiteIdLen)
{
    (void)memcpy_s(suiteId, suiteIdLen, "HPKE", strlen("HPKE"));
    uint32_t offset = strlen("HPKE");

    uint16_t id = HpkeI2OSPUint16(cipher->kemId);
    (void)memcpy_s(suiteId + offset, suiteIdLen - offset, &id, sizeof(uint16_t));
    offset += sizeof(uint16_t);

    id = HpkeI2OSPUint16(cipher->kdfId);
    (void)memcpy_s(suiteId + offset, suiteIdLen - offset, &id, sizeof(uint16_t));
    offset += sizeof(uint16_t);

    id = HpkeI2OSPUint16(cipher->aeadId);
    (void)memcpy_s(suiteId + offset, suiteIdLen - offset, &id, sizeof(uint16_t));
}

static void HpkeGenerateKemSuiteId(CRYPT_HPKE_KEM_AlgId kemId, uint8_t *suiteId, uint32_t suiteIdLen)
{
    (void)memcpy_s(suiteId, suiteIdLen, "KEM", strlen("KEM"));
    uint32_t offset = strlen("KEM");

    uint16_t kemIdNew = HpkeI2OSPUint16(kemId);
    (void)memcpy_s(suiteId + offset, suiteIdLen - offset, &kemIdNew, sizeof(uint16_t));
}

static int32_t HpkeHkdfExtract(CRYPT_MAC_AlgId macId, uint8_t *key, uint32_t keyLen, uint8_t *salt, uint32_t saltLen,
...
static int32_t HpkeHkdfExpand(CRYPT_MAC_AlgId macId, uint8_t *prk, uint32_t prkLen, uint8_t *info,
    ...
    uint8_t *suiteId, uint32_t suiteIdLen, uint8_t *out, uint32_t outLen)
{
    ...
    int32_t ret = HpkeHkdfExtract(macId, labeledIkm, labeledIkmLen, salt, saltLen, out, outLen);
    ...
    uint8_t *info, uint32_t infoLen, uint8_t *suiteId, uint32_t suiteIdLen, uint8_t *out, uint32_t outLen)
{
    ...
    uint32_t offset = 0;
    uint16_t outLenTmp = HpkeI2OSPUint16(outLen);
    ...
    int32_t ret = HpkeHkdfExpand(macId, prk, prkLen, labeledInfo, labeledInfoLen, out, outLen);
...
{
    CRYPT_MAC_AlgId macId = HpkeGetKemMacAlgId(&ctx->cipher);
    uint8_t eaePrk[HPKE_HKDF_MAX_EXTRACT_KEY_LEN];
    uint32_t eaePrkLen = HpkeGetKemExtractKeyLen(&ctx->cipher);
    ...
    uint8_t *out, uint32_t *outLen)
{
    ...
    ephemPub.id = HpkeGetKemPkeyAlgId(cipher);
    ...
    uint8_t *encapsulatedKey, uint32_t *encapsulatedKeyLen, uint8_t *sharedSecret, uint32_t sharedSecretLen)
{
    ...
    ret = GetPubKeyData(pkeyS, &ctx->cipher, enc, &encLen);
    if (ret != CRYPT_SUCCESS) {
        goto end;
    }

    ret = HpkeCreatePubKey(&ctx->cipher, pkR, pkRLen, &pkeyR);
    if (ret != CRYPT_SUCCESS) {
        goto end;
    }
    ...
    HpkeGenerateKemSuiteId(ctx->cipher.kemId, suiteId, HPKE_KEM_SUITEID_LEN);
...
{
    uint32_t extractKeyLen = HpkeGetKdfExtractKeyLen(&ctx->cipher);
    ...
    CRYPT_MAC_AlgId macId = HpkeGetKdfMacAlgId(&ctx->cipher);
...
{
    ctx->symKeyLen = HpkeGetAeadKeyLen(&ctx->cipher);
    ...
    ctx->exportSecretLen = HpkeGetKdfExtractKeyLen(&ctx->cipher);
    ...
    uint32_t infoLen)
{
    ...
    HpkeGenerateHpkeSuiteId(&ctx->cipher, suiteId, HPKE_HPKE_SUITEID_LEN);
    uint32_t contextLen;
    ...
    uint32_t secretLen = HpkeGetKdfExtractKeyLen(&ctx->cipher);
...
int32_t CRYPT_EAL_HpkeSetupSender(CRYPT_EAL_HpkeCtx *ctx, CRYPT_EAL_PkeyCtx *pkey, uint8_t *info, uint32_t infoLen,
    const uint8_t *pkR, uint32_t pkRLen, uint8_t *encapsulatedKey, uint32_t *encapsulatedKeyLen)
{
    if (ctx == NULL || pkR == NULL || pkRLen == 0 || encapsulatedKey == NULL || encapsulatedKeyLen == NULL) {
    ...
    if (pkRLen != HpkeGetEncapsulatedKeyLen(&ctx->cipher)) {
        return CRYPT_INVALID_ARG;
    }

    if (*encapsulatedKeyLen < HpkeGetEncapsulatedKeyLen(&ctx->cipher)) {
        return CRYPT_INVALID_ARG;
    }

    uint32_t sharedSecretLen = HpkeGetKemSharedKeyLen(&ctx->cipher);
    uint8_t *sharedSecret = BSL_SAL_Malloc(sharedSecretLen);
    if (sharedSecret == NULL) {
        return CRYPT_MEM_ALLOC_FAIL;
    }

    int32_t ret = HpkeEncap(ctx, pkey, pkR, pkRLen, encapsulatedKey, encapsulatedKeyLen, sharedSecret, sharedSecretLen);
    if (ret != CRYPT_SUCCESS) {
...
{
    CRYPT_CIPHER_AlgId id = HpkeGetAeadCipherAlgId(&ctx->cipher);
    ...
    uint32_t plainTextLen, uint8_t *cipherText, uint32_t *cipherTextLen)
{
    ...
    int32_t ret = HpkeAeadEncrypt(ctx, nonce, HPKE_AEAD_NONCE_LEN, aad, aadLen, plainText, plainTextLen, cipherText,
        cipherTextLen);
    if (ret == CRYPT_SUCCESS) {
        ctx->seq++;
    }
    ...
    uint32_t encapsulatedKeyLen, uint8_t *sharedSecret, uint32_t sharedSecretLen)
{
    ...
    int32_t ret = HpkeCreatePubKey(&ctx->cipher, encapsulatedKey, encapsulatedKeyLen, &pkeyS);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
    ...
    uint32_t aadLen, const uint8_t *cipherText, uint32_t cipherTextLen, uint8_t *plainText, uint32_t *plainTextLen)
{
    ...
    if (memcmp(newTag, cipherText + (cipherTextLen - HPKE_AEAD_TAG_LEN), HPKE_AEAD_TAG_LEN) != 0) {
...
{
    if (ctx == NULL || cipherText == NULL || cipherTextLen == 0 || plainTextLen == NULL) {
        return CRYPT_NULL_INPUT;
    }
    ...
    int32_t ret = HpkeAeadDecrypt(ctx, nonce, HPKE_AEAD_NONCE_LEN, aad, aadLen, cipherText, cipherTextLen, plainText,
        plainTextLen);
    if (ret == CRYPT_SUCCESS) {
        ctx->seq++;
    }
...
static int32_t HpkeGetEccOrder(CRYPT_HPKE_CipherSuite *cipher, BN_BigNum **order)
{
    CRYPT_EAL_PkeyCtx *pkey = NULL;
    int32_t ret = HpkeCreatePkeyCtx(cipher, &pkey);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
...
static int32_t HpkeExpandEccPriKey(CRYPT_HPKE_CipherSuite *cipher, uint8_t *dkpPrk, uint32_t dkpPrkLen, uint8_t *suiteId,
    uint32_t suiteIdLen, uint8_t *sk, uint32_t skLen)
{
    CRYPT_MAC_AlgId macId = HpkeGetKemMacAlgId(cipher);
    int32_t ret;
    ...
    ret = HpkeGetEccOrder(cipher, &order);
    if (ret != CRYPT_SUCCESS) {
        BN_Destroy(skBn);
        return ret;
    }
...
static int32_t HpkeDeriveKeyPair(CRYPT_HPKE_CipherSuite *cipher, uint8_t *ikm, uint32_t ikmLen, CRYPT_EAL_PkeyCtx **pctx)
{
    uint8_t suiteId[HPKE_KEM_SUITEID_LEN];
    HpkeGenerateKemSuiteId(cipher->kemId, suiteId, HPKE_KEM_SUITEID_LEN);

    uint8_t dkpPrk[HPKE_HKDF_MAX_EXTRACT_KEY_LEN];
    uint32_t dkpPrkLen = HpkeGetKemExtractKeyLen(cipher);
    CRYPT_MAC_AlgId macId = HpkeGetKemMacAlgId(cipher);
    ...
    uint32_t skLen = HpkeGetKemPrivateKeyLen(cipher);
    if (cipher->kemId == CRYPT_KEM_DHKEM_X25519_HKDF_SHA256) {
        ret = HpkeLabeledExpand(macId, dkpPrk, dkpPrkLen, (uint8_t *)"sk", strlen("sk"), (uint8_t *)"", 0,
            suiteId, HPKE_KEM_SUITEID_LEN, sk, skLen);
    } else {
        ret = HpkeExpandEccPriKey(cipher, dkpPrk, dkpPrkLen, suiteId, HPKE_KEM_SUITEID_LEN, sk, skLen);
    }
    ...
    CRYPT_EAL_PkeyCtx *pkey = NULL;
    ret = HpkeCreatePriKey(cipher, sk, skLen, &pkey);
    BSL_SAL_CleanseData(sk, skLen);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
    ...
    CRYPT_EAL_PkeyCtx **pctx)
{
    ...
    uint32_t ikmNewLen = HpkeGetKemPrivateKeyLen(&cipher);
    if (ikmLen != 0) {
        if (ikmLen < ikmNewLen) {
            return CRYPT_INVALID_ARG;
        }
        return HpkeDeriveKeyPair(&cipher, ikm, ikmLen, pctx);
    }
    ...
    ret = HpkeDeriveKeyPair(&cipher, ikmNew, ikmNewLen, pctx);
    ...
    uint32_t keyLen)
{
    ...
    if (keyLen > 255 * HpkeGetKdfExtractKeyLen(&ctx->cipher)) {
        return CRYPT_INVALID_ARG;
    }
    ...
    uint8_t *buff, uint32_t buffLen)
{
    ...
    if (buffLen != HpkeGetKemSharedKeyLen(&ctx->cipher)) {
        return CRYPT_INVALID_ARG;
    }
    ...
    ctx->sharedSecret = BSL_SAL_Malloc(buffLen);
    if (ctx->sharedSecret == NULL) {
        HpkeFreeKeyInfo(ctx);
        return CRYPT_MEM_ALLOC_FAIL;
    }
    (void)memcpy_s(ctx->sharedSecret, buffLen, buff, buffLen);
