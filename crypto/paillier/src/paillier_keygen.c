...
static int32_t Paillier_GenPQ(void *libCtx, CRYPT_PAILLIER_Para *para, BN_Optimizer *optimizer)
{
    uint32_t bits = para->bits;
    int32_t ret = BN_GenPrimeEx(libCtx, para->p, bits, true, optimizer, NULL);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ret = BN_GenPrimeEx(libCtx, para->q, bits, true, optimizer, NULL);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
    }
    return ret;
...
int32_t CRYPT_PAILLIER_Gen(CRYPT_PAILLIER_Ctx *ctx)
{
    ...
    ret = Paillier_GenPQ(ctx->libCtx, newCtx->para, optimizer);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        goto ERR;
    }
