...
#if defined(HITLS_CRYPTO_RSA) || defined(HITLS_CRYPTO_ECDSA) || defined(HITLS_CRYPTO_SM2) || defined(HITLS_CRYPTO_ED25519)
typedef struct {
...
{ \
    ...
    int32_t ret = CRYPT_EAL_SetPkeyMethod(&ctx->method, keyMethod, asyCipherMethod, exchMethod, signMethod, kemMethod); \
...
int32_t DECODER_Der2Key_SetParam(void *ctx, const BSL_Param *param)
{
    ...
    const BSL_Param *flag = BSL_PARAM_FindConstParam(param, CRYPT_PARAM_DECODE_FLAG_FREE_OUTDATA);
    if (flag != NULL) {
        if (flag->valueType != BSL_PARAM_TYPE_BOOL) {
            BSL_ERR_PUSH_ERROR(CRYPT_INVALID_ARG);
            return CRYPT_INVALID_ARG;
        }
        decoderCtx->isFreeOutData = *(const bool *)flag->value;
    }

    return CRYPT_SUCCESS;
}

#define DECODER_CHECK_PARAMS(decoderCtx, inParam, outParam) \
    int32_t ret; \
    void *key = NULL; \
    DECODER_Der2Key_Ctx *decoderCtx = (DECODER_Der2Key_Ctx *)ctx; \
    ...
    BSL_Buffer asn1Encode = {(uint8_t *)(uintptr_t)input->value, input->valueLen};

#define DECODER_CONSTRUCT_OUPUT_PARAMS(outParam) \
...
{ \
    DECODER_CHECK_PARAMS(decoderCtx, inParam, outParam) \
    ret = parseFunc(asn1Encode.data, asn1Encode.dataLen, NULL, (keyStructName **)&key); \
    if (ret != CRYPT_SUCCESS) { \
        BSL_ERR_PUSH_ERROR(ret); \
        return ret; \
    } \
    DECODER_CONSTRUCT_OUPUT_PARAMS(outParam) \
}
...
#endif
#ifdef HITLS_CRYPTO_SM2
DECODER_DEFINE_DER2KEY_NEW_CTX(Sm2, CRYPT_PKEY_SM2, g_defKeyMgmtSm2, g_defAsymCipherSm2, g_defExchSm2, g_defSignSm2, NULL)
#endif
#ifdef HITLS_CRYPTO_ED25519
