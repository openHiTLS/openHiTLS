...
#if defined(HITLS_CRYPTO_ENCODE_DECODE_KEY) && (defined(HITLS_CRYPTO_ECDSA) || \
    defined(HITLS_CRYPTO_SM2) || defined(HITLS_CRYPTO_ED25519))
#include "crypt_ecc.h"
...
int32_t CRYPT_ECC_ParseSubPubkeyAsn1Buff(uint8_t *buff, uint32_t buffLen, void **pubKey, bool isComplete)
{
    ...
    void *pctx = NULL;
    int32_t ret = CRYPT_DECODE_ParseSubPubkey(buff, buffLen, &subPubkeyInfo, isComplete);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ...
    bool isComplete)
{
    ...
    CRYPT_DECODE_SubPubkeyInfo subPubkeyInfo = {0};
    int32_t ret = CRYPT_DECODE_ParseSubPubkey(buffer, bufferLen, &subPubkeyInfo, isComplete);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
...
#endif // HITLS_CRYPTO_ENCODE_DECODE_KEY && HITLS_CRYPTO_RSA
