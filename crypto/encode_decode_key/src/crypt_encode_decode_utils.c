...
#if defined(HITLS_CRYPTO_ENCODE_DECODE_KEY)
#include <stdint.h>
#include "securec.h"
#include "bsl_types.h"
#include "bsl_asn1.h"
...
int32_t CRYPT_DECODE_ParseAlgoIdAsn1Buff(uint8_t *buff, uint32_t buffLen, BSL_ASN1_Buffer *algoId, uint32_t algoIdNum)
{
    uint8_t *tmpBuff = buff;
    uint32_t tmpBuffLen = buffLen;
...
int32_t CRYPT_DECODE_ParseSubPubkey(uint8_t *buff, uint32_t buffLen, CRYPT_DECODE_SubPubkeyInfo *subPubkeyInfo,
    bool isComplete)
{
    ...
    ret = CRYPT_DECODE_ParseAlgoIdAsn1Buff(pubAsn1->buff, pubAsn1->len, algoId, BSL_ASN1_TAG_ALGOID_ANY_IDX + 1);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
...
static int32_t ParsePk8PriParamAsn1(BSL_ASN1_Buffer *encode, BslCid *keyType, BSL_ASN1_Buffer *keyParam)
{
    ...
    BSL_ASN1_Buffer algoId[BSL_ASN1_TAG_ALGOID_ANY_IDX + 1] = {0};
    int32_t ret = CRYPT_DECODE_ParseAlgoIdAsn1Buff(algo->buff, algo->len, algoId, BSL_ASN1_TAG_ALGOID_ANY_IDX + 1);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
...
int32_t CRYPT_DECODE_Pkcs8Info(uint8_t *buff, uint32_t buffLen, CRYPT_ENCODE_DECODE_Pk8PrikeyInfo *pk8PrikeyInfo)
{
    ...
    if (keyType == BSL_CID_EC_PUBLICKEY || keyType == BSL_CID_RSASSAPSS) {
...
static int32_t ParseDeriveKeyPrfAlgId(BSL_ASN1_Buffer *asn, int32_t *prfId)
{
    if (asn->len != 0) {
        ...
        int32_t ret = CRYPT_DECODE_ParseAlgoIdAsn1Buff(asn->buff, asn->len, algoId, 2);
        if (ret != CRYPT_SUCCESS) {
            BSL_ERR_PUSH_ERROR(ret);
            return ret;
        }
