...
void* encrypt_message_thread(void* arg)
{
    message_thread_arg_t* targ = (message_thread_arg_t*)arg;
    int msg_index = targ->msg_index;
    ...
    int padded_len = 0;
    unsigned char* padded = pkcs7_padding((unsigned char*)plaintext, file_size, 16, &padded_len);
    free(plaintext);
    ...
    for (int i = 0; i < num_blocks; i++) {
        ...
        for (int j = 0; j < 16; j++) {
            buf[j] = padded[i * 16 + j] ^ iv[j];
        }
        sm4_one_round(ctx.sk, buf, ciphertext + i * 16);
        memcpy(iv, ciphertext + i * 16, 16);
    }

    // print_hex(ciphertext, padded_len); // 可选打印结果
    free(padded);
    free(ciphertext);
    printf("线程 %d 结束\n", msg_index);
    free(targ);
...
int main(void)
{
    ...
    int num_messages = sizeof(filenames) / sizeof(filenames[0]);

    pthread_t* threads = malloc(num_messages * sizeof(pthread_t));
    if (!threads) {
    ...
    for (int i = 0; i < num_messages; i++) {
        message_thread_arg_t* targ = malloc(sizeof(message_thread_arg_t));
        if (!targ) {
            fprintf(stderr, "内存分配失败！\n");
            exit(-1);
        }
        targ->msg_index = i;
        targ->filename = filenames[i];
        if (pthread_create(&threads[i], NULL, encrypt_message_thread, targ) != 0) {
    ...
    double enc_time = (end.tv_sec - start.tv_sec) + (end.tv_usec - start.tv_usec) / 1e6;
