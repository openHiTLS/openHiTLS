#include <stdint.h>
#include <string.h>
#include <stdlib.h>
#include <stdio.h>

// SM4完整S盒（256字节）
static const uint8_t sbox[256] = {
    
};

// 系统参数FK
static const uint32_t FK[4] = { 0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC };

// 固定参数CK（32个）
static const uint32_t CK[32] = {
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
};

// 循环左移
static inline uint32_t rotate_left(uint32_t x, int n) {
    return (x << n) | (x >> (32 - n));
}

// S盒替换
static uint32_t sbox_replace(uint32_t word) {
    uint8_t b0 = sbox[(word >> 24) & 0xFF];
    uint8_t b1 = sbox[(word >> 16) & 0xFF];
    uint8_t b2 = sbox[(word >> 8) & 0xFF];
    uint8_t b3 = sbox[word & 0xFF];
    return (b0 << 24) | (b1 << 16) | (b2 << 8) | b3;
}

// 线性变换
static uint32_t linear_transform_L(uint32_t x) {
    return x ^ rotate_left(x, 2) ^ rotate_left(x, 10) ^ rotate_left(x, 18) ^ rotate_left(x, 24);
}

static uint32_t linear_transform_Lprime(uint32_t x) {
    return x ^ rotate_left(x, 13) ^ rotate_left(x, 23);
}

// 密钥拓展
void sm4_key_schedule(const uint8_t key[16], uint32_t rk[32]) {
    uint32_t k[36];
    // 将16字节密钥转换为4个大端序字
    k[0] = ((uint32_t)key[0] << 24) | ((uint32_t)key[1] << 16) |
        ((uint32_t)key[2] << 8) | key[3];
    k[1] = ((uint32_t)key[4] << 24) | ((uint32_t)key[5] << 16) |
        ((uint32_t)key[6] << 8) | key[7];
    k[2] = ((uint32_t)key[8] << 24) | ((uint32_t)key[9] << 16) |
        ((uint32_t)key[10] << 8) | key[11];
    k[3] = ((uint32_t)key[12] << 24) | ((uint32_t)key[13] << 16) |
        ((uint32_t)key[14] << 8) | key[15];

    k[0] ^= FK[0];
    k[1] ^= FK[1];
    k[2] ^= FK[2];
    k[3] ^= FK[3];

    for (int i = 0; i < 32; i++) {
        uint32_t tmp = k[i + 1] ^ k[i + 2] ^ k[i + 3] ^ CK[i];
        tmp = sbox_replace(tmp);
        tmp = linear_transform_Lprime(tmp);
        rk[i] = k[i] ^ tmp;
        k[i + 4] = rk[i];
    }
}

// 加密单块
void sm4_encrypt_block(const uint32_t rk[32], const uint8_t plaintext[16], uint8_t ciphertext[16]) {
    uint32_t x[36];
    // 大端序加载明文
    for (int i = 0; i < 4; i++) {
        x[i] = ((uint32_t)plaintext[4 * i] << 24) |
            ((uint32_t)plaintext[4 * i + 1] << 16) |
            ((uint32_t)plaintext[4 * i + 2] << 8) |
            plaintext[4 * i + 3];
    }

    for (int i = 0; i < 32; i++) {
        uint32_t tmp = x[i + 1] ^ x[i + 2] ^ x[i + 3] ^ rk[i];
        tmp = sbox_replace(tmp);
        tmp = linear_transform_L(tmp);
        x[i + 4] = x[i] ^ tmp;
    }

    // 大端序存储密文
    for (int i = 0; i < 4; i++) {
        ciphertext[4 * i] = (x[35 - i] >> 24) & 0xFF;
        ciphertext[4 * i + 1] = (x[35 - i] >> 16) & 0xFF;
        ciphertext[4 * i + 2] = (x[35 - i] >> 8) & 0xFF;
        ciphertext[4 * i + 3] = x[35 - i] & 0xFF;
    }
}

// CBC模式加密
void sm4_cbc_encrypt(const uint8_t* plaintext, size_t len,
    const uint8_t key[16], const uint8_t iv[16],
    uint8_t* ciphertext) {
    uint32_t rk[32];
    sm4_key_schedule(key, rk);

    uint8_t previous_block[16];
    memcpy(previous_block, iv, 16);

    size_t pad = 16 - (len % 16);
    if (pad == 0) pad = 16;
    size_t padded_len = len + pad;
    uint8_t* padded_data = (uint8_t*)malloc(padded_len);
    memcpy(padded_data, plaintext, len);
    memset(padded_data + len, pad, pad);

    for (size_t i = 0; i < padded_len; i += 16) {
        uint8_t block[16];
        for (int j = 0; j < 16; j++) {
            block[j] = padded_data[i + j] ^ previous_block[j];
        }
        sm4_encrypt_block(rk, block, ciphertext + i);
        memcpy(previous_block, ciphertext + i, 16);
    }
    free(padded_data);
}

// 测试用例
int main() {
    uint8_t key[16] = {
        0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,
        0xfe,0xdc,0xba,0x98,0x76,0x54,0x32,0x10
    };
    uint8_t iv[16] = { 0 };
    uint8_t plaintext[16] = {
        0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,
        0xfe,0xdc,0xba,0x98,0x76,0x54,0x32,0x10
    };
    uint8_t ciphertext[32];

    sm4_cbc_encrypt(plaintext, 16, key, iv, ciphertext);

    printf("Ciphertext: ");
    for (int i = 0; i < 16; i++) {
        printf("%02x", ciphertext[i]);
    }
    printf("\n");
    return 0;
}
