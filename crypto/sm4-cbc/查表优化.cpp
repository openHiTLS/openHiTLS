...
#include <string.h>
#include <stdint.h>
#include <time.h>  // 需要包含这个头文件以使用计时功能
...
int main(void)
{
    ...
    /* ----------------- 加密过程 ----------------- */
    /* 待加密明文：从文件 msg1.txt 中读取 */
    const char* infile = "msg1.txt";
    ...
    size_t read_bytes = fread(plaintext, 1, file_size, fp_in);
    plaintext[read_bytes] = '\0';
    fclose(fp_in);
    ...
    fwrite(ciphertext, 1, padded_len, fp_out);
    fclose(fp_out);
    printf("密文已写入文件 %s\n", outfile);

    free(padded);
    free(plaintext);
    /* 注意：这里先保留 ciphertext 缓冲区供后续解密使用，
       实际中可以选择将其释放，并在解密时从文件中重新读取 */

       /* ----------------- 解密过程 ----------------- */
       /* 从文件 msg2.txt 读取密文 */
    fp_in = fopen(outfile, "rb");
    if (!fp_in) {
        fprintf(stderr, "无法打开文件 %s 进行解密\n", outfile);
        free(ciphertext);
        return -1;
    }
    fseek(fp_in, 0, SEEK_END);
    long cipher_file_size = ftell(fp_in);
    rewind(fp_in);
    unsigned char* cipher_from_file = (unsigned char*)malloc(cipher_file_size);
    if (!cipher_from_file) {
        fprintf(stderr, "内存分配失败！\n");
        fclose(fp_in);
        free(ciphertext);
    ...
    fread(cipher_from_file, 1, cipher_file_size, fp_in);
    fclose(fp_in);

    /* 为解密分配缓冲区 */
    unsigned char* decrypted = (unsigned char*)malloc(cipher_file_size);
    if (!decrypted) {
        fprintf(stderr, "内存分配失败！\n");
        free(cipher_from_file);
        free(ciphertext);
        return -1;
    }
    ...
    sm4_crypt_cbc(&ctx, 0, cipher_file_size, iv_dec, cipher_from_file, decrypted);
    clock_t end_dec = clock();
    long double dec_time = (long double)(end_dec - start_dec) / CLOCKS_PER_SEC;
    int unpadded_len = pkcs7_unpadding(decrypted, cipher_file_size, 16);
    if (unpadded_len < 0) {
        printf("解密时去填充失败！\n");
    }
    else {
