#include <stdio.h>
#include <stdint.h>
#include <string.h>

// 32-bit 循环左移
#define SM4_Rotl32(buf, n) (((buf) << (n)) | ((buf) >> (32 - (n))))

// 固定参数 CK（用于密钥扩展）
static const uint32_t SM4_CK[32] = {
    0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269,
    0x70777E85, 0x8C939AA1, 0xA8AFB6BD, 0xC4CBD2D9,
    0xE0E7EEF5, 0xFC030A11, 0x181F262D, 0x343B4249,
    0x50575E65, 0x6C737A81, 0x888F969D, 0xA4ABB2B9,
    0xC0C7CED5, 0xDCE3EAF1, 0xF8FF060D, 0x141B2229,
    0x30373E45, 0x4C535A61, 0x686F767D, 0x848B9299,
    0xA0A7AEB5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB0209,
    0x10171E25, 0x2C333A41, 0x484F565D, 0x646B7279
};

// S 盒（非线性变换）
static const uint8_t SM4_Sbox[256] = {
    
};

// 系统参数 FK（用于密钥初始化）
static const uint32_t SM4_FK[4] = {
    0xA3B1BAC6, 0x56AA3350, 0x677D9197, 0xB27022DC
};

// 密钥扩展
void SM4_KeySchedule(const uint8_t MK[16], uint32_t rk[32]) {
    uint32_t K[36], tmp, buf;
    for (int i = 0; i < 4; i++) {
        K[i] = SM4_FK[i] ^
               ((uint32_t)MK[4*i] << 24 | (uint32_t)MK[4*i+1] << 16 |
                (uint32_t)MK[4*i+2] << 8 | (uint32_t)MK[4*i+3]);
    }
    for (int i = 0; i < 32; i++) {
        tmp = K[i+1] ^ K[i+2] ^ K[i+3] ^ SM4_CK[i];
        buf = (uint32_t)SM4_Sbox[(tmp>>24)&0xFF] <<24 |
              (uint32_t)SM4_Sbox[(tmp>>16)&0xFF] <<16 |
              (uint32_t)SM4_Sbox[(tmp>>8 )&0xFF] <<8  |
              (uint32_t)SM4_Sbox[tmp&0xFF];
        K[i+4] = K[i] ^ (buf ^ SM4_Rotl32(buf,13) ^ SM4_Rotl32(buf,23));
        rk[i] = K[i+4];
    }
}

// 单块加密
void SM4_EncryptBlock(const uint8_t MK[16], const uint8_t in[16], uint8_t out[16]) {
    uint32_t X[36], tmp, buf, rk[32];
    SM4_KeySchedule(MK, rk);
    for (int j = 0; j < 4; j++) {
        X[j] = (uint32_t)in[4*j]<<24 | (uint32_t)in[4*j+1]<<16 |
               (uint32_t)in[4*j+2]<<8 | (uint32_t)in[4*j+3];
    }
    for (int i = 0; i < 32; i++) {
        tmp = X[i+1] ^ X[i+2] ^ X[i+3] ^ rk[i];
        buf = (uint32_t)SM4_Sbox[(tmp>>24)&0xFF] <<24 |
              (uint32_t)SM4_Sbox[(tmp>>16)&0xFF] <<16 |
              (uint32_t)SM4_Sbox[(tmp>>8 )&0xFF] <<8  |
              (uint32_t)SM4_Sbox[tmp&0xFF];
        X[i+4] = X[i] ^ (buf ^ SM4_Rotl32(buf,2) ^ SM4_Rotl32(buf,10)
                         ^ SM4_Rotl32(buf,18) ^ SM4_Rotl32(buf,24));
    }
    for (int j = 0; j < 4; j++) {
        uint32_t v = X[35-j];
        out[4*j]   = (v>>24)&0xFF;
        out[4*j+1] = (v>>16)&0xFF;
        out[4*j+2] = (v>>8 )&0xFF;
        out[4*j+3] = v&0xFF;
    }
}

// 单块解密
void SM4_DecryptBlock(const uint8_t MK[16], const uint8_t in[16], uint8_t out[16]) {
    uint32_t X[36], tmp, buf, rk[32];
    SM4_KeySchedule(MK, rk);
    for (int j = 0; j < 4; j++) {
        X[j] = (uint32_t)in[4*j]<<24 | (uint32_t)in[4*j+1]<<16 |
               (uint32_t)in[4*j+2]<<8 | (uint32_t)in[4*j+3];
    }
    for (int i = 0; i < 32; i++) {
        tmp = X[i+1] ^ X[i+2] ^ X[i+3] ^ rk[31-i];
        buf = (uint32_t)SM4_Sbox[(tmp>>24)&0xFF] <<24 |
              (uint32_t)SM4_Sbox[(tmp>>16)&0xFF] <<16 |
              (uint32_t)SM4_Sbox[(tmp>>8 )&0xFF] <<8  |
              (uint32_t)SM4_Sbox[tmp&0xFF];
        X[i+4] = X[i] ^ (buf ^ SM4_Rotl32(buf,2) ^ SM4_Rotl32(buf,10)
                         ^ SM4_Rotl32(buf,18) ^ SM4_Rotl32(buf,24));
    }
    for (int j = 0; j < 4; j++) {
        uint32_t v = X[35-j];
        out[4*j]   = (v>>24)&0xFF;
        out[4*j+1] = (v>>16)&0xFF;
        out[4*j+2] = (v>>8 )&0xFF;
        out[4*j+3] = v&0xFF;
    }
}

// CBC 模式加密
void SM4_CBC_Encrypt(const uint8_t MK[16], const uint8_t *pt, uint8_t *ct,
                     size_t len, const uint8_t iv[16]) {
    uint8_t prev[16], block[16];
    memcpy(prev, iv, 16);
    for (size_t i=0; i<len; i+=16) {
        memcpy(block, pt+i, 16);
        for (int j=0; j<16; j++) block[j] ^= prev[j];
        SM4_EncryptBlock(MK, block, ct+i);
        memcpy(prev, ct+i, 16);
    }
}

// CBC 模式解密
void SM4_CBC_Decrypt(const uint8_t MK[16], const uint8_t *ct, uint8_t *pt,
                     size_t len, const uint8_t iv[16]) {
    uint8_t prev[16], block[16];
    memcpy(prev, iv, 16);
    for (size_t i=0; i<len; i+=16) {
        SM4_DecryptBlock(MK, ct+i, block);
        for (int j=0; j<16; j++) pt[i+j] = block[j] ^ prev[j];
        memcpy(prev, ct+i, 16);
    }
}

int main() {
    // 示例主密钥与 IV
    uint8_t key[16] = {0x01,0x23,0x45,0x67, 0x89,0xAB,0xCD,0xEF,
                       0xFE,0xDC,0xBA,0x98, 0x76,0x54,0x32,0x10};
    uint8_t iv[16]  = {0};

    // 示例两块明文（32 字节）
    uint8_t pt[32] = {
        0x00,0x11,0x22,0x33, 0x44,0x55,0x66,0x77,
        0x88,0x99,0xAA,0xBB, 0xCC,0xDD,0xEE,0xFF,
        0xFF,0xEE,0xDD,0xCC, 0xBB,0xAA,0x99,0x88,
        0x77,0x66,0x55,0x44, 0x33,0x22,0x11,0x00
    };
    uint8_t ct[32], out[32];
    size_t len = sizeof(pt);

    SM4_CBC_Encrypt(key, pt, ct, len, iv);
    printf("CBC Ciphertext:\n");
    for (size_t i=0; i<len; i++) printf("%02X", ct[i]);
    printf("\n\n");

    SM4_CBC_Decrypt(key, ct, out, len, iv);
    printf("CBC Decrypted :\n");
    for (size_t i=0; i<len; i++) printf("%02X", out[i]);
    printf("\n");

    return 0;
}