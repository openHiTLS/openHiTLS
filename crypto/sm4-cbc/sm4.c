#include <stdint.h>
#include <stdio.h>
#include <string.h>

// SM4 算法核心代码
// S盒
static const uint8_t SBOX[256] = {
    
};

// 固定参数 FK 和 CK
static const uint32_t FK[4] = { 0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc };
static const uint32_t CK[32] = {
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
};

// T函数
static uint32_t T(uint32_t x) {
    uint8_t a[4];
    a[0] = SBOX[(x >> 24) & 0xFF];
    a[1] = SBOX[(x >> 16) & 0xFF];
    a[2] = SBOX[(x >> 8) & 0xFF];
    a[3] = SBOX[x & 0xFF];
    uint32_t result = ((uint32_t)a[0] << 24) | ((uint32_t)a[1] << 16) | ((uint32_t)a[2] << 8) | a[3];
    return result ^ (result << 2) ^ (result << 10) ^ (result << 18) ^ (result << 24);
}

// 密钥扩展
void sm4_key_schedule(const uint8_t *key, uint32_t *rk) {
    uint32_t K[4];
    for (int i = 0; i < 4; i++) {
        K[i] = ((uint32_t)key[4 * i] << 24) | ((uint32_t)key[4 * i + 1] << 16) |
               ((uint32_t)key[4 * i + 2] << 8) | key[4 * i + 3];
        K[i] ^= FK[i];
    }
    for (int i = 0; i < 32; i++) {
        K[i % 4] ^= T(K[(i + 1) % 4] ^ K[(i + 2) % 4] ^ K[(i + 3) % 4] ^ CK[i]);
        rk[i] = K[i % 4];
    }
}

// 加密/解密
void sm4_crypt(const uint8_t *input, uint8_t *output, const uint32_t *rk, int encrypt) {
    uint32_t X[4];
    for (int i = 0; i < 4; i++) {
        X[i] = ((uint32_t)input[4 * i] << 24) | ((uint32_t)input[4 * i + 1] << 16) |
               ((uint32_t)input[4 * i + 2] << 8) | input[4 * i + 3];
    }
    for (int i = 0; i < 32; i++) {
        uint32_t temp = X[0] ^ T(X[1] ^ X[2] ^ X[3] ^ rk[encrypt ? i : (31 - i)]);
        X[0] = X[1];
        X[1] = X[2];
        X[2] = X[3];
        X[3] = temp;
    }
    for (int i = 0; i < 4; i++) {
        output[4 * i] = (X[3 - i] >> 24) & 0xFF;
        output[4 * i + 1] = (X[3 - i] >> 16) & 0xFF;
        output[4 * i + 2] = (X[3 - i] >> 8) & 0xFF;
        output[4 * i + 3] = X[3 - i] & 0xFF;
    }
}

// 测试主函数
int main() {
    uint8_t key[16] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef,
                       0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10};
    uint8_t plaintext[16] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xab, 0xcd, 0xef,
                             0xfe, 0xdc, 0xba, 0x98, 0x76, 0x54, 0x32, 0x10};
    uint8_t ciphertext[16];
    uint8_t decrypted[16];
    uint32_t round_keys[32];

    sm4_key_schedule(key, round_keys);
    sm4_crypt(plaintext, ciphertext, round_keys, 1);
    sm4_crypt(ciphertext, decrypted, round_keys, 0);

    printf("Plaintext: ");
    for (int i = 0; i < 16; i++) {
        printf("%02x ", plaintext[i]);
    }
    printf("\n");

    printf("Ciphertext: ");
    for (int i = 0; i < 16; i++) {
        printf("%02x ", ciphertext[i]);
    }
    printf("\n");

    printf("Decrypted: ");
    for (int i = 0; i < 16; i++) {
        printf("%02x ", decrypted[i]);
    }
    printf("\n");

    return 0;
}