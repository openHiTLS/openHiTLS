    ...
    uint8_t *sign, uint32_t *signLen)
{
    ...
    ret = CRYPT_EAL_EncodeSignBN(r, s, sign, signLen);
    BN_Destroy(r);
    BN_Destroy(s);
    return ret;
}
...
static int32_t VrifyCheckSign(const CRYPT_ECDSA_Ctx *ctx, BN_BigNum *r, BN_BigNum *s)
{
...
static int32_t EcdsaVerifyCore(const CRYPT_ECDSA_Ctx *ctx, BN_BigNum *d, BN_BigNum *r, BN_BigNum *s)
{
    ...
    const uint8_t *sign, uint32_t signLen)
{
    ...
    GOTO_ERR_IF(CRYPT_EAL_DecodeSignBN(sign, signLen, r, s), ret);

    GOTO_ERR_IF(VrifyCheckSign(ctx, r, s), ret);
