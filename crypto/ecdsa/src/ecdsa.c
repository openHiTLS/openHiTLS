...
static void SetCurveId(BSL_Param *params, CRYPT_PKEY_ParaId curveId)
{
    params->key = CRYPT_PARAM_EC_CURVE_ID;
    params->valueType = BSL_PARAM_TYPE_INT32;
    params->value = &curveId;
    params->valueLen = sizeof(int32_t);
    params->useLen = sizeof(int32_t);
}

static BSL_Param *CallocEcdsaParams(CRYPT_ECDSA_Ctx *ctx, int32_t type)
{
    ...
    if (type == CRYPT_KEYMGMT_SELECT_UNKNOWN) {
        ...
        if (curveId != CRYPT_PKEY_PARAID_MAX) {
            SetCurveId(&params[index], curveId);
            index++;
        }
...
int32_t CRYPT_ECDSA_Export(CRYPT_ECDSA_Ctx *ctx, int32_t flag, int32_t type, CRYPT_EAL_ProcessFuncCb cb, void *args)
{
    ...
    if ((flag & CRYPT_PKEY_FLAG_DUP) != 0) {
        params = CallocEcdsaParams(ctx, type);
        if (params == NULL) {
            BSL_ERR_PUSH_ERROR(CRYPT_MEM_ALLOC_FAIL);
            return CRYPT_MEM_ALLOC_FAIL;
        }
