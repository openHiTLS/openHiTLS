...
typedef struct {
    ...
    uint32_t xLen;  // Must be 32bytes.
    uint8_t *y;     // YCoordinate
    uint32_t yLen;  // Must be 32bytes.
    uint8_t *hash;    // HASH
    uint32_t hashLen; // Must be 32bytes.
    uint8_t *cipher;    // CipherText
    uint32_t cipherLen; // Same length as plaintxet.
 ...
 * @param encodeLen [out] The length of the SM2 encoded data.
 * @return: CRYPT_SUCCESS: Success, other: Error.
 */
