    ...
    BSL_Buffer *encode, const BSL_Buffer *pwd, CRYPT_EAL_PkeyCtx **ealPriKey)
{
    switch (type) {
        ...
        case CRYPT_PRIKEY_RSA:
            return ParseRsaPrikeyAsn1Buff(libctx, attrName, encode->data, encode->dataLen, NULL, BSL_CID_UNKNOWN, ealPriKey);
    ...
    const uint8_t *pwd, uint32_t pwdlen, CRYPT_EAL_PkeyCtx **ealPKey)
{
    ...
    return CRYPT_EAL_DecodeBuffKey_Ex(NULL, NULL, format, type, encode, &pwdBuffer, ealPKey);
}
    ...
    uint8_t *pwd, uint32_t pwdlen, CRYPT_EAL_PkeyCtx **ealPKey)
{
    ...
    return CRYPT_EAL_DecodeFileKey_Ex(NULL, NULL, format, type,
