...
static int32_t DecodeSignSequence(const uint8_t *encode, uint32_t encodeLen, BSL_ASN1_Buffer *asnArr,
    uint32_t asnArrLen)
{
    BSL_ASN1_Template templ = {g_signTempl, sizeof(g_signTempl) / sizeof(g_signTempl[0])};
...
int32_t CRYPT_EAL_DecodeSignBN(const uint8_t *encode, uint32_t encodeLen, BN_BigNum *r, BN_BigNum *s)
{
    ...
    int32_t ret = DecodeSignSequence(encode, encodeLen, asnArr, 2);  // 2: r and s
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }
