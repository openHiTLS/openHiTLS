...
int32_t CRYPT_EAL_GetSignEncodeLen(uint32_t rLen, uint32_t sLen, uint32_t *maxLen)
{
    ...
    int32_t ret = BSL_ASN1_GetEncodeLenByContentLen(rLen + 1, &rEncodeLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ret = BSL_ASN1_GetEncodeLenByContentLen(sLen + 1, &sEncodeLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ...
    ret = BSL_ASN1_GetEncodeLenByContentLen(rEncodeLen + sEncodeLen, maxLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
    }
    return ret;
...
static int32_t EncodeSignSequence(uint8_t *rBuf, uint32_t rLen, uint8_t *sBuf, uint32_t sLen,
    uint8_t *encode, uint32_t *encodeLen)
{
    ...
    uint8_t *outBuf = NULL;
    uint32_t outLen = 0;
    int32_t ret = BSL_ASN1_EncodeTemplate(&templ, asnArr, 2, &outBuf, &outLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
...
int32_t CRYPT_EAL_EncodeSignBN(const BN_BigNum *r, const BN_BigNum *s, uint8_t *encode, uint32_t *encodeLen)
{
    ...
    ret = EncodeSignSequence(rBuf, rLen, sBuf, sLen, encode, encodeLen);
...
static int32_t DecodeSignSequence(const uint8_t *encode, uint32_t encodeLen,
    const uint8_t **rBuf, uint32_t *rLen, const uint8_t **sBuf, uint32_t *sLen)
{
    ...
    int32_t ret = BSL_ASN1_DecodeTemplate(&templ, NULL, &tmpEnc, &tmpEncLen, asnArr, 2);  // 2: r and s
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ...
    // Use the buffer pointers returned by decode template directly
    *rBuf = asnArr[0].buff;
    *rLen = asnArr[0].len;
    *sBuf = asnArr[1].buff;
    *sLen = asnArr[1].len;
...
int32_t CRYPT_EAL_DecodeSignBN(const uint8_t *encode, uint32_t encodeLen, BN_BigNum *r, BN_BigNum *s)
{
    if (encode == NULL || r == NULL || s == NULL || encodeLen == 0) {
        BSL_ERR_PUSH_ERROR(CRYPT_NULL_INPUT);
        return CRYPT_NULL_INPUT;
    }

    const uint8_t *rBuf = NULL;
    const uint8_t *sBuf = NULL;
    uint32_t rLen = 0;
    uint32_t sLen = 0;

    // Decode ASN.1 sequence to get r and s components
    int32_t ret = DecodeSignSequence(encode, encodeLen, &rBuf, &rLen, &sBuf, &sLen);
    if (ret != CRYPT_SUCCESS) {
        return ret;
    }

    // Convert decoded buffers to big numbers
    ret = BN_Bin2Bn(r, rBuf, rLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }

    ret = BN_Bin2Bn(s, sBuf, sLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
    }
    ...
    uint32_t *encodeLen)
{
    ...
    /* + 1: if high bit is 1, should add a leading 0x00 */
    int32_t ret = BSL_ASN1_GetEncodeLenByContentLen(xLen + 1, &xEncodeLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }

    ret = BSL_ASN1_GetEncodeLenByContentLen(yLen + 1, &yEncodeLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }

    ret = BSL_ASN1_GetEncodeLenByContentLen(hashLen, &hashEncodeLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }

    ret = BSL_ASN1_GetEncodeLenByContentLen(dataLen, &cipherEncodeLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ...
    ret = BSL_ASN1_GetEncodeLenByContentLen(xEncodeLen + yEncodeLen + hashEncodeLen + cipherEncodeLen, encodeLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
    }
    return ret;
...
int32_t CRYPT_EAL_EncodeSm2EncryptData(const CRYPT_SM2_EncryptData *data, uint8_t *encode, uint32_t *encodeLen)
{
    ...
    if (data == NULL || encode == NULL || encodeLen == NULL) {
        BSL_ERR_PUSH_ERROR(CRYPT_NULL_INPUT);
        return CRYPT_NULL_INPUT;
    }
    ...
    uint8_t *outBuf = NULL;
    uint32_t outLen = 0;
    ret = BSL_ASN1_EncodeTemplate(&templ, asnArr, SM2_ENCRYPT_DATA_ITEM_NUM, &outBuf, &outLen);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
...
int32_t CRYPT_EAL_DecodeSm2EncryptData(const uint8_t *encode, uint32_t encodeLen, CRYPT_SM2_EncryptData *data)
{
    ...
    ret = BSL_ASN1_DecodeTemplate(&templ, NULL, &tmpEnc, &tmpEncLen, asnArr, SM2_ENCRYPT_DATA_ITEM_NUM);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
