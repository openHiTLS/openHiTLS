    ...
    const BSL_Buffer *pwd, CRYPT_EAL_PkeyCtx **ealPriKey)
{
    ...
    BSL_Buffer asn1 = {0};
    ret = BSL_PEM_ParsePemToAsn1((char **)&(encode->data), &(encode->dataLen), &symbol, &(asn1.data),
        &(asn1.dataLen));
    if (ret != BSL_SUCCESS) {
    ...
    CRYPT_EAL_PkeyCtx **ealPubKey)
{
    ...
    BSL_Buffer asn1 = {0};
    ret = BSL_PEM_ParsePemToAsn1((char **)&(encode->data), &(encode->dataLen), &symbol, &(asn1.data), &(asn1.dataLen));
    if (ret != BSL_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ...
    const CRYPT_EncodeParam *encodeParam, int32_t type, BSL_Buffer *encode)
{
    ...
    ret = BSL_PEM_GenAsn1ToPem(asn1.data, asn1.dataLen, &symbol, (char **)&encode->data, &encode->dataLen);
    BSL_SAL_Free(asn1.data);
    if (ret != BSL_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
    }
