...
static int32_t CryptSignParams(CRYPT_EAL_PkeyCtx *pubKey, CRYPT_EAL_PkeyCtx *prvKey, CRYPT_MD_AlgId *hashId)
{
    ...
    switch (algId) {
        ...
        case CRYPT_PKEY_RSA:
            *hashId = GetSupportedHashId();
            return CryptRsaEmsaPairSet(pubKey, prvKey, *hashId);
#endif
#ifdef HITLS_CRYPTO_SM2
        case CRYPT_PKEY_SM2:
            *hashId = CRYPT_MD_SM3;
            return CryptSm2PairSet(pubKey, prvKey);
#endif
        default:
...
int32_t CRYPT_EAL_PkeyPairCheck(CRYPT_EAL_PkeyCtx *pubKey, CRYPT_EAL_PkeyCtx *prvKey)
{
    ...
    ret = CryptSignParams(tempPubKey, tempPrivKey, &hashId);
    if (ret != CRYPT_SUCCESS) {
        goto EXIT;
    }
    uint8_t toBeSig[] = {1};
