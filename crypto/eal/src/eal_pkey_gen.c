...
int32_t CRYPT_EAL_PkeySetPub(CRYPT_EAL_PkeyCtx *pkey, const CRYPT_EAL_PkeyPub *key)
{
    ...
    switch(key->id) {
        ...
        case CRYPT_PKEY_X25519:
            BSL_Param para[2] = {{CRYPT_PARAM_CURVE25519_PUBKEY, BSL_PARAM_TYPE_OCTETS, key->key.curve25519Pub.data, key->key.curve25519Pub.len, 0},
                 BSL_PARAM_END};
...
int32_t CRYPT_EAL_PkeySetPrv(CRYPT_EAL_PkeyCtx *pkey, const CRYPT_EAL_PkeyPrv *key)
{
    ...
    switch(key->id) {
        ...
        case CRYPT_PKEY_SM2:
            ...
            break;
        case CRYPT_PKEY_PAILLIER:
            BSL_Param paParam[5] = {{CRYPT_PARAM_PAILLIER_N, BSL_PARAM_TYPE_OCTETS, key->key.paillierPrv.n, key->key.paillierPrv.nLen, 0},
                {CRYPT_PARAM_PAILLIER_LAMBDA, BSL_PARAM_TYPE_OCTETS, key->key.paillierPrv.lambda, key->key.paillierPrv.lambdaLen, 0},
