...
{
    BSL_Param *temp = (BSL_Param *)BSL_PARAM_FindConstParam(param, CRYPT_PARAM_DH_P);
    if (temp == NULL) {
        BSL_ERR_PUSH_ERROR(CRYPT_DH_PARA_ERROR);
        return CRYPT_DH_PARA_ERROR;
    }
...
{
    BSL_Param *temp = (BSL_Param *)BSL_PARAM_FindConstParam(param, CRYPT_PARAM_DH_Q);
    if (temp == NULL) {
        return CRYPT_SUCCESS;
    }
...
{
    BSL_Param *temp = (BSL_Param *)BSL_PARAM_FindConstParam(param, CRYPT_PARAM_DH_G);
    if (temp == NULL) {
        BSL_ERR_PUSH_ERROR(CRYPT_DH_PARA_ERROR);
        return CRYPT_DH_PARA_ERROR;
    }
...
int32_t CRYPT_DH_GetPrvKey(const CRYPT_DH_Ctx *ctx, BSL_Param *para)
{
    ...
    BSL_Param *prv = (BSL_Param *)BSL_PARAM_FindParam(para, CRYPT_PARAM_DH_PRVKEY);
...
int32_t CRYPT_DH_GetPubKey(const CRYPT_DH_Ctx *ctx, BSL_Param *para)
{
    ...
    BSL_Param *pub =  (BSL_Param *)BSL_PARAM_FindParam(para, CRYPT_PARAM_DH_PUBKEY);
