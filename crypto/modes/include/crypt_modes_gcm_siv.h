...
#ifndef CRYPT_MODES_GCM_SIV_H
#define CRYPT_MODES_GCM_SIV_H
...
typedef struct ModesGcmSivCtx MODES_GCM_SIV_Ctx;

MODES_GCM_SIV_Ctx *MODES_GCM_SIV_NewCtx(int32_t algId);
MODES_GCM_SIV_Ctx *MODES_GCM_SIV_NewCtxEx(void *libCtx, int32_t algId);

int32_t MODES_GCM_SIV_InitCtxEx(MODES_GCM_SIV_Ctx *modeCtx, const uint8_t *key, uint32_t keyLen, const uint8_t *iv,
    uint32_t ivLen, void *param, bool enc);

int32_t MODES_GCM_SIV_Update(MODES_GCM_SIV_Ctx *modeCtx, const uint8_t *in, uint32_t inLen, uint8_t *out,
    uint32_t *outLen);
