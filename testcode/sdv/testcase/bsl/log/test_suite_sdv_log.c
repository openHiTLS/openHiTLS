...
void SDV_BSL_LOG_VERSION_API_TC001(void)
{
    ...
    ASSERT_TRUE(versionLen == strlen("openHiTLS 0.3.4 21 Jul 2026"));
    ASSERT_TRUE(memcmp(version, "openHiTLS 0.3.4 21 Jul 2026", versionLen) == 0);
    uint64_t versionNum = BSL_LOG_GetVersionNum();
    ASSERT_EQ(versionNum, 0x0030004fULL);
EXIT:
    return;
}
/* END_CASE */
