 ...
 * @title  Test BSL_ASN1_GetEncodeLenByContentLen function
 */
/* BEGIN_CASE */
void SDV_BSL_ASN1_GET_ENCODE_LEN_FUNC_TC001(int contentLen, int expectLen, int ret)
{
    uint32_t encodeLen = 0;
    ASSERT_EQ(BSL_ASN1_GetEncodeLenByContentLen(contentLen, &encodeLen), ret);
    if (ret == BSL_SUCCESS) {
 ...
 * @title  Test BSL_ASN1_GetEncodeLenByContentLen abnormal input parameter
 */
/* BEGIN_CASE */
void SDV_BSL_ASN1_GET_ENCODE_LEN_API_TC001(void)
{
    uint32_t encodeLen = 0;
    // Test null pointer
    ASSERT_EQ(BSL_ASN1_GetEncodeLenByContentLen(1, NULL), BSL_NULL_INPUT);

    // Test length overflow
    ASSERT_EQ(BSL_ASN1_GetEncodeLenByContentLen(UINT32_MAX, &encodeLen), BSL_ASN1_ERR_LEN_OVERFFLOW);
EXIT:
    return;
}
/* END_CASE */
