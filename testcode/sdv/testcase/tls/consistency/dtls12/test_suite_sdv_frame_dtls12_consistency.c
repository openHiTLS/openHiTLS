...
static const uint8_t g_dtls12AppCookie[] = {0x64, 0x74, 0x6c, 0x73, 0x31, 0x32, 0x63, 0x62};
static uint32_t g_dtls12AppCookieGenCnt = 0;
static uint32_t g_dtls12AppCookieVerifyCnt = 0;

static void ResetDtls12AppCookieCnt(void)
{
    g_dtls12AppCookieGenCnt = 0;
    g_dtls12AppCookieVerifyCnt = 0;
}

static int32_t UT_Dtls12AppCookieGenerateCb(HITLS_Ctx *ctx, uint8_t *cookie, uint32_t *cookieLen)
{
    (void)ctx;
    if (cookie == NULL || cookieLen == NULL || *cookieLen < sizeof(g_dtls12AppCookie)) {
        return HITLS_COOKIE_GENERATE_ERROR;
    }
    memcpy(cookie, g_dtls12AppCookie, sizeof(g_dtls12AppCookie));
    *cookieLen = sizeof(g_dtls12AppCookie);
    g_dtls12AppCookieGenCnt++;
    return HITLS_COOKIE_GENERATE_SUCCESS;
}

static int32_t UT_Dtls12AppCookieVerifyCb(HITLS_Ctx *ctx, const uint8_t *cookie, uint32_t cookieLen)
{
    (void)ctx;
    g_dtls12AppCookieVerifyCnt++;
    if (cookieLen == sizeof(g_dtls12AppCookie) &&
        memcmp(cookie, g_dtls12AppCookie, sizeof(g_dtls12AppCookie)) == 0) {
...
/* @
* @test UT_TLS_DTLS_CONSISTENCY_RFC6347_HELLO_VERIFY_REQ_TC006
* @spec -
* @title DTLS1.2 app cookie callbacks generate and verify exact cookie bytes.
* @precon nan
* @brief
* 1. Configure DTLS1.2 cookie exchange with appGenCookieCb and appVerifyCookieCb. Expected result 1.
* 2. Invoke HS_CalcCookie and HS_CheckCookie with the generated cookie. Expected result 2.
* 3. Tamper with the cookie and invoke HS_CheckCookie again. Expected result 3.
* @expect
* 1. The callbacks are configured successfully.
* 2. The generated cookie bytes match the callback output and verification succeeds.
* 3. The verification callback rejects the tampered cookie.
* @prior Level 1
* @auto TRUE
@ */
/* BEGIN_CASE */
void UT_TLS_DTLS_CONSISTENCY_RFC6347_HELLO_VERIFY_REQ_TC006(void)
{
    FRAME_Init();
    ResetDtls12AppCookieCnt();
    HITLS_Config *tlsConfig = HITLS_CFG_NewDTLS12Config();
    ASSERT_TRUE(tlsConfig != NULL);
    ASSERT_EQ(HITLS_CFG_SetDtlsCookieExchangeSupport(tlsConfig, true), HITLS_SUCCESS);
    ASSERT_EQ(HITLS_CFG_SetCookieGenCb(tlsConfig, UT_Dtls12AppCookieGenerateCb), HITLS_SUCCESS);
    ASSERT_EQ(HITLS_CFG_SetCookieVerifyCb(tlsConfig, UT_Dtls12AppCookieVerifyCb), HITLS_SUCCESS);
    FRAME_LinkObj *server = FRAME_CreateLink(tlsConfig, BSL_UIO_UDP);
    ASSERT_TRUE(server != NULL);
    HITLS_Ctx *serverTlsCtx = FRAME_GetTlsCtx(server);
    ASSERT_TRUE(serverTlsCtx != NULL);
    ...
    ClientHelloMsg clientHello = {0};
    uint8_t cookie[DTLS_COOKIE_LEN] = {0};
    uint32_t cookieLen = sizeof(cookie);
    bool isCookieValid = false;

    ASSERT_EQ(HS_CalcCookie(serverTlsCtx, &clientHello, cookie, &cookieLen), HITLS_SUCCESS);
    ASSERT_EQ(g_dtls12AppCookieGenCnt, 1);
    ASSERT_EQ(cookieLen, sizeof(g_dtls12AppCookie));
    ASSERT_EQ(memcmp(cookie, g_dtls12AppCookie, sizeof(g_dtls12AppCookie)), 0);

    clientHello.cookie = cookie;
    clientHello.cookieLen = cookieLen;
    ASSERT_EQ(HS_CheckCookie(serverTlsCtx, &clientHello, &isCookieValid), HITLS_SUCCESS);
    ASSERT_TRUE(isCookieValid);
    ASSERT_EQ(g_dtls12AppCookieVerifyCnt, 1);

    cookie[0] ^= 0x01;
    isCookieValid = true;
    ASSERT_EQ(HS_CheckCookie(serverTlsCtx, &clientHello, &isCookieValid), HITLS_SUCCESS);
    ASSERT_TRUE(!isCookieValid);
    ASSERT_EQ(g_dtls12AppCookieVerifyCnt, 2);
...
/* @
* @test UT_TLS_DTLS_CONSISTENCY_RFC6347_HELLO_VERIFY_REQ_TC007
* @spec -
* @title DTLS1.2 default cookie verification accepts cookie generated by previous MAC key.
* @precon nan
* @brief
* 1. Generate a default DTLS1.2 cookie. Expected result 1.
* 2. Simulate MAC key rotation by moving the generating key to preMacKey. Expected result 2.
* 3. Verify the generated cookie. Expected result 3.
* @expect
* 1. Cookie generation succeeds.
* 2. The current MAC key differs from the previous MAC key.
* 3. Cookie verification succeeds through preMacKey.
* @prior Level 1
* @auto TRUE
@ */
/* BEGIN_CASE */
void UT_TLS_DTLS_CONSISTENCY_RFC6347_HELLO_VERIFY_REQ_TC007(void)
{
    FRAME_Init();
    HITLS_Config *tlsConfig = HITLS_CFG_NewDTLS12Config();
    ASSERT_TRUE(tlsConfig != NULL);
    ...
    ASSERT_EQ(HS_CalcCookie(serverTlsCtx, &clientHello, cookie, &cookieLen), HITLS_SUCCESS);
    ASSERT_TRUE(cookieLen != 0);
    CookieInfo *cookieInfo = &serverTlsCtx->negotiatedInfo.cookieInfo;
    ASSERT_TRUE(memcmp(cookieInfo->macKey, zeroMacKey, MAC_KEY_LEN) != 0);
...
/* @
* @test UT_DTLS_RFC6347_RECV_ALERT_AFTER_CCS_TC001
...
* @precon nan
* @brief
* 1. Create a DTLS 1.2 client and server and park the client before it receives the server CCS. Expected result 1.
...
/* @
* @test UT_TLS_DTLS_CONSISTENCY_REASS_CLEAR_TC001
* @spec -
* @title HS_DeInit should clear stale DTLS 1.2 reassembly messages.
* @precon nan
* @brief
* 1. Create a DTLS 1.2 link and initialize the handshake context. Expected result 1.
* 2. Append a complete but future-sequence handshake message to the reassembly queue. Expected result 2.
* 3. Deinitialize the handshake context. Expected result 3.
* @expect
* 1. The initialization is successful.
* 2. The reassembly queue is not empty.
* 3. The reassembly queue remains allocated on the connection but has no stale node.
* @prior Level 1
* @auto TRUE
@ */
/* BEGIN_CASE */
void UT_TLS_DTLS_CONSISTENCY_REASS_CLEAR_TC001(void)
{
    FRAME_Init();
    HITLS_Config *tlsConfig = HITLS_CFG_NewDTLS12Config();
    FRAME_LinkObj *server = NULL;
    HITLS_Ctx *ctx = NULL;
    uint8_t rawMsg[DTLS_HS_MSG_HEADER_SIZE + 1u] = {0};
    HS_MsgInfo msgInfo = {0};

    ASSERT_TRUE(tlsConfig != NULL);
    server = FRAME_CreateLink(tlsConfig, BSL_UIO_UDP);
    ASSERT_TRUE(server != NULL);
    ctx = FRAME_GetTlsCtx(server);
    ASSERT_TRUE(ctx != NULL);
    ctx->negotiatedInfo.version = HITLS_VERSION_DTLS12;
    ASSERT_TRUE(ctx->reassMsg != NULL);
    ASSERT_EQ(HS_Init(ctx), HITLS_SUCCESS);
    ASSERT_TRUE(ctx->hsCtx != NULL);
    ASSERT_TRUE(HS_ReassQueueIsEmpty(ctx));

    msgInfo.type = CERTIFICATE;
    msgInfo.length = 1u;
    msgInfo.sequence = ctx->hsCtx->expectRecvSeq + 1u;
    msgInfo.fragmentOffset = 0u;
    msgInfo.fragmentLength = 1u;
    msgInfo.rawMsg = rawMsg;
    msgInfo.headerAndBodyLen = sizeof(rawMsg);
    rawMsg[0] = (uint8_t)msgInfo.type;
    BSL_Uint24ToByte(msgInfo.length, &rawMsg[DTLS_HS_MSGLEN_ADDR]);
    BSL_Uint16ToByte(msgInfo.sequence, &rawMsg[DTLS_HS_MSGSEQ_ADDR]);
    BSL_Uint24ToByte(msgInfo.fragmentOffset, &rawMsg[DTLS_HS_FRAGMENT_OFFSET_ADDR]);
    BSL_Uint24ToByte(msgInfo.fragmentLength, &rawMsg[DTLS_HS_FRAGMENT_LEN_ADDR]);
    rawMsg[DTLS_HS_MSG_HEADER_SIZE] = 0xAAu;

    ASSERT_EQ(HS_ReassAppend(ctx, &msgInfo), HITLS_SUCCESS);
    ASSERT_TRUE(!HS_ReassQueueIsEmpty(ctx));
...
/* @
* @test UT_TLS_DTLS_CONSISTENCY_CLEAR_TIMER_TC001
* @spec -
* @title HITLS_Clear should clear stale DTLS timer state.
* @precon nan
* @brief
* 1. Create a DTLS 1.2 link and set retransmission and 2MSL timer state on the context. Expected result 1.
* 2. Call HITLS_Clear. Expected result 2.
* 3. Query DTLS timeout state. Expected result 3.
* @expect
* 1. The initialization is successful.
* 2. The retransmission timer and 2MSL timer state are cleared.
* 3. The cleared context has no pending DTLS timeout action.
* @prior Level 1
* @auto TRUE
@ */
/* BEGIN_CASE */
void UT_TLS_DTLS_CONSISTENCY_CLEAR_TIMER_TC001(void)
{
    FRAME_Init();
    HITLS_Config *tlsConfig = HITLS_CFG_NewDTLS12Config();
    FRAME_LinkObj *server = NULL;
    HITLS_Ctx *ctx = NULL;
    uint64_t remainTimeOut = 1u;
    ...
    ctx->timeoutValue = 1000u;
    ctx->timeoutNum = 3u;
    ctx->deadline = (BSL_TIME){BSL_TIME_SYSTEM_EPOCH_YEAR, 1, 1, 0, 0, 0, 0, 1};
    ctx->dtls2MslDeadline = (BSL_TIME){BSL_TIME_SYSTEM_EPOCH_YEAR, 1, 1, 0, 0, 0, 0, 1};
    ctx->isDtls2MslTimerActive = true;

    ASSERT_EQ(HITLS_Clear(ctx), HITLS_SUCCESS);
    ASSERT_EQ(ctx->timeoutValue, 0u);
    ASSERT_EQ(ctx->timeoutNum, 0u);
    ASSERT_EQ(ctx->deadline.year, 0u);
    ASSERT_EQ(ctx->deadline.month, 0u);
    ASSERT_EQ(ctx->deadline.day, 0u);
    ASSERT_EQ(ctx->deadline.hour, 0u);
    ASSERT_EQ(ctx->deadline.minute, 0u);
    ASSERT_EQ(ctx->deadline.second, 0u);
    ASSERT_EQ(ctx->deadline.millSec, 0u);
    ASSERT_EQ(ctx->deadline.microSec, 0u);
    ASSERT_EQ(ctx->isDtls2MslTimerActive, false);
    ASSERT_EQ(ctx->dtls2MslDeadline.year, 0u);
    ASSERT_EQ(ctx->dtls2MslDeadline.month, 0u);
    ASSERT_EQ(ctx->dtls2MslDeadline.day, 0u);
    ASSERT_EQ(ctx->dtls2MslDeadline.hour, 0u);
    ASSERT_EQ(ctx->dtls2MslDeadline.minute, 0u);
    ASSERT_EQ(ctx->dtls2MslDeadline.second, 0u);
    ASSERT_EQ(ctx->dtls2MslDeadline.millSec, 0u);
    ASSERT_EQ(ctx->dtls2MslDeadline.microSec, 0u);
    ASSERT_EQ(HITLS_DtlsGetTimeout(ctx, &remainTimeOut), HITLS_MSG_HANDLE_ERR_WITHOUT_TIMEOUT_ACTION);
    ASSERT_EQ(remainTimeOut, 0u);
