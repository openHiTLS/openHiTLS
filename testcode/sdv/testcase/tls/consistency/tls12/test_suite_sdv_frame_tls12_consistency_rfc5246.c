...
* status then changed to alerting, call close to break the chain, and then call hitls_read to read the plaintext app message.
* It is expected that the read message failed.
* @precon  nan
* @brief  1. Use the default configuration items to configure the client and server. Expected result 1 is obtained.
