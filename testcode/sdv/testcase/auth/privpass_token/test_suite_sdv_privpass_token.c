...
void SDV_AUTH_PRIVPASS_TOKEN_SERIALIZATION_TC001(int type, Hex *buffer)
{
    ...
    outputLen--;
    ASSERT_EQ(HITLS_Auth_PrivPassSerialization(ctx, challenge, output, &outputLen), HITLS_AUTH_PRIVPASS_BUFFER_NOT_ENOUGH);
    outputLen++;
...
void SDV_AUTH_PRIVPASS_TOKEN_SERIALIZATION_INVALID_TC003(int type, Hex *buffer)
{
    ...
    if (type == HITLS_AUTH_PRIVPASS_TOKEN_REQUEST) {
        ...
        ASSERT_EQ(HITLS_Auth_PrivPassSerialization(ctx, token, output, &outputLen), HITLS_AUTH_PRIVPASS_INVALID_TOKEN_REQUEST);
    }
    if (type == HITLS_AUTH_PRIVPASS_TOKEN_RESPONSE) {
        ...
        ASSERT_EQ(HITLS_Auth_PrivPassSerialization(ctx, token, output, &outputLen), HITLS_AUTH_PRIVPASS_INVALID_TOKEN_RESPONSE);
        token->st.tokenResponse->type = 1;
    }
    if (type == HITLS_AUTH_PRIVPASS_TOKEN_INSTANCE) {
        token->st.token->tokenType = 0; // not support prv type
        ASSERT_EQ(HITLS_Auth_PrivPassSerialization(ctx, token, output, &outputLen), HITLS_AUTH_PRIVPASS_INVALID_TOKEN_INSTANCE);
    }
exit:
    HITLS_Auth_PrivPassFreeToken(token);
    HITLS_Auth_PrivPassFreeCtx(ctx);
    ...
    Hex *originInfo)
{
    ...
    ctx = HITLS_Auth_PrivPassNewCtx(HITLS_AUTH_PRIV_PASS_PUB_VERIFY_TOKENS);
    ASSERT_NE(ctx, NULL);
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenChallenge(ctx, param1, &tokenChallenge), HITLS_AUTH_PRIVPASS_INVALID_TOKEN_TYPE);
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenChallenge(ctx, param3, &tokenChallenge), HITLS_AUTH_PRIVPASS_INVALID_ISSUER_NAME);
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenChallenge(ctx, param4, &tokenChallenge), HITLS_AUTH_PRIVPASS_INVALID_ORIGIN_INFO);
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenChallenge(ctx, param5, &tokenChallenge), HITLS_AUTH_PRIVPASS_INVALID_REDEMPTION);
exit:
    CRYPT_EAL_RandDeinit();
    HITLS_Auth_PrivPassFreeToken(tokenChallenge);
    HITLS_Auth_PrivPassFreeCtx(ctx);
    ...
    Hex *request, Hex *response, Hex *token)
{
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassTokenCtrl(finalToken, HITLS_AUTH_PRIVPASS_GET_TOKEN_NONCE, param, 0), HITLS_AUTH_SUCCESS);
...
void SDV_AUTH_PRIVPASS_TOKEN_CHALLENGE_OBTAIN_TC001(Hex *challenge)
{
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassTokenCtrl(tokenChallenge1, HITLS_AUTH_PRIVPASS_GET_TOKENCHALLENGE_TYPE, param, 0),
        HITLS_AUTH_SUCCESS);
    ASSERT_EQ(HITLS_Auth_PrivPassTokenCtrl(tokenChallenge1, HITLS_AUTH_PRIVPASS_GET_TOKENCHALLENGE_REDEMPTION, param, 0),
        HITLS_AUTH_SUCCESS);
    ASSERT_EQ(HITLS_Auth_PrivPassTokenCtrl(tokenChallenge1, HITLS_AUTH_PRIVPASS_GET_TOKENCHALLENGE_ORIGININFO, param, 0),
        HITLS_AUTH_SUCCESS);
    ASSERT_EQ(HITLS_Auth_PrivPassTokenCtrl(tokenChallenge1, HITLS_AUTH_PRIVPASS_GET_TOKENCHALLENGE_ISSUERNAME, param, 0),
        HITLS_AUTH_SUCCESS);
...
void SDV_AUTH_PRIVPASS_TEST_SET_CRYPTO_CB_TC001(Hex *ski, Hex *pki)
{
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenReq(ctx, tokenChallenge, &tokenRequest1), HITLS_AUTH_PRIVPASS_INVALID_TOKEN_TYPE);
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenResponse(ctx, tokenRequest, &tokenResponse1), HITLS_AUTH_PRIVPASS_INVALID_TOKEN_TYPE);
    ...
    pkeyCtx = NULL;
    ASSERT_EQ(HITLS_Auth_PrivPassSetCryptCb(ctx, HITLS_AUTH_PRIVPASS_INVALID_CRYPTO_CB_TYPE, NewPkeyCtxTmp),
        HITLS_AUTH_PRIVPASS_INVALID_CRYPTO_CALLBACK_TYPE);
    ASSERT_EQ(HITLS_Auth_PrivPassSetCryptCb(ctx, HITLS_AUTH_PRIVPASS_NEW_PKEY_CTX, NewPkeyCtxTmp), HITLS_AUTH_SUCCESS);
    pkeyCtx = ctx->method.newPkeyCtx(NULL, NULL, HITLS_AUTH_PRIVPASS_CRYPTO_RSA);
...
void SDV_AUTH_PRIVPASS_SET_KEY_TC002()
{
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassSetPrvkey(ctx, NULL, prvBuffer.data, prvBuffer.dataLen),
        HITLS_AUTH_SUCCESS);
    ASSERT_EQ(HITLS_Auth_PrivPassSetPubkey(ctx, pubBuffer.data, pubBuffer.dataLen), HITLS_AUTH_PRIVPASS_CHECK_KEYPAIR_FAILED);
exit:
...
void SDV_AUTH_PRIVPASS_TOKEN_INVALID_INTERACTION_TC001()
{
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenResponse(NULL, tokenRequest, &tokenResponse), HITLS_AUTH_PRIVPASS_INVALID_INPUT);
    ASSERT_EQ(HITLS_Auth_PrivPassGenToken(NULL, tokenChallenge, tokenResponse, &finalToken), HITLS_AUTH_PRIVPASS_INVALID_INPUT);
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenResponse(ctx, tokenRequest, &tokenResponse), HITLS_AUTH_PRIVPASS_INVALID_INPUT);
    ASSERT_EQ(HITLS_Auth_PrivPassGenToken(ctx, tokenChallenge, tokenResponse, &finalToken), HITLS_AUTH_PRIVPASS_INVALID_INPUT);
...
void SDV_AUTH_PRIVPASS_TOKEN_INVALID_INTERACTION_TC002(Hex *challenge, Hex *request, Hex *response, Hex *token)
{
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassGenToken(ctx, tokenResponse, tokenResponse, &finalToken), HITLS_AUTH_PRIVPASS_INVALID_INPUT);
    ASSERT_EQ(HITLS_Auth_PrivPassGenToken(ctx, tokenChallenge, tokenChallenge, &finalToken), HITLS_AUTH_PRIVPASS_INVALID_INPUT);
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenResponse(ctx, tokenRequest, &tokenResponse1), HITLS_AUTH_PRIVPASS_NO_PRVKEY_INFO);
    ASSERT_EQ(HITLS_Auth_PrivPassGenToken(ctx, tokenChallenge, tokenResponse, &finalToken1), HITLS_AUTH_PRIVPASS_NO_PUBKEY_INFO);
