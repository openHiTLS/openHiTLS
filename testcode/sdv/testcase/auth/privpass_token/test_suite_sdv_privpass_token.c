...
void SDV_AUTH_PRIVPASS_TOKEN_SERIALIZATION_TC001(int type, Hex *buffer)
{
    ...
    HITLS_Auth_PrivPassCtx *ctx = HITLS_Auth_PrivPassCtxNew(HITLS_AUTH_PRIV_PASS_PUB_VERIFY_TOKENS);
    ASSERT_NE(ctx, NULL);
...
void SDV_AUTH_PRIVPASS_TOKEN_SERIALIZATION_TC002(Hex *tokenType, Hex *issuerName, Hex *redemption, Hex *originInfo)
{
    ...
    TestRandInit();
    ctx = HITLS_Auth_PrivPassCtxNew(HITLS_AUTH_PRIV_PASS_PUB_VERIFY_TOKENS);
    ASSERT_NE(ctx, NULL);
...
exit:
    ...
    HITLS_Auth_PrivPassTokenFree(tokenChallenge3);
    HITLS_Auth_PrivPassTokenFree(tokenChallenge4);
    HITLS_Auth_PrivPassTokenFree(tokenChallenge3_1);
    HITLS_Auth_PrivPassTokenFree(tokenChallenge4_1);
    HITLS_Auth_PrivPassCtxFree(ctx);
}
/* END_CASE */
...
void SDV_AUTH_PRIVPASS_TOKEN_SERIALIZATION_INVALID_TC001()
{
   uint8_t output[MAX_LEN];
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassSerialization(NULL, token, output, &outputLen), HITLS_AUTH_INVALID_INPUT);
    ASSERT_EQ(HITLS_Auth_PrivPassSerialization(ctx, NULL, output, &outputLen), HITLS_AUTH_INVALID_INPUT);
    ASSERT_EQ(HITLS_Auth_PrivPassSerialization(ctx, token, NULL, &outputLen), HITLS_AUTH_INVALID_INPUT);
    ASSERT_EQ(HITLS_Auth_PrivPassSerialization(ctx, token, output, NULL), HITLS_AUTH_INVALID_INPUT);
exit:
    ...
    Hex *redemption, Hex *originInfo)
{
    ...
    TestRandInit();
    // Create context
    client = HITLS_Auth_PrivPassCtxNew(HITLS_AUTH_PRIV_PASS_PUB_VERIFY_TOKENS);
    ASSERT_NE(client, NULL);
    issuer = HITLS_Auth_PrivPassCtxNew(HITLS_AUTH_PRIV_PASS_PUB_VERIFY_TOKENS);
    ASSERT_NE(issuer, NULL);
    server = HITLS_Auth_PrivPassCtxNew(HITLS_AUTH_PRIV_PASS_PUB_VERIFY_TOKENS);
    ...
    Hex *originInfo)
{
    ...
    ctx = HITLS_Auth_PrivPassCtxNew(HITLS_AUTH_PRIV_PASS_PUB_VERIFY_TOKENS);
    ASSERT_NE(ctx, NULL);
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenChallenge(ctx, param1, &tokenChallenge), HITLS_AUTH_INVALID_TOKEN_TYPE);
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenChallenge(ctx, param2, &tokenChallenge),
        HITLS_AUTH_NO_TOKEN_CHALLENGE_ISSUERNAME);
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenChallenge(ctx, param3, &tokenChallenge), HITLS_AUTH_INVALID_ISSUER_NAME);
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenChallenge(ctx, param4, &tokenChallenge), HITLS_AUTH_INVALID_ORIGIN_INFO);
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenChallenge(ctx, param5, &tokenChallenge), HITLS_AUTH_INVALID_REDEMPTION);
exit:
    CRYPT_EAL_RandDeinit();
    HITLS_Auth_PrivPassTokenFree(tokenChallenge);
    HITLS_Auth_PrivPassCtxFree(ctx);
...
void SDV_AUTH_PRIVPASS_TEST_SET_CRYPTO_CB_TC001(Hex *ski, Hex *pki)
{
    ...
    pkeyCtx = ctx->method.newPkeyCtx(NULL, HITLS_AUTH_PRIVPASS_CRYPTO_RSA);
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassSetCryptCb(ctx, HITLS_AUTH_PRIVPASS_INVALID_CRYPTO_CB_TYPE, NewPkeyCtxTmp),
        HITLS_AUTH_INVALID_CRYPTO_CALLBACK_TYPE);
...
void SDV_AUTH_PRIVPASS_SET_KEY_TC001(Hex *ski, Hex *pki)
{
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassSetPrvkey(NULL, NULL, ski->x, ski->len), HITLS_AUTH_INVALID_INPUT);
    ASSERT_EQ(HITLS_Auth_PrivPassSetPrvkey(ctx, NULL, NULL, ski->len), HITLS_AUTH_INVALID_INPUT);
    // Test zero length
    ASSERT_EQ(HITLS_Auth_PrivPassSetPubkey(ctx, pki->x, 0), HITLS_AUTH_INVALID_INPUT);
    ASSERT_EQ(HITLS_Auth_PrivPassSetPrvkey(ctx, NULL, ski->x, 0), HITLS_AUTH_INVALID_INPUT);
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassSetPrvkey(ctx, NULL, ski->x, ski->len), HITLS_AUTH_NO_KEYPAIR_CHECK_CALLBACK);
