...
void SDV_AUTH_PRIVPASS_TOKEN_SERIALIZATION_TC002(Hex *tokenType, Hex *issuerName, Hex *redemption, Hex *originInfo)
{
    ...
    uint8_t output5[MAX_LEN >> 1];
    uint32_t outputLen5 = MAX_LEN >> 1;
    ...
    ASSERT_EQ(HITLS_Auth_PrivPassGenTokenChallenge(ctx, param5, &tokenChallenge5), HITLS_AUTH_SUCCESS);
    ASSERT_EQ(HITLS_Auth_PrivPassSerialization(ctx, tokenChallenge5, output5, &outputLen5), HITLS_AUTH_SUCCESS);
    ASSERT_EQ(HITLS_Auth_PrivPassDeserialization(ctx, HITLS_AUTH_PRIVPASS_TOKEN_CHALLENGE, output5, outputLen5,
        &tokenChallenge5_1), HITLS_AUTH_SUCCESS);
