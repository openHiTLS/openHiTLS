    ...
    Hex *redemption, Hex *originInfo)
{
    ...
    BSL_Param param[5] = {
        {CRYPT_PARAM_PRIV_PASS_TOKENTYPE, BSL_PARAM_TYPE_UINT16, &tokenTypeValue, 2, 2},
        {CRYPT_PARAM_PRIV_PASS_ISSUERNAME, BSL_PARAM_TYPE_OCTETS_PTR, issuerName->x, issuerName->len, issuerName->len},
        {CRYPT_PARAM_PRIV_PASS_REDEMPTION, BSL_PARAM_TYPE_OCTETS_PTR, redemption->x, redemption->len, redemption->len},
        {CRYPT_PARAM_PRIV_PASS_ORIGININFO, BSL_PARAM_TYPE_OCTETS_PTR, originInfo->x, originInfo->len, originInfo->len},
        BSL_PARAM_END
    };
...
void SDV_AUTH_PRIVPASS_TOKEN_CHALLENGE_OBTAIN_TC001(Hex *challenge)
{
    ...
    HiTLS_Auth_PrivPassCtx *ctx = NULL;
    BSL_Param param[5] = {
        {CRYPT_PARAM_PRIV_PASS_TOKENTYPE, BSL_PARAM_TYPE_UINT16, &tokenType, 2, 0},
        {CRYPT_PARAM_PRIV_PASS_ISSUERNAME, BSL_PARAM_TYPE_OCTETS_PTR, issuerNameBuffer, issuerNameBufferLen, 0},
        {CRYPT_PARAM_PRIV_PASS_REDEMPTION, BSL_PARAM_TYPE_OCTETS_PTR, redemptionBuffer, redemptionBufferLen, 0},
        {CRYPT_PARAM_PRIV_PASS_ORIGININFO, BSL_PARAM_TYPE_OCTETS_PTR, originInfoBuffer, originInfoBufferLen, 0},
        BSL_PARAM_END
    };
    // Create context
    ctx = HiTLS_Auth_PrivPassCtxNew();
    ...
    ASSERT_EQ(HiTLS_Auth_PrivPassCtrl(tokenChallenge1, HITLS_AUTH_PRIVPASS_GET_TOKENCHALLENGE_TYPE, param, 0), 0);
    ASSERT_EQ(HiTLS_Auth_PrivPassCtrl(tokenChallenge1, HITLS_AUTH_PRIVPASS_GET_TOKENCHALLENGE_REDEMPTION, param, 0), 0);
    ASSERT_EQ(HiTLS_Auth_PrivPassCtrl(tokenChallenge1, HITLS_AUTH_PRIVPASS_GET_TOKENCHALLENGE_ORIGININFO, param, 0), 0);
    ASSERT_EQ(HiTLS_Auth_PrivPassCtrl(tokenChallenge1, HITLS_AUTH_PRIVPASS_GET_TOKENCHALLENGE_ISSUERNAME, param, 0), 0);
    ASSERT_EQ(HiTLS_Auth_PrivPassGenTokenChallenge(ctx, param, &tokenChallenge2), 0);
    ASSERT_EQ(HiTLS_Auth_PrivPassSerialization(ctx, tokenChallenge2, tokenChallengeBuffer, &tokenChallengeBufferLen), 0);
