...
void SDV_CMS_PARSE_DIGESTINFO_TC001(Hex *buff, int alg, Hex *digest)
{
    ...
    int32_t ret = SDV_CMS_ParseDigestInfo(NULL, &cid, &output);
    ASSERT_EQ(ret, CRYPT_NULL_INPUT);

    ret = SDV_CMS_ParseDigestInfo((BSL_Buffer *)buff, &cid, NULL);
    ASSERT_EQ(ret, CRYPT_NULL_INPUT);

    ret = SDV_CMS_ParseDigestInfo((BSL_Buffer *)buff, &cid, &output);
    ASSERT_EQ(ret, CRYPT_SUCCESS);
...
void SDV_CMS_PARSE_DIGESTINFO_TC002(Hex *buff, int alg, Hex *digest)
{
    BSL_Buffer output = {0};
    BslCid cid = BSL_CID_UNKNOWN;
    int32_t ret =  SDV_CMS_ParseDigestInfo((BSL_Buffer *)buff, &cid, &output);
    ASSERT_EQ(ret, CRYPT_SUCCESS);
    ASSERT_EQ(alg, cid);
    ASSERT_EQ(memcmp(output.data, digest->x, digest->len), 0);
exit:
...
void SDV_CMS_ENCODE_DIGESTINFO_TC001()
{
    ...
    BslCid cid = 0;
    BSL_Buffer digest = {0};
    int32_t ret = SDV_CMS_EncodeDigestInfoBuff(BSL_CID_MD5, NULL, NULL);
    ASSERT_EQ(ret, CRYPT_NULL_INPUT);
    ret = SDV_CMS_EncodeDigestInfoBuff(BSL_CID_MD5, &input, NULL);
    ASSERT_EQ(ret, CRYPT_NULL_INPUT);
    input.dataLen = 1;
    ret = SDV_CMS_EncodeDigestInfoBuff(BSL_CID_MD5, &input, &output);
    ASSERT_EQ(ret, CRYPT_NULL_INPUT);
    input.dataLen = 0;
    ret = SDV_CMS_EncodeDigestInfoBuff(BSL_CID_MD5, &input, &output);
    ASSERT_EQ(ret, CRYPT_SUCCESS);
    ret = SDV_CMS_ParseDigestInfo(&output, &cid, &digest);
    ASSERT_EQ(ret, SDV_CMS_ERR_INVALID_DATA);
    BSL_SAL_FREE(output.data);
...
void SDV_CMS_ENCODE_DIGESTINFO_TC002(int algid, Hex *in)
{
    ...
    BslCid cid = 0;
    BSL_Buffer digest = {0};
    int32_t ret = SDV_CMS_EncodeDigestInfoBuff(algid, &input, &output);
    ASSERT_EQ(ret, CRYPT_SUCCESS);
    ret = SDV_CMS_ParseDigestInfo(&output, &cid, &digest);
    ASSERT_EQ(ret, CRYPT_SUCCESS);
    ASSERT_EQ(cid, algid);
