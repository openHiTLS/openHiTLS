...
void SDV_PKCS12_ENCODE_P12_TC003(Hex *buff)
{
    ...
    HITLS_X509_AttrsFree(p12_1.entityCert->attributes, HITLS_PKCS12_AttributesFree); // test p12-encode of entityCert attribute = NULL.
    p12_1.entityCert->attributes = NULL;
    ret = HITLS_PKCS12_GenBuff(BSL_FORMAT_ASN1, &p12_1, &encodeParam, true, &output4);
    ASSERT_EQ(ret, HITLS_X509_SUCCESS);

    HITLS_X509_AttrsFree(p12_1.key->attributes, HITLS_PKCS12_AttributesFree); // test p12-encode of key attribute = NULL.
