...
{
    HITLS_X509_DN dnName1[1] = {{BSL_CID_COMMONNAME, (uint8_t *)"OH", 2}};
    HITLS_X509_DN dnName2[1] = {{BSL_CID_COUNTRYNAME, (uint8_t *)"CN", 2}};
...
static int32_t SetCrlEntry(HITLS_X509_Crl *crl, BslList *issuerDN)
{
    ...
    ASSERT_EQ(HITLS_X509_CrlEntryCtrl(entry, HITLS_X509_CRL_SET_REVOKED_INVALID_TIME,&invalidTimeExt, sizeof(HITLS_X509_RevokeExtTime)), 0);
    ASSERT_EQ(HITLS_X509_CrlEntryCtrl(entry, HITLS_X509_CRL_SET_REVOKED_CERTISSUER,&certIssuer, sizeof(HITLS_X509_RevokeExtCertIssuer)), 0);
...
static int32_t SetCertExt(HITLS_X509_Cert *cert)
{
    ...
    oid = BSL_OBJ_GetOidFromCID(BSL_CID_CE_SERVERAUTH);
    ASSERT_NE(oid, NULL);
...
{
#if defined(HITLS_BSL_SAL_FILE) && defined(HITLS_CRYPTO_KEY_PARSE)
