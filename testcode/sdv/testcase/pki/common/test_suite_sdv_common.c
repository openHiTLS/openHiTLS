...
void SDV_CRYPT_EAL_DecodeBuffKey_Ex_TC001(void)
{
    ...
    ASSERT_EQ(CRYPT_EAL_DecodeBuffKey_Ex(NULL, NULL, BSL_FORMAT_ASN1, CRYPT_PUBKEY_RSA, NULL, NULL, NULL), 
        CRYPT_INVALID_ARG);
    ASSERT_EQ(CRYPT_EAL_DecodeBuffKey_Ex(NULL, NULL, BSL_FORMAT_ASN1, CRYPT_PUBKEY_RSA, &encode, NULL, NULL), 
        CRYPT_INVALID_ARG);
    
    // Test invalid encode buffer
    ASSERT_EQ(CRYPT_EAL_DecodeBuffKey_Ex(NULL, NULL, BSL_FORMAT_ASN1, CRYPT_PUBKEY_RSA, &encode, &pwd, &key), 
        CRYPT_INVALID_ARG);
    encode.data = data;
    ASSERT_EQ(CRYPT_EAL_DecodeBuffKey_Ex(NULL, NULL, BSL_FORMAT_ASN1, CRYPT_PUBKEY_RSA, &encode, &pwd, &key), 
        CRYPT_INVALID_ARG);
    ...
    ASSERT_EQ(CRYPT_EAL_DecodeBuffKey_Ex(NULL, NULL, 0xff, CRYPT_PUBKEY_RSA, &encode, &pwd, &key), 
        CRYPT_DECODE_NO_SUPPORT_FORMAT);

    // Test invalid type
    ASSERT_EQ(CRYPT_EAL_DecodeBuffKey_Ex(NULL, NULL, BSL_FORMAT_ASN1, 0xff, &encode, &pwd, &key), 
        CRYPT_DECODE_NO_SUPPORT_TYPE);

    // Test invalid password buffer for encrypted private key
    pwd.data = pwdData;
    pwd.dataLen = PWD_MAX_LEN + 1;
    ASSERT_EQ(CRYPT_EAL_DecodeBuffKey_Ex(NULL, NULL, BSL_FORMAT_ASN1, CRYPT_PRIKEY_PKCS8_ENCRYPT, 
...
void SDV_CRYPT_EAL_DecodeFileKey_Ex_TC001(void)
{
    ...
    // Test NULL parameters
    ASSERT_EQ(CRYPT_EAL_DecodeFileKey_Ex(NULL, NULL, BSL_FORMAT_ASN1, CRYPT_PUBKEY_RSA, NULL, NULL, NULL), 
        CRYPT_INVALID_ARG);

    // Test invalid path
    char longPath[PATH_MAX_LEN + 2] = {0};
    memset(longPath, 'a', PATH_MAX_LEN + 1);
    ASSERT_EQ(CRYPT_EAL_DecodeFileKey_Ex(NULL, NULL, BSL_FORMAT_ASN1, CRYPT_PUBKEY_RSA, longPath, &pwd, &key), 
        CRYPT_INVALID_ARG);

    // Test invalid format
    ASSERT_EQ(CRYPT_EAL_DecodeFileKey_Ex(NULL, NULL, 0xff, CRYPT_PUBKEY_RSA, 
        "../testdata/cert/asn1/rsa2048pub_pkcs1.der", &pwd, &key), CRYPT_DECODE_NO_SUPPORT_FORMAT);

    // Test invalid type
    ASSERT_EQ(CRYPT_EAL_DecodeFileKey_Ex(NULL, NULL, BSL_FORMAT_ASN1, 0xff, 
        "../testdata/cert/asn1/rsa2048pub_pkcs1.der", &pwd, &key), CRYPT_DECODE_NO_SUPPORT_TYPE);

    // Test invalid password buffer for encrypted private key
    pwd.data = pwdData;
    pwd.dataLen = PWD_MAX_LEN + 1;
    ASSERT_EQ(CRYPT_EAL_DecodeFileKey_Ex(NULL, NULL, BSL_FORMAT_ASN1, CRYPT_PRIKEY_PKCS8_ENCRYPT,
        "../testdata/cert/asn1/prime256v1_pkcs8_enc.der", &pwd, &key), CRYPT_INVALID_ARG);

    // Test non-existent file
    pwd.dataLen = 0;
    ASSERT_EQ(CRYPT_EAL_DecodeFileKey_Ex(NULL, NULL, BSL_FORMAT_ASN1, CRYPT_PUBKEY_RSA,
        "../testdata/cert/asn1/non_existent.der", &pwd, &key), BSL_FILE_NOT_EXIST);
