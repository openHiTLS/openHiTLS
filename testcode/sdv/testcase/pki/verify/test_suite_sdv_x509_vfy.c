...
typedef struct {
    HITLS_X509_StoreCtx *srcStore;
    uint32_t expectStoreCount;
    uint32_t expectCrlCount;
    int32_t expectMaxDepth;
    uint32_t expectSecurityBits;
    int64_t expectTime;
    uint64_t expectFlags;
    int32_t expectPurpose;
#ifdef HITLS_PKI_X509_VFY_LOCATION
    uint32_t expectCaPathCount;
#endif
    uint32_t loops;
    int32_t result;
...
static int32_t X509_RunStoreDupVerify(HITLS_X509_StoreCtx *storeCtx, const char *entityPath, uint32_t *chainCount)
{
    int32_t ret;
    HITLS_X509_Cert *entity = NULL;
    HITLS_X509_List *chain = NULL;
...
static bool X509_CheckStoreDupCloneState(const HITLS_X509_StoreCtx *dupStore, const X509StoreDupCloneThreadArg *threadArg)
{
    if (dupStore == NULL || threadArg == NULL) {
        return false;
    }
    if (X509_TestListCount(dupStore->store) != threadArg->expectStoreCount ||
        X509_TestListCount(dupStore->crl) != threadArg->expectCrlCount) {
        return false;
    }
    if (dupStore->verifyParam.maxDepth != threadArg->expectMaxDepth ||
        dupStore->verifyParam.securityBits != threadArg->expectSecurityBits ||
        dupStore->verifyParam.time != threadArg->expectTime ||
        dupStore->verifyParam.flags != threadArg->expectFlags ||
        dupStore->verifyParam.purpose != threadArg->expectPurpose) {
        return false;
    }
    if (dupStore->libCtx != threadArg->srcStore->libCtx ||
        dupStore->attrName != threadArg->srcStore->attrName ||
        dupStore->certChain != NULL) {
        return false;
    }
#ifdef HITLS_PKI_X509_VFY_LOCATION
    if (X509_TestListCount(dupStore->caPaths) != threadArg->expectCaPathCount) {
        return false;
    }
#endif
#ifdef HITLS_CRYPTO_SM2
    if (threadArg->srcStore->verifyParam.sm2UserId.data == NULL) {
        if (dupStore->verifyParam.sm2UserId.data != NULL) {
            return false;
        }
    } else if (dupStore->verifyParam.sm2UserId.data == NULL ||
        dupStore->verifyParam.sm2UserId.data == threadArg->srcStore->verifyParam.sm2UserId.data ||
        dupStore->verifyParam.sm2UserId.dataLen != threadArg->srcStore->verifyParam.sm2UserId.dataLen ||
        memcmp(dupStore->verifyParam.sm2UserId.data, threadArg->srcStore->verifyParam.sm2UserId.data,
            dupStore->verifyParam.sm2UserId.dataLen) != 0) {
        return false;
    }
#endif
#ifdef HITLS_PKI_X509_VFY_CB
    if (dupStore->verifyCb != threadArg->srcStore->verifyCb ||
        dupStore->usrData != threadArg->srcStore->usrData ||
        dupStore->error != 0 || dupStore->curDepth != 0 ||
        dupStore->curCert != NULL || dupStore->peerCertChain != NULL) {
        return false;
    }
#endif
    return true;
...
static void X509StoreDupCloneThread(void *arg)
{
    ...
    for (uint32_t i = 0; i < threadArg->loops; i++) {
        ...
        if (!X509_CheckStoreDupCloneState(dupStore, threadArg)) {
...
static void X509StoreDupVerifyThread(void *arg)
{
    ...
    for (uint32_t i = 0; i < threadArg->loops; i++) {
        ...
        int32_t ret = X509_RunStoreDupVerify(dupStore, threadArg->entityPath, &chainCount);
        HITLS_X509_StoreCtxFree(dupStore);
 ...
 *         4. Verify that duplicated list contents and basic verify parameters
 ...
 *         4. Verify that duplicated scalar settings and optional CA path changes stay isolated while
 *            both stores can still verify the same entity certificate successfully.
...
void SDV_X509_STORE_CTX_DUP_ISOLATION_TC001(char *rootCertPath, char *entityCertPath)
{
    ...
    storeCtx->verifyParam.securityBits = 384;

    ASSERT_EQ(storeCtx->verifyParam.maxDepth, 4);
    ASSERT_EQ(dupStore->verifyParam.maxDepth, 9);
    ASSERT_EQ(storeCtx->verifyParam.securityBits, 384);
    ...
    ASSERT_EQ(X509_RunStoreDupVerify(storeCtx, entityCertPath, &sourceChainCount),
        HITLS_PKI_SUCCESS);
    ASSERT_EQ(X509_RunStoreDupVerify(dupStore, entityCertPath, &dupChainCount), HITLS_PKI_SUCCESS);
...
void SDV_X509_STORE_CTX_DUP_MULTI_THREAD_TC001(char *rootCertPath, char *entityCertPath)
{
    ...
    ASSERT_EQ(HITLS_X509_StoreCtxCtrl(storeCtx, HITLS_X509_STORECTX_DEEP_COPY_SET_CA, root,
        sizeof(HITLS_X509_Cert)), HITLS_PKI_SUCCESS);
    storeCtx->verifyParam.maxDepth = 6;
    storeCtx->verifyParam.securityBits = 192;
    storeCtx->verifyParam.time = 0;
    storeCtx->verifyParam.flags = 0;
    storeCtx->verifyParam.purpose = 0;
#ifdef HITLS_PKI_X509_VFY_LOCATION
    ASSERT_EQ(HITLS_X509_StoreCtxCtrl(storeCtx, HITLS_X509_STORECTX_ADD_CA_PATH,
        (void *)"/tmp/x509-store-dup-thread-path", strlen("/tmp/x509-store-dup-thread-path")), HITLS_PKI_SUCCESS);
#endif
#ifdef HITLS_CRYPTO_SM2
    ASSERT_EQ(HITLS_X509_StoreCtxCtrl(storeCtx, HITLS_X509_STORECTX_SET_VFY_SM2_USERID, "dup-thread-sm2-user-id",
        strlen("dup-thread-sm2-user-id")), HITLS_PKI_SUCCESS);
#endif

    ASSERT_EQ(X509_RunStoreDupVerify(storeCtx, entityCertPath, &expectChainCount), HITLS_PKI_SUCCESS);
    ASSERT_TRUE(expectChainCount > 0);

    cloneArg.srcStore = storeCtx;
    cloneArg.expectStoreCount = X509_TestListCount(storeCtx->store);
    cloneArg.expectCrlCount = X509_TestListCount(storeCtx->crl);
    cloneArg.expectMaxDepth = storeCtx->verifyParam.maxDepth;
    cloneArg.expectSecurityBits = storeCtx->verifyParam.securityBits;
    cloneArg.expectTime = storeCtx->verifyParam.time;
    cloneArg.expectFlags = storeCtx->verifyParam.flags;
    cloneArg.expectPurpose = storeCtx->verifyParam.purpose;
#ifdef HITLS_PKI_X509_VFY_LOCATION
    cloneArg.expectCaPathCount = X509_TestListCount(storeCtx->caPaths);
#endif
