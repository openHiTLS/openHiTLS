 ...
 */
/* BEGIN_CASE */
void SDV_CRYPTO_RSA_RSABSSA_BLINDING_FUNC_TC001(int keyLen, int hashId, Hex *msg, int saltLen)
{
    if (IsMdAlgDisabled(hashId)) {
        SKIP_TEST();
    }
    ...
    ASSERT_TRUE(CRYPT_EAL_PkeyCtrl(pkey, CRYPT_CTRL_SET_RSA_EMSA_PSS, paraPtr, paraSize) == CRYPT_SUCCESS);
    ...
    /* public key verify */
    ASSERT_TRUE(CRYPT_EAL_PkeyVerify(pkey, hashId, msg->x, msg->len, unBlindSig, unBlindSigLen) == CRYPT_SUCCESS);
    ...
    Hex *blindSigBuf, Hex *sigBuf)
{
    ...
    CRYPT_EAL_PkeyCtx *pkey = NULL;
    CRYPT_RSA_PssPara pssParam = {
        .saltLen = salt->len,
        .mdId = CRYPT_MD_SHA384,
        .mgfId = CRYPT_MD_SHA384
    };
    pkey = CRYPT_EAL_PkeyNewCtx(CRYPT_PKEY_RSA);
    ASSERT_TRUE(pkey != NULL);
...
void SDV_CRYPTO_RSA_RSABSSA_BLINDING_INVALID_PARAM_TC001(void)
{
    ...
    ASSERT_TRUE(CRYPT_EAL_PkeyBlind(pkey, CRYPT_MD_SHA384, msg, msgLen, blindMsg, &smallLen) == CRYPT_RSA_BUFF_LEN_NOT_ENOUGH);
    ASSERT_TRUE(CRYPT_EAL_PkeyUnBlind(pkey, msg, msgLen, unBlindSig, &smallLen) == CRYPT_RSA_BUFF_LEN_NOT_ENOUGH);

    // pad type is wrong.
    ASSERT_TRUE(CRYPT_EAL_PkeyBlind(pkey, CRYPT_MD_SHA384, msg, msgLen, blindMsg, &blindMsgLen) == CRYPT_RSA_PAD_NO_SET_ERROR);
    ...
    ASSERT_TRUE(CRYPT_EAL_PkeyBlind(pkey, CRYPT_MD_SHA256, msg, msgLen, blindMsg, &unBlindSigLen) == CRYPT_RSA_ERR_MD_ALGID);
exit:
    CRYPT_EAL_PkeyFreeCtx(pkey);
}
/* END_CASE */