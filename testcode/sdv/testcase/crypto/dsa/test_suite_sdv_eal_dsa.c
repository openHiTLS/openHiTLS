 ...
 */
/* BEGIN_CASE */
void SDV_CRYPTO_DSA_SET_PARAEX_API_TC001(Hex *p, Hex *q, Hex *g, int isProvider)
{
    CRYPT_EAL_PkeyCtx *pkey = NULL;
    uint8_t tmp[1];
...
/**
 * @test   SDV_CRYPTO_DSA_GET_PARAEX_API_TC001
 * @title  DSA: CRYPT_EAL_PkeyGetParaEx test.
 * @precon Registering memory-related functions.
 *         Dsa para vertors.
 * @brief
 *    1. Create the contexts of the dsa algorithm, expected result 1.
 *    2. Call the CRYPT_EAL_PkeySetParaEx method with correct parameters, expected result 2
 *    3. Call the CRYPT_EAL_PkeySetParaEx method: para.id != pkey.id, expected result 3
 *    4. Call the CRYPT_EAL_PkeySetParaEx method: pkey=NULL or para=NULL, expected result 4
 *    5. Call the CRYPT_EAL_PkeySetParaEx method with correct parameters, expected result 5
 *    6. Check whether the configured parameters are the same as the obtained parameters, expected result 6
 * @expect
 *    1. Success, and context is not NULL.
 *    2. CRYPT_SUCCESS
 ...
 */
/* BEGIN_CASE */
void SDV_CRYPTO_DSA_GET_PARAEX_API_TC001(Hex *p, Hex *q, Hex *g, int isProvider)
{
    uint8_t buf_p[1030] = {0};
    uint32_t bufLen = sizeof(buf_p);
    uint8_t buf_q[1030] = {0};
    ...
    CRYPT_EAL_PkeyPara para1 = {0};
    CRYPT_EAL_PkeyPara para2 = {0};
    CRYPT_Param param1 = {0};
    CRYPT_Param param2 = {0};
    Set_DSA_Para(&para1, NULL, NULL, p, q, g, NULL, NULL);
    param1.param = &para1.para.dhPara;
    param1.paramLen = sizeof(para1.para.dhPara);
    Set_DSA_Para(&para2, NULL, NULL, &getP, &getQ, &getG, NULL, NULL);
    param2.param = &para2.para.dhPara;
    param2.paramLen = sizeof(para2.para.dhPara);
    TestMemInit();
    CRYPT_EAL_PkeyCtx *pKey = NULL;
    if (isProvider == 1) {
        pKey = CRYPT_EAL_ProviderPkeyNewCtx(NULL, CRYPT_PKEY_DSA, CRYPT_EAL_PKEY_KEYMGMT_OPERATE, "provider=default");
    ...
    para2.id = CRYPT_PKEY_RSA;
    param2.param = &para2.para.dhPara;
    ASSERT_TRUE(CRYPT_EAL_PkeySetParaEx(pKey, &param1) == CRYPT_SUCCESS);

    ASSERT_TRUE(CRYPT_EAL_PkeyGetParaEx(NULL, &param2) == CRYPT_NULL_INPUT);
    ASSERT_TRUE(CRYPT_EAL_PkeyGetParaEx(pKey, NULL) == CRYPT_NULL_INPUT);

    para2.id = CRYPT_PKEY_DSA;
    param2.param = &para2.para.dhPara;
    ASSERT_TRUE(CRYPT_EAL_PkeyGetParaEx(pKey, &param2) == CRYPT_SUCCESS);
    ASSERT_TRUE(para1.para.dsaPara.pLen == para2.para.dsaPara.pLen);
    ASSERT_TRUE(memcmp(para1.para.dsaPara.p, para2.para.dsaPara.p, para1.para.dsaPara.pLen) == 0);
    ASSERT_TRUE(para1.para.dsaPara.qLen == para2.para.dsaPara.qLen);
    ASSERT_TRUE(memcmp(para1.para.dsaPara.q, para2.para.dsaPara.q, para1.para.dsaPara.qLen) == 0);
