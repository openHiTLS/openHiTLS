...
/* BEGIN_CASE */
void SDV_CRYPTO_SM4_GETINFO_API_TC001(void)
{
    TestMemInit();
    uint32_t val = 0;

    ASSERT_EQ(CRYPT_EAL_CipherGetInfo(CRYPT_CIPHER_SM4_CCM, CRYPT_INFO_IS_AEAD, &val), CRYPT_SUCCESS);
    ASSERT_EQ(val, 1);

    ASSERT_EQ(CRYPT_EAL_CipherGetInfo(CRYPT_CIPHER_SM4_CCM, CRYPT_INFO_KEY_LEN, &val), CRYPT_SUCCESS);
    ASSERT_EQ(val, 16);

    ASSERT_EQ(CRYPT_EAL_CipherGetInfo(CRYPT_CIPHER_SM4_CCM, CRYPT_INFO_IV_LEN, &val), CRYPT_SUCCESS);
    ASSERT_EQ(val, 12);

    ASSERT_EQ(CRYPT_EAL_CipherGetInfo(CRYPT_CIPHER_SM4_CCM, CRYPT_INFO_BLOCK_LEN, &val), CRYPT_SUCCESS);
    ASSERT_EQ(val, 1);

    ASSERT_EQ(CRYPT_EAL_CipherGetInfo(CRYPT_CIPHER_SM4_CCM, CRYPT_INFO_IS_STREAM, &val), CRYPT_SUCCESS);
    ASSERT_EQ(val, 1);
