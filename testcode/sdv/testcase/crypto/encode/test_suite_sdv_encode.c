 ...
 * @title  Test CRYPT_EAL_EncodeSignBn normal encode function
 */
/* BEGIN_CASE */
void SDV_ENCODE_SIGN_BN_FUNC_TC001(Hex *r, Hex *s, Hex *expect)
{
    ...
    ASSERT_EQ(CRYPT_EAL_EncodeSignBn(bnR, bnS, encode, &encodeLen), CRYPT_SUCCESS);
 ...
 * @title  Test CRYPT_EAL_EncodeSignBn abnormal input parameter
 */
/* BEGIN_CASE */
void SDV_ENCODE_SIGN_BN_API_TC001(Hex *r, Hex *s)
{
    ...
    ASSERT_EQ(CRYPT_EAL_EncodeSignBn(bnR, bnS, encode, &encodeLen), CRYPT_INVALID_ARG);
    ASSERT_TRUE(BN_Bin2Bn(bnR, r->x, r->len) == CRYPT_SUCCESS);
    ASSERT_EQ(CRYPT_EAL_EncodeSignBn(bnR, bnS, encode, &encodeLen), CRYPT_INVALID_ARG);
    ASSERT_TRUE(BN_Bin2Bn(bnS, s->x, s->len) == CRYPT_SUCCESS);

    // Test null pointer
    ASSERT_EQ(CRYPT_EAL_EncodeSignBn(NULL, bnS, encode, &encodeLen), CRYPT_NULL_INPUT);
    ASSERT_EQ(CRYPT_EAL_EncodeSignBn(bnR, NULL, encode, &encodeLen), CRYPT_NULL_INPUT);
    ASSERT_EQ(CRYPT_EAL_EncodeSignBn(bnR, bnS, NULL, &encodeLen), CRYPT_NULL_INPUT);
    ASSERT_EQ(CRYPT_EAL_EncodeSignBn(bnR, bnS, encode, NULL), CRYPT_NULL_INPUT);
    ...
    encodeLen = 1;
    ASSERT_EQ(CRYPT_EAL_EncodeSignBn(bnR, bnS, encode, &encodeLen), CRYPT_ENCODE_BUFF_NOT_ENOUGH);
 ...
 * @title  Test CRYPT_EAL_DecodeSignBn normal decode function
 */
/* BEGIN_CASE */
void SDV_DECODE_SIGN_BN_FUNC_TC001(Hex *encode, Hex *expectR, Hex *expectS, int ret)
{
    ...
    ASSERT_EQ(CRYPT_EAL_DecodeSignBn(encode->x, encode->len, bnR, bnS), ret);
 ...
 * @title  Test CRYPT_EAL_DecodeSignBn abnormal input parameter
 */
/* BEGIN_CASE */
void SDV_DECODE_SIGN_BN_API_TC001(Hex *encode)
{
    ...
    // Test null pointer
    ASSERT_EQ(CRYPT_EAL_DecodeSignBn(NULL, encode->len, bnR, bnS), CRYPT_NULL_INPUT);
    ASSERT_EQ(CRYPT_EAL_DecodeSignBn(encode->x, 0, bnR, bnS), CRYPT_NULL_INPUT);
    ASSERT_EQ(CRYPT_EAL_DecodeSignBn(encode->x, encode->len, NULL, bnS), CRYPT_NULL_INPUT);
    ASSERT_EQ(CRYPT_EAL_DecodeSignBn(encode->x, encode->len, bnR, NULL), CRYPT_NULL_INPUT);
...
void SDV_ENCODE_DECODE_SIGN_COMBO_TC001(Hex *r, Hex *s)
{
    ...
    // Decode signature
    ASSERT_EQ(CRYPT_EAL_DecodeSignBn(encode, encodeLen, decR, decS), CRYPT_SUCCESS);
