...
void SDV_BSL_ASN1_PARSE_ENCPK8_TC001(int isProvider, char *path, int fileType, Hex *pass, int mdId, Hex *msg, Hex *sign)
{
    ...
    CRYPT_EAL_PkeyCtx *pkeyCtx = NULL;
    if (isProvider) {
        ASSERT_EQ(CRYPT_EAL_ProviderDecodeFileKey(NULL, NULL, BSL_FORMAT_ASN1, fileType, path, &pwdBuffer, &pkeyCtx), 0);
    } else {
...
void SDV_BSL_ASN1_PARSE_BUFF_PROVIDER_TC001(int format, int type, char *path, Hex *password)
{
    ...
    CRYPT_EAL_PkeyCtx *pkeyCtx = NULL;
    ASSERT_EQ(CRYPT_EAL_ProviderDecodeBuffKey(NULL, NULL, BSL_FORMAT_UNKNOWN, CRYPT_ENCODE_UNKNOW, &encode, &pass, &pkeyCtx), 
        CRYPT_SUCCESS);
    CRYPT_EAL_PkeyFreeCtx(pkeyCtx);
    pkeyCtx = NULL;
    encode.data = data;
