...
void SDV_CRYPTO_SLH_DSA_API_CTRL_TC001(void)
{
    ...
    ASSERT_TRUE(CRYPT_EAL_PkeyCtrl(pkey, CRYPT_CTRL_SET_SLH_DSA_PREHASH_ID, &prehashId, sizeof(prehashId)) == CRYPT_SUCCESS);
    prehashId = CRYPT_MD_MAX;
    ASSERT_TRUE(CRYPT_EAL_PkeyCtrl(pkey, CRYPT_CTRL_SET_SLH_DSA_PREHASH_ID, &prehashId, sizeof(prehashId)) == CRYPT_SLHDSA_ERR_PREHASH_ID_NOT_SUPPORTED);
EXIT:
    CRYPT_EAL_PkeyFreeCtx(pkey);
    return;
}
...
/* BEGIN_CASE */
void SDV_CRYPTO_SLH_DSA_GENKEY_TC001(void)
{
    TestMemInit();
    TestRandInit();
    CRYPT_EAL_PkeyCtx *pkey = NULL;
    ...
    ASSERT_TRUE(CRYPT_EAL_PkeyCtrl(pkey, CRYPT_CTRL_SET_SLH_DSA_ALG_ID, (void *)&algId, sizeof(algId)) == CRYPT_SUCCESS);
...
void SDV_CRYPTO_SLH_DSA_SIGN_KAT_TC001(int id, Hex *key, Hex *addrand, Hex *msg, Hex *context, Hex *sig)
{
    ...
    if (addrand->len == 0) {
        ...
        ASSERT_EQ(CRYPT_EAL_PkeyCtrl(pkey, CRYPT_CTRL_SET_SLH_DSA_DETERMINISTIC, (void *)&isDeterministic, sizeof(isDeterministic)), CRYPT_SUCCESS);
    } else {
        ASSERT_EQ(CRYPT_EAL_PkeyCtrl(pkey, CRYPT_CTRL_SET_SLH_DSA_ADDRAND, (void *)addrand->x, addrand->len), CRYPT_SUCCESS);
    }
...
void SDV_CRYPTO_SLH_DSA_SIGN_KAT_TC002(int id, Hex *key, Hex *msg, Hex *context, int prehashId, Hex *sig)
{
    ...
    ASSERT_EQ(CRYPT_EAL_PkeyCtrl(pkey, CRYPT_CTRL_SET_SLH_DSA_PREHASH_ID, (void *)&prehash, sizeof(prehash)), CRYPT_SUCCESS);
    CRYPT_EAL_PkeyPrv prv;
    (void)memset_s(&prv, sizeof(CRYPT_EAL_PkeyPrv), 0, sizeof(CRYPT_EAL_PkeyPrv));
    prv.id = CRYPT_PKEY_SLH_DSA;
    prv.key.slhDsaPrv.seed = key->x;
