...
void SDV_CRYPTO_ELGAMAL_SET_PRV_API_TC001(Hex *q,int k_bits, int bits, int isProvider)
{
    ...
    prvKey.key.elgamalPrv.q= prvQ;
    prvKey.key.elgamalPrv.qLen = 600;
 ...
 */
/* BEGIN_CASE */
void SDV_CRYPTO_ELGAMAL_DEC_API_TC001(Hex *q,Hex *p, Hex *g ,Hex *x,  Hex *in, int isProvider)
{
    uint8_t crypt[512];
    uint32_t cryptLen = 512;
    CRYPT_EAL_PkeyPrv prvkey = {0};
    ...
    prvkey.key.elgamalPrv.q = q->x;
    prvkey.key.elgamalPrv.qLen = q->len;
...
int Compare_PrvKey(CRYPT_EAL_PkeyPrv *prvKey1, CRYPT_EAL_PkeyPrv *prvKey2)
{
    if (prvKey1->key.elgamalPrv.pLen != prvKey2->key.elgamalPrv.pLen || prvKey1->key.elgamalPrv.gLen != prvKey2->key.elgamalPrv.gLen ||
        prvKey1->key.elgamalPrv.xLen != prvKey2->key.elgamalPrv.xLen || prvKey1->key.elgamalPrv.qLen != prvKey2->key.elgamalPrv.qLen) {
        return -1;  // -1 indicates failure
    }
    if (memcmp(prvKey1->key.elgamalPrv.g, prvKey2->key.elgamalPrv.g, prvKey1->key.elgamalPrv.gLen) != 0 ||
        ...
        memcmp(prvKey1->key.elgamalPrv.q, prvKey2->key.elgamalPrv.q, prvKey1->key.elgamalPrv.qLen) != 0 ||
...
void SDV_CRYPTO_ELGAMAL_SET_KEY_API_TC001( Hex *q, int k_bits, int bits, int isProvider)
{
    ...
    /* pkey1 */
    /* Generate a key pair. */
    printf("%x",CRYPT_EAL_PkeySetPara(pkey1, &para));
