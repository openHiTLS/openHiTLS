    ...
    uint8_t *encapsulatedKey, uint32_t *encapsulatedKeyLen)
{
    ...
    ret = CRYPT_EAL_HpkeGenerateKeyPair(cipherSuite, ikmE->x, ikmE->len, &pkeyS1, NULL, NULL);
    ASSERT_EQ(ret, CRYPT_SUCCESS);

    ret = CRYPT_EAL_HpkeGenerateKeyPair(cipherSuite, ikmR->x, ikmR->len, &pkeyR1, NULL, NULL);
    ASSERT_EQ(ret, CRYPT_SUCCESS);
    ...
    ctxS1 = CRYPT_EAL_HpkeNewCtx(CRYPT_HPKE_SENDER, mode, cipherSuite, NULL, NULL);
    ...
    ctxR1 = CRYPT_EAL_HpkeNewCtx(CRYPT_HPKE_RECIPIENT, mode, cipherSuite, NULL, NULL);
...
static int32_t HpkeRandomTest(CRYPT_HPKE_Mode mode, CRYPT_HPKE_KEM_AlgId kemId, CRYPT_HPKE_KDF_AlgId kdfId, CRYPT_HPKE_AEAD_AlgId aeadId)
{
    ...
    ret = CRYPT_EAL_HpkeGenerateKeyPair(cipherSuite, NULL, 0, &pkeyS, NULL, NULL);
    ASSERT_EQ(ret, CRYPT_SUCCESS);

    // prepare Recipient key
    ret = CRYPT_EAL_HpkeGenerateKeyPair(cipherSuite, NULL, 0, &pkeyR, NULL, NULL);
    ASSERT_EQ(ret, CRYPT_SUCCESS);
    ...
    // Sender init
    ctxS = CRYPT_EAL_HpkeNewCtx(CRYPT_HPKE_SENDER, mode, cipherSuite, NULL, NULL);
    ...
    // Recipient init
    ctxR = CRYPT_EAL_HpkeNewCtx(CRYPT_HPKE_RECIPIENT, mode, cipherSuite, NULL, NULL);
 ...
 */
/* BEGIN_CASE */
void SDV_CRYPT_EAL_HPKE_API_TC005(int role)
{
    CRYPT_EAL_RandInit(CRYPT_RAND_SHA256, NULL, NULL, NULL, 0);
    ...
    hpkeCtx = CRYPT_EAL_HpkeNewCtx(role, CRYPT_HPKE_MODE_BASE, cipherSuite, NULL, NULL);
    ...
    hpkeCtx = CRYPT_EAL_HpkeNewCtx(role, CRYPT_HPKE_MODE_BASE, cipherSuite, NULL, "provider=none");
    ASSERT_TRUE(hpkeCtx == NULL);

    hpkeCtx = CRYPT_EAL_HpkeNewCtx(role, CRYPT_HPKE_MODE_BASE, cipherSuite, NULL, "provider=default");
    ASSERT_TRUE(hpkeCtx != NULL);
    ...
    ret = CRYPT_EAL_HpkeSeal(hpkeCtx, NULL, 0, massage, massageLen, cipherText, &cipherTextLen);
    ASSERT_EQ(ret, CRYPT_HPKE_ERR_CALL);
    ...
    ret = CRYPT_EAL_HpkeSetSeq(hpkeCtx, 0xFFFFFFFFFFFFFFFF);
    ASSERT_EQ(ret, CRYPT_INVALID_ARG);

    ret = CRYPT_EAL_HpkeOpen(hpkeCtx, NULL, 0, massage, massageLen, cipherText, &cipherTextLen);
    ASSERT_EQ(ret, CRYPT_HPKE_ERR_CALL);

    ret = CRYPT_EAL_HpkeExportSecret(hpkeCtx, NULL, 0, buff, 0);
    ASSERT_EQ(ret, CRYPT_NULL_INPUT);

    ret = CRYPT_EAL_HpkeExportSecret(hpkeCtx, NULL, 0, buff, buffLen);
    ASSERT_EQ(ret, CRYPT_HPKE_ERR_CALL);
    ...
    CRYPT_HPKE_CipherSuite cipherSuite, uint8_t *info, uint32_t infoLen, uint8_t *sharedSecret, uint32_t sharedSecretLen)
{
    ...
    ctx = CRYPT_EAL_HpkeNewCtx(role, mode, cipherSuite, NULL, NULL);
    ...
    for (i = 0; i < sizeof(kemIds) / sizeof(CRYPT_HPKE_KEM_AlgId); i++) {
        for (j = 0; j < sizeof(kdfIds) / sizeof(CRYPT_HPKE_KDF_AlgId); j++) {
            for (k = 0; k < sizeof(aeadIds) / sizeof(CRYPT_HPKE_AEAD_AlgId); k++) {
                ...
                ASSERT_EQ(CRYPT_EAL_HpkeGenerateKeyPair(cipherSuite, NULL, 0, &pctx, NULL, NULL), CRYPT_SUCCESS);
                CRYPT_EAL_PkeyFreeCtx(pctx);

                uint32_t ikmLen = 1024*1024;
                uint8_t *ikm = (uint8_t *)malloc(ikmLen);
                memset_s(ikm, ikmLen, 0xFF, ikmLen);
                ASSERT_EQ(CRYPT_EAL_HpkeGenerateKeyPair(cipherSuite, ikm, ikmLen, &pctx, NULL, NULL), CRYPT_SUCCESS);
                CRYPT_EAL_PkeyFreeCtx(pctx);
                free(ikm);
            }
        }
