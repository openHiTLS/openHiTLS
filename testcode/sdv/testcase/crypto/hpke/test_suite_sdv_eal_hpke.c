...
{
    // check symkey
    uint8_t symkey[HPKE_AEAD_MAX_KEY_LEN] = { 0 };
    uint32_t symkeyLen = HPKE_AEAD_MAX_KEY_LEN;
    ...
    uint32_t nonceLen = HPKE_AEAD_NONCE_LEN;
    ...
    // check base nonce
    uint8_t exporterS[HPKE_HKDF_MAX_EXTRACT_KEY_LEN] = { 0 };
    uint32_t exporterSLen = HPKE_HKDF_MAX_EXTRACT_KEY_LEN;
...
void SDV_CRYPT_EAL_HPKE_API_TC004(void)
{
    ...
    CRYPT_HPKE_AEAD_AlgId aeadIds[] = {CRYPT_AEAD_AES_GCM_128, CRYPT_AEAD_AES_GCM_256, CRYPT_AEAD_CHACHA20_POLY1305};
...
static CRYPT_EAL_HpkeCtx *GenHpkeCtxWithoutKenDerive(CRYPT_HPKE_Role role, CRYPT_HPKE_mode mode, CRYPT_HPKE_CipherSuite cipherSuite, uint8_t *symKey, uint32_t symKeyLen,
    uint8_t *baseNonce, uint32_t baseNonceLen)
{
    CRYPT_EAL_HpkeCtx *ctx = NULL;

    ctx = CRYPT_EAL_HpkeNewCtx(role, mode, cipherSuite);
    ASSERT_TRUE(ctx != NULL);
    ASSERT_EQ(CRYPT_EAL_HpkeSetParam(ctx, CRYPT_HPKE_PARAM_SYM_KEY, symKey, symKeyLen), CRYPT_SUCCESS);
    ASSERT_EQ(CRYPT_EAL_HpkeSetParam(ctx, CRYPT_HPKE_PARAM_BASE_NONCE, baseNonce, baseNonceLen), CRYPT_SUCCESS);
    return ctx;
exit:
...
static int32_t HpkeTestImportKeyAndBaseNonce(CRYPT_HPKE_mode mode, CRYPT_HPKE_CipherSuite cipherSuite)
{
    CRYPT_EAL_HpkeCtx *ctxS = NULL;
    CRYPT_EAL_HpkeCtx *ctxR = NULL;

    uint32_t keyLen = 32;
    if (cipherSuite.aeadId == CRYPT_AEAD_AES_GCM_128) {
        keyLen = 16;
    }
    uint32_t baseNonceLen = HPKE_AEAD_NONCE_LEN;

    uint8_t symKey[HPKE_AEAD_MAX_KEY_LEN];;
    uint8_t baseNonce[HPKE_AEAD_NONCE_LEN];


    ctxS = GenHpkeCtxWithoutKenDerive(CRYPT_HPKE_SENDER, mode, cipherSuite, symKey, keyLen, baseNonce, baseNonceLen);
    ASSERT_TRUE(ctxS != NULL);

    ctxR = GenHpkeCtxWithoutKenDerive(CRYPT_HPKE_RECIPIENT, mode, cipherSuite, symKey, keyLen, baseNonce, baseNonceLen);
    ASSERT_TRUE(ctxR != NULL);
 ...
 * @title  import key and base_nonce test.
 * @precon nan
 * @brief
 *    1.NA.
 * @expect
    ...
    for (i = 0; i < sizeof(kemIds) / sizeof(CRYPT_HPKE_KEM_AlgId); i++) {
        for (j = 0; j < sizeof(kdfIds) / sizeof(CRYPT_HPKE_KDF_AlgId); j++) {
            for (k = 0; k < sizeof(aeadIds) / sizeof(CRYPT_HPKE_AEAD_AlgId); k++) {
                ...
                ASSERT_EQ(HpkeTestImportKeyAndBaseNonce(mode, cipherSuite), CRYPT_SUCCESS);
            }
        }
    }
exit:
 ...
 * @title  import key and base_nonce test based on standard vector.
 * @precon nan
 * @brief
 *    1.NA.
 * @expect
 ...
 */
/* BEGIN_CASE */
void SDV_CRYPT_EAL_HPKE_API_TC007(int mode, int kemId, int kdfId, int aeadId, Hex *symKey, Hex *baseNonce, int seq, Hex *pt, Hex *aad, Hex *ct)
{
    CRYPT_EAL_RandInit(CRYPT_RAND_SHA256, NULL, NULL, NULL, 0);
    ...
    ctxS = GenHpkeCtxWithoutKenDerive(CRYPT_HPKE_SENDER, mode, cipherSuite, symKey->x, symKey->len, baseNonce->x, baseNonce->len);
    ASSERT_TRUE(ctxS != NULL);

    ctxR = GenHpkeCtxWithoutKenDerive(CRYPT_HPKE_RECIPIENT, mode, cipherSuite, symKey->x, symKey->len, baseNonce->x, baseNonce->len);
    ASSERT_TRUE(ctxR != NULL);
 ...
 * @title  import exporter secret and test export secret.
 * @precon nan
 * @brief
 *    1.NA.
 * @expect
 ...
 */
/* BEGIN_CASE */
void SDV_CRYPT_EAL_HPKE_API_TC008(int mode, int kemId, int kdfId, int aeadId, Hex *importerSecret, Hex *exporterContext, int L, Hex *exportedValue)
{
    CRYPT_EAL_RandInit(CRYPT_RAND_SHA256, NULL, NULL, NULL, 0);
    ...
    ctxS = CRYPT_EAL_HpkeNewCtx(CRYPT_HPKE_SENDER, mode, cipherSuite);
    ASSERT_TRUE(ctxS != NULL);
    ASSERT_EQ(CRYPT_EAL_HpkeSetParam(ctxS, CRYPT_HPKE_PARAM_EXPORTER_SECRET, importerSecret->x, importerSecret->len), CRYPT_SUCCESS);

    ctxR = CRYPT_EAL_HpkeNewCtx(CRYPT_HPKE_RECIPIENT, mode, cipherSuite);
    ASSERT_TRUE(ctxR != NULL);
    ASSERT_EQ(CRYPT_EAL_HpkeSetParam(ctxR, CRYPT_HPKE_PARAM_EXPORTER_SECRET, importerSecret->x, importerSecret->len), CRYPT_SUCCESS);
