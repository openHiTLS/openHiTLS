...
static int32_t Sm2KeyGen(void *ctx)
{
    int rc = CRYPT_SUCCESS;
    BENCH_TIMES(rc = CRYPT_EAL_PkeyGen(ctx), rc, CRYPT_SUCCESS, 1000, "sm2 keyGen");
    return rc;
}
...
static int32_t Sm2KeyDerive(void *ctx)
{
    int rc = CRYPT_SUCCESS;
    ...
    BENCH_TIMES(Sm2KeyDeriveInner(ctx, peerCtx), rc, CRYPT_SUCCESS, 1000, "sm2 keyDerive");
ERR_OUT:
    CRYPT_EAL_PkeyFreeCtx(peerCtx);
    return rc;
}
...
static int32_t Sm2Enc(void *ctx)
{
    int rc = CRYPT_SUCCESS;
    BENCH_TIMES(Sm2EncInner(ctx), rc, CRYPT_SUCCESS, 10000, "sm2 enc");
    return rc;
}

static int32_t Sm2Dec(void *ctx)
{
    int rc;
    uint8_t plainText[32];
    ...
    BENCH_TIMES(CRYPT_EAL_PkeyDecrypt(ctx, cipherText, outLen, plainText, &plainTextLen), rc, CRYPT_SUCCESS, 10000,
                "sm2 dec");
    return rc;
}
...
static int32_t Sm2Sign(void *ctx)
{
    int rc;
    BENCH_TIMES(Sm2SignInner(ctx), rc, CRYPT_SUCCESS, 10000, "sm2 sign");
    return rc;
}

static int32_t Sm2Verify(void *ctx)
{
    int rc;
    uint8_t plainText[32];
    uint8_t signature[256];
    ...
    BENCH_TIMES(CRYPT_EAL_PkeyVerify(ctx, CRYPT_MD_SM3, plainText, sizeof(plainText), signature, signatureLen), rc,
                CRYPT_SUCCESS, 10000, "sm2 verify");
    return rc;
}
