...
static int32_t InstantOperation(Operation *op, void *ctx)
{
    if (op->id & KEY_GEN_ID) {
        return ((KeyGen)op->oper)(ctx);
    }
    if (op->id & KEY_DERIVE_ID) {
        return ((KeyDerive)op->oper)(ctx);
    }
    if (op->id & ENC_ID) {
        return ((Enc)op->oper)(ctx);
    }
    if (op->id & DEC_ID) {
        return ((Dec)op->oper)(ctx);
    }
    if (op->id & SIGN_ID) {
        return ((Sign)op->oper)(ctx);
    }
    if (op->id & VERIFY_ID) {
        return ((Verify)op->oper)(ctx);
    }
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < num; i++) {
        CtxOps *ctxOps = benchs[i]->ctxOps;
        void *ctx = NULL;

        BENCH_SETUP(ctx, ctxOps);

        for (int j = 0; j < benchs[i]->filteredOpsNum; j++) {
            Operation *op = &ctxOps->ops[j];
            ret = InstantOperation(op, ctx);
