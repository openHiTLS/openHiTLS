...
int main(int32_t argc, char *argv[])
{
    ...
    HITLS_CERT_X509 *serverCert = NULL;
    ...
    /* 加载证书：需要用户实现 */
    ret = HITLS_X509_ParseFileCert(BSL_PARSE_FORMAT_ASN1, CERTS_PATH "ca.der", &rootCA);
    if (ret != HITLS_SUCCESS) {
        printf("Parse ca failed.\n");
        goto exit;
    }
    ret = HITLS_X509_ParseFileCert(BSL_PARSE_FORMAT_ASN1, CERTS_PATH "inter.der", &subCA);
    if (ret != HITLS_SUCCESS) {
...
exit:
    ...
    HITLS_X509_Adapt_CertFree(serverCert);
